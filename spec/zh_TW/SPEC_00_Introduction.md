...
| **`v0.10.0-draft.1`** (目前) | **`v2`**（凍結） | `v1`~`v2` | 在席是誰（REAL_02 §5.1.2「重建序」補足、§4.2.6.3 結案）：耐久記錄必須使收受順序成為**全序**，重建／壓實／載入三處同一個順序；平手**不得**單以對方身分打破（純由對方決定的順序與受害者無關，磨一次通吃）；新 MUST「缺席與不可解析不是同一件事」。§4.3.5.1 的抽樣解明寫**不**移轉——同一個形狀不蘊涵同一個答案 |
