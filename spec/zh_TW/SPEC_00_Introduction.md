...
| **`v0.11.0-draft.1`** (目前) | **`v2`**（凍結） | `v1`~`v2` | 原子寫入(REAL_01 §4 收尾):耐久寫入**必須**為原子安裝——同目錄臨時檔 + `fsync` + `rename`,並行讀者不得看見截斷或短暫缺席的目標;失敗不得留下可被目錄掃描誤認為內容的殘檔。SPEC_10 §4.1.2 一併拆解:**收斂撞 ⊥ 必須停下並回報**(觀測的責任),**要不要固化由操作者決定**;固化的單位是 savepoint 不是「提交」 |
