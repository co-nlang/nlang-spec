...
| **`v0.16.0-draft.1`** (目前) | **`v2`**（凍結） | `v1`~`v2` | **政策的上限與實作的上限（W4‴）**。ERROR_CODES **§2.7.3 新設**並**更正 §2.7.2**：`#stack_overflow`（實作說「我到不了那裡」）與 `#max_depth_exceeded`（操作者說「我選擇在這裡停」）**不是同一件事**——調旋鈕對前者沒有用。四條 MUST：實作上限必須存在且嚴格小於實際能承受的深度／不得以政策之名回報／**不得鑄 `#blur`**／不得出現在旋鈕表裡。SPEC_09 §6 旋鈕表新增「可設 `#_`？」欄，`timeout` 創世預設改 **`#_`**（O41）；§6.0.2 增「**旋鈕是介面，不是把手榴彈**」。**破壞性 #10**：創世 `~%Config` 改值移動每一個根 CAID（`~%Config` 在系統軸，系統軸進 CAID） |
