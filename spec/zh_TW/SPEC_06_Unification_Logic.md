...
| **$v \ \\& \ \\{ \text{name: "x"} \\}$** | `{ %val: v, name: "x" }` | 原子與不衝突的欄位平行存在。 |
| **$v \ \\& \ \\{ \\%val: v' \\}$** | `{ %val: (v \ \& \ v') \}` | 若兩者皆有 `%val`，對其原子值進行格論合併。 |
| **$v \ \\& \ \\{ \\%val: v, \dots \\}$** | `{ %val: v, \dots \}` | 若原子值相同，結果保持該值。 |
| **$v \ \\& \ \\{ \\%val: v', \dots \\}$** | `_\|_` | 若 $v \neq v'$，引發原子層級衝突。 |
| **$v \ \\& \ \\{\\{ \dots \\}\\}$** (Cocoon) | 依 Cocoon 規則 | 若 Cocoon 未宣告 `%val`，則合併失敗 (`_\|_`)。 |
...
3.  **效能獨立性**：規格不保證特定定義在所有引擎上的執行效能一致。引擎可根據其目標場景（如嵌入式或高性能伺服器）自由選擇優化路徑（詳細優化建議請參閱 **[REAL_01](./REAL_01_Ouroboros_Engineering.md)**）。
...
## 2. 非嚴格性與邏輯防火牆 (Non-strictness)
`n/` 允許節點局部處於 `_|_` 狀態而不導致全域崩潰。
*   **惰性求值 (Lazy Evaluation)**：只有當路徑被顯式觀測（如輸出或參與關鍵運算）時，`_|_` 才會引發可見的錯誤。
...
當使用 `...` 展開運算子時：
1.  **Combo 展開**：將欄位直接混入目標，目標維持其原有的開放/閉屬性。
2.  **Cocoon 展開**：將定義混入，展開後的內容在目標容器中不再具備原先的封閉約束，且展開動作不具備傳染性。
