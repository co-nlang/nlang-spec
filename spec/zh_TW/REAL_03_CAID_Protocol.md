...
**實作註記（非規範）**：剝除非內容欄位**必須由被序列化的型別驅動**，不得由序列化後的形狀判斷。判例：某實作以「JSON 物件同時具有 `span` 鍵與 `kind` 鍵」判定其為語法節點，而**使用者資料的欄位名恰好落在同一位置**；`{ kind: 1, span: 2 }` 於是被寫成 `{ kind: 1 }`，提交**回報成功**而物件從此不可讀（位址係按未剝除的值算出）。形狀是編碼的產物，型別才是意圖。
