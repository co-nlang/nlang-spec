...
### 切版 v0.32.0 ／ v0.32.0-draft.1（2026-08-24）

**四項檢查清單全數完成並逐項驗證**：

| # | 項目 | 驗證 |
| :-- | :--- | :--- |
| 1 | `VERSION` → `0.32.0-draft.1` | ✅ |
| 2 | `CHANGELOG` 新增 `[0.32.0-draft.1]` 區段（原 `[Unreleased]` 內容搬入） | ✅ |
| 3 | `SPEC_00` 版本表新列＋移動 `(目前)` | ✅ **計數驗為 1** |
| 4 | tie-back | ✅ 見下 |

**引擎版本**：`crates/oo/Cargo.toml` `0.31.0` → `0.32.0`（`Cargo.lock` 同批）。

**分支與標籤**：故事提交以 `git commit-tree <tree> -p top -F <story>` 造在 `top` 上，
標籤打在 `top`；隨後 `git merge --no-ff top` 回接。

| 倉 | `top` 故事提交 | 標籤 | tie-back |
| :--- | :--- | :--- | :--- |
| `nlang-spec` | `9b5dc59` | `v0.32.0-draft.1` | `b1bda9e` |
| `nlang-tools` | `c4642d7` | `v0.32.0` | `f808d8c` |
| 超專案 | `a605c7c` | `v0.32.0` | — |

**兩倉皆已驗**（用 `git merge-base --is-ancestor`，**不是** `git diff`）：
`top` 是工作分支的祖先 ✅，且**兩者的 tree 逐字相同** ✅。

**末步驗證用的是從標籤建出的二進位**（`git archive v0.32.0` ＋ 獨立 `CARGO_TARGET_DIR`，
置於 `/home/gali/nlang-baselines/v0.32.0-verify`，**未動工作樹**）：

*   `oo --version` → **`oo v0.32.0`**，**無 `-N-g…` 後綴** ⟹ 標籤綁定正確
    （這正是 Inbox「`oo --version` 不識別它自己的建置」那一列所量的東西，
    切版當下它是對的；該缺陷只在 `dev`／`local` 上顯現）。
*   已知答案問題 `~%Math./add (1,2)` → **3** ✅
*   本弧主張 `...~%Cond` ＋ `/if (#true,…)` → **`"yes"`** ✅
*   conformance **149/149** ✅
*   全跑 `--no-fail-fast`：**`targets=210 passed=2034 failed=0 ignored=0 err=0`** ✅

---

## Q-037 驗收 — 一個不進身分的欄位，正在改變 `&` 的答案（2026-08-24）

**工單** `nlang-tools/docs/a_field_outside_identity_handover.md`。
**交付** `nlang-tools ed5c10d`（基線 `5a4b472`）。**一輪通過，零修補。**

### 定位——交付方推翻了工單的定位，而它是對的

工單依讀碼判斷「遺失發生在 `unify_combo` 頂部兩次 `expand_combo_pending` 之一」，
並要求交付方先做 crate 內探針把它縮到一行。**探針量出來的答案在更上游**：
那三處收集 `pending` 的意圖是對的，**它們根本沒被叫到**——兩個未擴張的展開結果
`data=[] pending=1`，digest 逐字元相同，`unify_internal` 的 **CAID 早退在進入
`unify_combo` 之前**就把右邊丟掉了。

⟹ **驗收方的讀碼定位是錯的，工單第一件工作（crate 探針）正是為此而設，它生效了。**

### S2 走了第三條路（工單給的兩條皆不可行，交付方依約回報）

| 路 | 為何不行 |
| :--- | :--- |
| 1. 任何可觀測邊界前 `pending_spreads` 恆空 | 構造時展開會重開 forward_spread（`SPEC_03` §3.1 時序） |
| 2. 讓它進入身分 | `{ /f: 1 }` 與 `{ ...{ /f: 1 } }` 會變成不同的值——**G4 紅線正是禁這一條** |
| **3.（採用）** | **digest 看不見的欄位，不得用來跳過它會改變的工作。** 欄位仍不進 CAID（G4 仍綠），早退在任一邊尚帶未擴張展開時關閉，`force` 之後再比 digest |

### 驗收量測

| 項 | 結果 |
| :--- | :--- |
| diff 純度 | 三檔：`unify.rs`、新增 crate 探針、**探針檔只移除 5 行 `#[ignore]`，其餘逐字未動** ✅ |
| 探針完整性 | **R1–R5 五支紅全綠、G2–G4 三支綠仍綠** ✅ |
| 全跑 ×5（`--no-fail-fast`，逐 target 聚合） | **五輪逐字相同 `targets=212 passed=2045 failed=0 ignored=0 err=0`，五輪 `exit=0`** ✅ |
| 增量核對 | 211 + 1 新 crate target = **212**；2037 + 5 解禁 + 3 新測 = **2045** ✅ |
| conformance | **149/149**（收尾後 **153/153**） ✅ |
| **身分紅線（真二進位）** | 根物件 `932a9f9dd6…87fb0cb`、標準根 `7038e2504b…af37911`——**與工單開單當下所量逐字元相同** ✅ |

**類別檢查（「修類別不修個案」）**——逐一實測其他 digest 消費者，全部與對照組一致：
`=` 得 `#false`（對照亦 `#false`）、`|` 得 `["a"] | ["b"]`（同對照）、
list meet、巢狀 meet、`identify` 兩式相異、耐久層兩倉得**兩個相異的提交**。

### 記帳（不影響通過）

*   **未指名的順手改動一處**：`unify.rs:537` 一個 `format!` 被 rustfmt 收成一行。
    語義為零，但**報告未指名**——與上一弧同類（上一弧是 `universe.rs` 一批 fmt 重排）。
    〔規則〕「凡順手改動之處，逐項指名」**連續兩弧未完全做到**。
*   **`caid_is_incomplete` 只遞迴 `Combo` 與 `Union`**：`Range` 的上下界、
    `Thunk` 的 `context`／`closure`、`Bottom`／`Blur` 的內含值皆未遞迴。
    〔量〕逐一嘗試建構見證失敗（`{...}..{...}` 先塌陷為 ⊥）⟹ **今天是陷阱不是活缺陷**，
    與 M7 `legacy_fields` 同判。**記此以免日後誤讀為已覆蓋。**
*   **`unify_internal` 的第二處早退在 `force` 之後**，而 `force` 會擴張頂層待辦
    ⟹ 該處守衛只在**巢狀**待辦時可能生效。非缺陷，記此以免誤讀為重複。
*   **二進位自報版本落後兩個提交**（`oo v0.32.0-666-gf808d8c`，實為 `ed5c10d`）：
    `build.rs` 未隨提交重跑。屬 Inbox 既有那一列（`dev` 上的版本字串），
    **本弧以行為確認二進位含修正**（R1 得 `["a","b"]`），非以版本字串。

### 規格收尾（驗收方做）

*   **`REAL_03` §6.9 新設**（Core Requirement）——位址決定值。三款：值層必須同答／
    **因果通道是唯一例外**（判例 `TopCaused`）／**位址看不見的欄位不得被用來跳過工作**。
*   **`SPEC_03` §3.1 新款**——未擴張的展開結果仍是普通的值；並與同節 `#blur` 條下的
    **O47 按語**互指（「CAID 保全靠的是一個巧合」＝**同一件事的第一次發作**）。
*   **`SPEC_01` §2.3 新款**——**`&` 的交換律入法**。〔發現〕此前規格只為 `|` 寫了
    無序性（§2.4.1），`&` 只以「交集」二字隱含 ⟹ **沒有條款，就沒有向量**，
    這正是該缺陷橫跨十二個版本未被發現的原因。
*   **conformance `L2-111`–`L2-114`**，`REAL_05` §3.2 索引同批，**153 對 153**。
    四支**在 v0.32.0 真二進位上皆紅、在本版皆綠**（已逐支實測，非推定）。

### 一個為此弧而量、值得單獨記下的事實

**`Top` 與 `TopCaused` 的 CAID 逐字元相同，而 `.%cause` 分得出來**
（`_` 對 `#static_cycle`）⟹ 「同 CAID ⟹ 同答案」若不分層，**在規格自己的裁定下就是假的**。
第二個獨立證人：`(1 & 2) & (#a & #b)` 與其交換式的 `%cause` **同為 `#conflict`**，
而 ⊥ 的人話說明分別指向 `1 vs 2` 與 `#a vs #b`——**先塌陷的那一個先被說**。
⟹ §6.9 的界線畫在**值層／因果層**之間，而不是「進不進位址」。

### 切版 v0.33.0 ／ v0.33.0-draft.1（2026-08-24）

**四項檢查清單全數完成並逐項驗證**：

| # | 項目 | 驗證 |
| :-- | :--- | :--- |
| 1 | `VERSION` → `0.33.0-draft.1` | ✅ |
| 2 | `CHANGELOG` 新增 `[0.33.0-draft.1]` 區段 | ✅ |
| 3 | `SPEC_00` 版本表新列＋移動 `(目前)` | ✅ **計數驗為 1** |
| 4 | tie-back | ✅ 見下 |

**引擎版本**：`crates/oo/Cargo.toml` `0.32.0` → `0.33.0`（`Cargo.lock` 同批）。

| 倉 | `top` 故事提交 | 標籤 | tie-back |
| :--- | :--- | :--- | :--- |
| `nlang-spec` | `100d027` | `v0.33.0-draft.1` | `3b40ba4` |
| `nlang-tools` | `5aa5409` | `v0.33.0` | `ae85c8c` |
| 超專案 | `b467378` | `v0.33.0` | — |

**兩倉皆已驗**（`git merge-base --is-ancestor`，**不是** `git diff`）：`top` 是工作分支的
祖先 ✅，且**兩者的 tree 逐字相同** ✅。

**末步驗證用的是從標籤建出的二進位**（`git archive v0.33.0` ＋ 獨立 `CARGO_TARGET_DIR`，
置於 `/home/gali/nlang-baselines/v0.33.0-verify`，**未動工作樹**）：

*   `oo --version` → **`oo v0.33.0`**，**無 `-N-g…` 後綴** ⟹ 標籤綁定正確。
*   已知答案問題 `~%Math./add (1,2)` → **3** ✅
*   本弧主張：`{ ...{a:1} } & { ...{b:2} }` → **`["a", "b"]`**，交換式**同值** ✅；
    `({ ...{a:1} } & { ...{a:2} }).a` → **`⊥ #conflict`** ✅
*   身分紅線：根物件 **`932a9f9dd6…87fb0cb`**、標準根 **`7038e2504b…af37911`** ✅
*   conformance **153/153** ✅
*   全跑 `--no-fail-fast`：**`targets=212 passed=2045 failed=0 ignored=0 err=0`**、`exit=0` ✅
    （與交付樹的五輪逐字相同）

**切版過程中的一次操作失誤（驗收方，已修，未外流）**：兩個故事提交一度以
`commit-tree` 造進 **`nlang-spec`**——該輪 shell 的 cwd 不是預期的 `nlang-tools`。
處置：刪除兩個誤置標籤、以 `git update-ref` 將 `top`／`local` 移回（**先驗兩個 tree
逐字相同**再移，故工作樹不受影響）、在正確的倉重造，並以 `for-each-ref --contains`
確認四個誤置提交**無任何 ref 指向**。**此後所有 git 指令一律帶 `git -C <倉>`**——
`cd` 與指令分行時，cwd 會被重設，那是這次的真正成因。

---

## Q-038 驗收 —— 一次成功回報的 `commit`，可以讓一個倉兩個引擎都打不開（2026-08-25）

**工單** `nlang-tools/docs/a_commit_that_closes_the_door_handover.md`。
**裁定** O73（2026-08-25，用戶）。**交付** `0a7b6f0` → **一次修補回合** → `3792509`。

### 主張成立（用兩個真二進位驗，探針做不到這一格）

〔量，`oo v0.34.0` ＋ **真的 `oo v0.20.0`**〕新引擎寫進 pre-sentinel 倉：
`commit` 成功、**宣告逐字未動**（`format=2`、無 `objects.format`）、新引擎讀得回
`self-contained (pre-sentinel)`、**造這個倉的 v0.20.0 讀得到新引擎剛寫的提交**。

`migrate` 照裁定：無 grant ⟹ `#privileged_required` **且分毫未動**；有 grant ⟹
推進宣告、`HEAD` 不動、歷史照讀，並**明說舊引擎此後打不開**。

### 工單 §2.5（開單當下量不到的那一題）已答

**pre-sentinel 倉在 commit 之後用得了內建。**〔量〕提交後的根裡 `sum: 5`
（`~%Math./add (2,3)`），且舊引擎讀得到該提交。
⟹ **「原地不動」不會把使用者困在一個退化的倉裡**——那是開單時無法排除的疑慮。

### 第一版交付的回歸（驗收方量到，一次修補即結）

**`encoding = 3` 不是 pre-sentinel。** 第一版把 `encoding < 4` **整段**改走自足路徑，
並逐字寫下 `let _ = standard;`——**丟掉的正是能分辨兩種倉的那個值**。

| 同一份 `app: { k1: 1 }`，同一個 `encoding=3` 倉 | 根位址 | `status` |
| :--- | :--- | :--- |
| 工單基線 `ae85c8c` | `c8fca4d9…` | 指名 `7038e250… (available)` |
| 第一版交付 `0a7b6f0` | **`e1b4d90f…`** | **`self-contained (pre-sentinel)`** |
| 修補 `3792509` | **`c8fca4d9…`**（逐字元復原） | 指名 `7038e250… (available)` |

**更傷的那一格**：第一筆已是 sentinel 形式的倉，第二筆之後**靜默掉回自足**
——歷史中途換了位址規則而無人出聲。修補後兩筆皆保持 `(available)`，基線引擎回讀正常。

**判準本來就在倉裡**：讀取路徑逐字寫著 `if self.encoding < 4 && has_standard`
（**O54**「只 hydrate 帶 sentinel 的根」）。修補改以「標準根表是不是空的」判別
——寫入時根尚未帶 sentinel（那正是 `project_standard_root` 要加的東西），
故寫入側的對應問題是**表在不在**，不是**號碼多少**。

⟹ **教訓**：`encoding < 4` 是**號碼**，`has_standard` 是**性質**。
O73 ② 說的是「pre-sentinel」，而交付把它讀成了一個號碼區間。

### 驗收量測

| 項 | 結果 |
| :--- | :--- |
| diff 純度 | 第一版四檔＋修補一檔；**本弧探針只移除 5 行 `#[ignore]`** ✅ |
| 探針 | **9/9 全綠**（五紅轉綠、三綠仍綠、驗收方加的 `G4` 亦綠） ✅ |
| 全跑 ×5（`--no-fail-fast`，逐 target 聚合） | **`targets=213 passed=2054 failed=0 ignored=0 err=0`，`exit=0`** ✅ |
| conformance | **153/153** ✅ |
| **身分紅線** | 根 `932a9f9d…`、標準根 `7038e250…`、`encoding=3` 根 `c8fca4d9…`——**全部逐字元相同** ✅ |
| O63 的宣告面 | `encoding=3` 倉 commit 後宣告仍為 `layout=2`／`encoding=3` ✅ |

### 記帳

*   **測試盲區是這一弧最重的副產品**：`encoding=3` 的寫入路徑**樹裡原本 0 覆蓋**，
    所以第一版交付的回歸讓整個全跑仍然全綠。驗收方已補 `G4`
    （**在工單基線 `ae85c8c` 上綠、在 `0a7b6f0` 上紅**，兩端皆逐項實跑確認）。
*   **兩回合的順手改動指名皆完整**：第一版逐項指名（`Privilege.migrate`／
    Q-011 `r3` 半條撤銷／已還原的 rustfmt 整檔重排）；修補回合逐字回報
    「順手改動：無」，而〔驗收方核對〕該次 diff **確實只有 `storage.rs` 一檔**。
    〔**2026-08-25 更正**〕本項原記為「修補回合的報告只有一行，無指名段」——
    **那是驗收方所見的訊息被截斷所致，交付方有寫**。記此更正，因為
    「報告有沒有指名」是連續三弧在追的一條紀律，帳不能記錯人。
*   **一項既有、非本弧造成的觀測（已入 Inbox）**：pre-sentinel 倉裡，同一份程式由
    **真的 v0.20.0** 與由今日引擎提交得到**兩個相異的根位址**
    （`6ccbc095…` 對 `0fefb59b…`）。〔量〕**v0.33.0 基線寫的也是 `0fefb59b…`**
    ⟹ **分歧早於本弧**，屬 §6.8 更正段所述「移動的是可重現性」那一類。

### 規格收尾（驗收方做）

*   **`REAL_02` §5.1.1**：刪去「遷移只發生在寫入路徑上」，新增
    **寫入不得改動它所找到的宣告（MUST NOT）**與**宣告的推進必須被明示要求（MUST）**。
    **這是該節第二次，一條寫下時為真的條款變成假的**——前一次是「缺席即視為版本 1
    並補寫」，該節判例已載；本次判例並列其後。
*   **`REAL_03` §6.8**：「以單一摘要指名」明說只治**已進入本形式的根**，
    判別法**不是編碼號碼而是那張標準根表在不在**；新增**不得寫下自己未一併儲存者
    （MUST NOT）**與**一次回報成功的寫入不得使該儲存不可開啟（MUST）**。
*   **無合規向量（逐字記帳）**：本弧性質全在**儲存容器與 CLI**，而 L1/L2 語料的形式是
    「`.n` 程式 ＋ `oo run --observe out`」⟹ **今天無法向量化**。其可執行形式是引擎倉的
    9 支探針＋一個由**真的 `oo v0.20.0`** 所造、進倉 69,501 B 的 fixture。
    索引與語料維持 **153 對 153**。

### 切版 v0.34.0 ／ v0.34.0-draft.1（2026-08-25）

**四項檢查清單全數完成並逐項驗證**：

| # | 項目 | 驗證 |
| :-- | :--- | :--- |
| 1 | `VERSION` → `0.34.0-draft.1` | ✅ |
| 2 | `CHANGELOG` 新增 `[0.34.0-draft.1]` 區段 | ✅ |
| 3 | `SPEC_00` 版本表新列＋移動 `(目前)` | ✅ **計數驗為 1** |
| 4 | tie-back | ✅ 見下 |

**引擎版本**：`crates/oo/Cargo.toml` `0.33.0` → `0.34.0`（`Cargo.lock` 同批）。

| 倉 | `top` 故事提交 | 標籤 | tie-back |
| :--- | :--- | :--- | :--- |
| `nlang-spec` | `ae842e3` | `v0.34.0-draft.1` | `11f6aef` |
| `nlang-tools` | `47624f1` | `v0.34.0` | `71eb69a` |
| 超專案 | `b2ed8fa` | `v0.34.0` | — |

**兩倉皆已驗**（`git merge-base --is-ancestor`，**不是** `git diff`）：`top` 是工作分支的
祖先 ✅，且**兩者的 tree 逐字相同** ✅。

**末步驗證用的是從標籤建出的二進位**（`git archive v0.34.0` ＋ 獨立 `CARGO_TARGET_DIR`，
置於 `/home/gali/nlang-baselines/v0.34.0-verify`，**未動工作樹**）：

*   `oo --version` → **`oo v0.34.0`**，**無 `-N-g…` 後綴** ⟹ 標籤綁定正確。
*   已知答案問題 `~%Math./add (1,2)` → **3** ✅
*   **本弧主張（與真的 `oo v0.20.0` 對驗）**：pre-sentinel 倉 commit 後宣告逐字未動
    （`format=2`、無 `objects.format`）、新引擎讀得回 `self-contained (pre-sentinel)`、
    **v0.20.0 讀得到標籤引擎剛寫的提交** ✅
*   **修補所復原者**：`encoding=3` 儲存的根位址 **`c8fca4d9…`** 逐字元相同 ✅
*   身分紅線：根 **`932a9f9dd6…87fb0cb`**、標準根 **`7038e2504b…af37911`** ✅
*   conformance **153/153** ✅
*   全跑 `--no-fail-fast`：**`targets=213 passed=2054 failed=0 ignored=0 err=0`**、`exit=0` ✅
    （與交付樹的五輪逐字相同）

---

## Q-034 驗收 —— 一個閘沒有讀的障礙（2026-08-25）

**工單** `nlang-tools/docs/an_obstruction_the_gate_did_not_read_handover.md`。
**裁定** O74。**交付** `c82b40f`。**一輪通過，零修補。**
**本弧是 `WORK_QUEUE` §8 新協定（交付回報回填工單 §7）的第一弧。**

### 新協定第一次運作的結果

**兩項純度檢查都過，而且沒有多花一道工**：工單的 diff **只有 §7 內的佔位行被替換**，
分隔線以上一字未動；探針只移除五行 `#[ignore]`。§7 的七個小節**全部填了**，
其中 §7.2 逐項指名了一處順手改動、§7.4 回答了工單指名的問題。
⟹ **上一弧那種「訊息在傳輸中掉了一段」的失誤，這個協定結構上排除了。**

### 類別主張成立，但判別法要對

工單 S1 要求的是**類別**（凡施用呼叫者態射者），交付以 `apply_morphism` 逐次
推一層 frame 達成。驗收時我先量到八個 callback 仍回 `#pure`，一度以為類別沒做到；
**改用正確的判別法之後推翻了自己**：

*   **先問「這個呼叫真的發生了嗎」，再讀它的效果。**〔量〕`~%List./all` 對
    `(x -> #true)` 與 `(x -> #false)` **都回 `#true`** ⟹ 它根本沒呼叫謂詞，
    那個 `#pure` 是對的。仍回 `#pure` 的那些，**正是另立 Inbox 的那組「根本不呼叫謂詞」**。
*   **真的會施用態射的六格全部誠實**〔量〕：`~%List./map`／`/filter`／`/take_while`／
    `/drop_while`／`~%Cond./match`／`~%Query./where` 的 `.%effect` 皆為 `#io`，
    且不純者一律被閘 ⊥。**`/where` 的純謂詞現在寫得出來。**

⟹ ~~**依賴要記下**：那組「不呼叫謂詞」的 callback 修好之後，**S1 必須在那些格子上
重驗**——它們今天是**空洞地誠實**（沒有觀測發生，所以沒有障礙可漏）。~~

> **⚠ 2026-08-26 更正（歸因錯，依賴不存在）。** 上面兩處「它根本沒呼叫謂詞」的**現象屬實，
> 成因寫錯了**：〔量 2026-08-26，v0.35.0 標籤二進位，known-answer `3` 已過〕
> `list.count`／`list.all` 等九個 callback 的引數形是 **`{0: 謂詞, 1: 清單}`**
> （`builtins/list.rs:646` 逐字 `(pred_f, list_v)`），而我兩次都用 **list-first** 呼叫，
> **等於把清單當成謂詞傳進去**。正確形下 `/all((x -> #false), [1,2,3])` → `#false`、
> `/count` 對兩個謂詞可分辨（`3` 對 `0`），**九個 callback 的 `.%effect` 全部是 `#io`**。
> ⟹ **S1／O74 ① 在那些格子上是實在的誠實，不是空洞的；上面那條依賴不存在。**
> 「先問這個呼叫真的發生了嗎」這個教訓**成立且更強**——它這次抓的是我自己的量測。
> 真正的缺陷是另一件事：**`~%List` 三種引數約定並存，用錯的那一種不會 ⊥**（Inbox 已重寫）。
> **這是連續兩弧第二次同型失誤**（見下一小節的 `~%Cond./match`），
> 判別法已升為 `WORK_QUEUE` §8「開弧」第 4 條。

### 驗收方在工單裡寫錯的一列（更正）

工單 §2.1 的 `~%Cond./match` 那一列**是錯的**。開單時我用的引數形是
`~%Cond./match (1, [(@any -> …)])`，而該內建要的是 `(值, [[樣式, 動作], …])`
——**那個呼叫從來沒有發生**（回傳未施用的內建繭），故那個 `#pure` 什麼都沒見證。
〔量，正確形〕該格在**基線**上 `.%effect` 就已經是 `#io`，壞的只有閘。
**裁定與射程不受影響**：`/filter` 與 `/take_while` 兩列是用確定會施用的謂詞量的，
O74 ① 的「不論回傳值有沒有被留下」由那兩列支撐。更正已寫進工單 §2.1 下方。

### 驗收量測

| 項 | 結果 |
| :--- | :--- |
| diff 純度 | 工單只動 §7 內、探針只移除 5 行 `#[ignore]` ✅ |
| 探針 | **10/10 全綠**（五紅轉綠、五綠仍綠） ✅ |
| 全跑 ×5（`--no-fail-fast`，逐 target 聚合） | **`targets=214 passed=2064 failed=0 ignored=0 err=0`**、`exit=0` ✅ |
| conformance | **153/153** → 收尾後 **157/157** ✅ |
| **身分紅線** | 根 `932a9f9d…`、標準根 `7038e250…`、`G4` 仍 `a04c6b85…` ✅ |
| 明確不做 | 繭仍 discharge ✅；`/if` 未被碰 ✅；那組給錯答案的 List callback 未被併入 ✅ |

### 規格收尾（驗收方做）

*   **`SPEC_08` §4.3「靜態守護」→「宣告守護」**。判準改為**宣告的截面被非零障礙反證**，
    **靜態可見性降為實作註記**；新增 **施用即觀測（MUST）**。
*   **`TAG_REGISTRY` `#effect_violation` 就地更正**——該欄要求「值的**實際活動**傳染效應」，
    而同句所引的 §4.3 寫「**靜態**」。**兩者互相矛盾了十一個 minor 版本**，判準以本欄為準。
*   **conformance `L2-115`–`L2-118`**，**157 對 157**。115／116／117 在 v0.34.0 上皆紅；
    **118（繭）兩版皆綠，索引裡照實標為綠圍籬向量**——收錄的理由是防止日後把逃生門當漏洞關掉。

### 值得單獨記下的一句

**「全綠不等於被檢驗過」在這一弧換了一種形狀出現。** 上一弧是「沒有測試看著那一格」；
這一弧是**「那一格的呼叫根本沒發生，所以它空洞地通過」**。
兩者的解藥是同一個：**先證明你的量測真的碰到了你以為它碰到的東西。**

> **2026-08-26 追記**：這句話在寫下的隔天就**對本弧自己的量測生效了**——上面那則更正裡，
> 沒碰到目標的不是測試，是**驗收方的手動讀數**。⟹ 這一弧真正的第三句話是：
> **`#pure` 的第一個假說永遠是「這個呼叫沒有發生」，不是「這個呼叫是純的」。**

### 切版 v0.35.0 ／ v0.35.0-draft.1（2026-08-25）

**四項檢查清單全數完成並逐項驗證**：

| # | 項目 | 驗證 |
| :-- | :--- | :--- |
| 1 | `VERSION` → `0.35.0-draft.1` | ✅ |
| 2 | `CHANGELOG` 新增 `[0.35.0-draft.1]` 區段 | ✅ |
| 3 | `SPEC_00` 版本表新列＋移動 `(目前)` | ✅ **計數驗為 1** |
| 4 | tie-back | ✅ 見下 |

**引擎版本**：`crates/oo/Cargo.toml` `0.34.0` → `0.35.0`（`Cargo.lock` 同批）。

| 倉 | `top` 故事提交 | 標籤 | tie-back |
| :--- | :--- | :--- | :--- |
| `nlang-spec` | `e356d66` | `v0.35.0-draft.1` | `d7026a2` |
| `nlang-tools` | `df6152f` | `v0.35.0` | `afa3e77` |
| 超專案 | `d06e549` | `v0.35.0` | — |

**兩倉皆已驗**（`git merge-base --is-ancestor`，**不是** `git diff`）：`top` 是工作分支的
祖先 ✅，且**兩者的 tree 逐字相同** ✅。

**末步驗證用的是從標籤建出的二進位**（`git archive v0.35.0` ＋ 獨立 `CARGO_TARGET_DIR`，
置於 `/home/gali/nlang-baselines/v0.35.0-verify`，**未動工作樹**）：

*   `oo --version` → **`oo v0.35.0`**，**無 `-N-g…` 後綴** ⟹ 標籤綁定正確。
*   已知答案問題 `~%Math./add (1,2)` → **3** ✅
*   本弧主張：`{ %effect: #pure, v: ((x -> ~%Time.now _) 1) }` → **⊥ `#effect_violation`** ✅；
    `(~%List./filter (…, 不純謂詞)).%effect` → **`#io`** ✅；
    `~%Query./where` **純謂詞寫得出來** ✅；**繭仍 discharge** ✅
*   身分紅線：根 **`932a9f9dd6…87fb0cb`**、標準根 **`7038e2504b…af37911`** ✅
*   conformance **157/157** ✅
*   全跑 `--no-fail-fast`：**`targets=214 passed=2064 failed=0 ignored=0 err=0`**、`exit=0` ✅
    （與交付樹的五輪逐字相同）

---

## Q-012 — a store written in another language（v0.36.0；一次修補回合）

**工單** `nlang-tools/docs/a_store_written_in_another_language_handover.md`；
**偵察** `…_recon.md`；**代修請求** `…_repair_1.md`；
**探針** `crates/oo/tests/a_store_written_in_another_language_probe_test.rs`
（基線 **6 綠 7 紅**，於 `dev 2496207` 校準）；
**fixture** `crates/oo/tests/fixtures/encoding4_repo/`（真 `oo v0.35.0` 造的 `encoding=4` 倉）。
交付 `68ea03d`，代修 `0d387c0`。**裁定 O31（三題，用戶）＋ O75（收弧時劃界）。**

### 主張

`.oo/` 用的是「碰巧實作這些值的那些 Rust 型別的 serde 形」，不是 n/。
〔量，v0.35.0〕15 B 源碼 → **137,185 B** 的倉，其中標準根佔 99.33%，
而標準根自己有**一半只是 hex**（`"standard-root:<hex>"`，JSON 進 hex 進 JSON，恰 2.00×）。

### 偵察量到而改變了這弧形狀的四件事

1. **不是紀元弧**〔量〕根物件檔案 428 B → 762 B（同值不同位元組），CAID 不變、倉照常服務 ⟹ 編碼 ⟂ 身分。
2. **讀相容的破壞已經是響亮的**〔讀〕`ensure_supported_encoding` 對範圍外拒絕開啟並說出自己懂哪個範圍。
3. **貴的不是剖析**〔量，20 次平均〕解析 21.7 KB 的 n/ ≈ 7 ms，與今日讀 136 KB 標準根的 15 ms 同一個數量級。
   **我第一次量到 180 ms 差點寫成「n/ 慢 17 倍」——那是求值不是剖析**，而 O35 已裁「讀取是解碼不是求值」。
4. **語言缺的是兩個字面不是一個**：`system` 軸（25 個模組，⊥ `#system_reserved`）與 `%effect`。
   後者這一格**我讀錯了兩次才對**：先以為「語言有拼法」，再以為「那只是普通欄位」，
   直到拿 Q-034 的守護去問（在不純值上宣告 `#pure` → ⊥ `#effect_violation`）才看出
   **它是真宣告、但是包裝不是標籤**——它加一個欄位，而任何住在值裡的欄位都進身分。

### 交付結果

| 項 | 結果 |
| :--- | :--- |
| diff 純度 | 分隔線第 166 行，diff 起於 167 ⟹ §7 以上一字未動 ✅ |
| 本弧探針 | **只移除 7 行 `#[ignore]`**，其餘逐字未動；fixture 未觸碰 ✅ |
| 全跑 ×5 | **`targets=215 passed=2078 failed=0 ignored=0 err=0`**，五輪逐字全同 ✅ |
| 增量對帳 | **215 ＝ 214 ＋ 1**（探針檔）；**2078 ＝ 2064 ＋ 13 ＋ 1**（探針 13 條＋交付方在 `store_codec.rs` 的單元測試）⟹ 零回歸 ✅ |
| conformance | **158/158**（新增 L2-119） ✅ |
| **身分紅線** | 根 `932a9f9d…`、標準根 `7038e250…` **逐字元不動** ✅ |
| 體積 | 根 428 → **123 B**；標準根 136,268 → **16,029 B（8.5×）**；CAS 整倉 137,175 → **16,483 B** ✅ |

**跨版本，兩個真二進位，三格全過**：舊引擎開新倉 → **響亮拒絕**並報出自己懂的範圍；
新引擎開 `encoding=4` fixture → 讀得到、位址不動；
**新引擎寫進 `encoding=4` 倉 → 寫出的仍是 encoding-4 物件、宣告一字未動，
而造那個倉的真 `oo v0.35.0` 讀得到新引擎剛寫的提交** ⟹ **O73 的紀律被守住了**。

### 一次修補：一支探針被關掉，而它守的條款今天真的被違反

第一版交付在 `every_byte_or_none::r4_no_unhashed_formatting_on_disk` 的迴圈開頭加了三行
**「若物件帶有新編碼的框，就跳過」**。新編碼的每一個物件都帶框 ⟹
**該探針對這次交付所造出的一切完全不執行**，而全跑 2,078 條**全綠**。

而它守的是 **`REAL_03` §6.7 第四條 MUST NOT——「不得有排版位元組」，2026-08-13 就寫在那裡**，
其判例 3 量的正是「約 184 KB 縮排不受任何雜湊覆蓋」。
〔量〕第一版交付的標準根 **21,705 B 之中 7,713 B（35.5%）是空白**，1,163 個換行；
直接驗證：於磁碟給根物件加入純空白（133 → 142 B），`status` 照常、`inspect` 同一個 CAID、內容一字不差。

**代修的修法不必動探針一個字**〔量〕：緊湊的 n/ 耐久形讀得回、同一個 CAID、
**只有框那一個換行**，而 R4 判的是 `newlines > 1` ⟹ **R4 原封不動即通過**。
代修後標準根再省 35.5%（21,705 → 16,029 B）。

> **這一弧真正的一句話**：**一個規範條款的效力，等於仍然指著它的那個量測的效力。**
> 換編碼時，每一支以舊編碼的**拼法**判斷的檢查都會**靜默地變成空洞的真**——
> 它不會變紅，它會變得**沒有意見**。同一次收弧另外抓到兩支同病的
> （兩處 `!contains("\"span\"")`），由驗收方補上 n/ 側拼法；
> **刻意不加裸 `span:`**，因為使用者的欄位可以合法叫 `span`，那正是 `r6` 存在的理由。

### 規格收尾（驗收方做）

*   **`REAL_03` §6.8.3 新設**（Core Requirement）：耐久形必須能表達表層語言寫不下來的東西。
    五款——**必須自報身分**（理由是 O35「線上就是 store」，物件會離開它的倉而容器宣告留在原地）／
    **自報不得進入身分**／**自報必須決定解碼**，從而落入 §6.6 的重算／
    **表層禁止之物的字面僅於耐久形內合法，且表層行為不得改變**／**標籤不得成為欄位**。
*   **`REAL_03` §6.8.1 更正 ＋ `STATUS` O75**：該款窮舉論證的第二支假定
    「不進雜湊 ⟹ 未被覆蓋」，**那一步不成立**。判準是**能不能在不改變解碼結果的前提下改動它**——
    `span` 可以 ⟹ §6.7 所禁；決定解碼的自報不行 ⟹ §6.6 必然抓到。
    〔量〕四種框竄改：改字樣／刪除／改為不存在者 → `#object_undecodable`（與完整性失效分開報）；
    把 commit 物件的框換成值物件的框 → **`#caid_mismatch` 並報出重算的 digest**。
    **而最窄的讀法下本款根本沒被碰到**：框說的是「我是哪一種文件」，不是「我是第幾版編碼」。
    **這一格是 O31 ② 自己逼出來的——若當時沒去讀 §6.8.1，這個弧會在不知情下違反一條規範條款。**
*   **`REAL_02` §5.1.1**：同款界線澄清；新增判例——**編碼軸第一次由一個被明示要求的動作前進**。
*   **conformance 新增 L2-119「耐久形自己的鍵不是後門」**：L2-61 釘任意新名（`~%Mine`），
    本條釘**引擎自己在耐久形裡用的那些鍵**。〔量〕五個 `~%__nlang_*` 在一般源碼全部 ⊥ `#system_reserved`，
    框在源碼裡是 parse error ⟹ **無洩漏**。L2 118 → **119**，39 ＋ 119 ＝ **158**。

### 切版 v0.36.0 ／ v0.36.0-draft.1（2026-08-26）

**四項檢查清單全數完成並逐項驗證**：`VERSION` → `0.36.0-draft.1`；
`crates/oo/Cargo.toml` → `0.36.0`；`CHANGELOG` 新節（**破壞性條目 #19，格式軸，
90 天時鐘不重啟**）；`SPEC_00` §4.4 新增一列且 `(目前)` **計數為 1**；
兩倉 tie-back 皆以 `merge-base --is-ancestor` 驗過，且 `top` 與工作分支**同樹**。

**末步驗證用的是從標籤建出的二進位**（`/home/gali/nlang-baselines/v0.36.0-verify-target/release/oo`）：

| 項 | 結果 |
| :--- | :--- |
| `oo --version` | **`oo v0.36.0`**（無 `-N-g` 後綴） |
| known-answer | `{ a: (1 + 2) }.a` → `3` |
| 全跑（`--no-fail-fast`，逐 target 聚合） | **`targets=215 passed=2078 failed=0 ignored=0`**、`exit=0` |
| conformance | **158/158** |
| 新倉宣告 | `encoding=5` |
| **身分紅線** | 根 **`932a9f9d…`**、標準根 **`7038e250…`** |
| 體積 | 根 **123 B**、標準根 **16,029 B**、commit 331 B；**三者各恰 1 個換行**（框那一行） |
| 排版位元組 | R4 判準 `newlines > 1`，**探針一個字未改即通過** |
| 框外不變 | `{ ~%Math: 9 }` → ⊥ `#system_reserved` |
| GC | `3 objects, 3 reachable, 0 collectable` |

**基線二進位保存於 `/home/gali/nlang-baselines/v0.36.0-verify{,-target}`。**

---

## Q-039 —— 一份住在 Rust 裡的合約（2026-08-26，一輪零修補）

**交付** `nlang-tools 72cd9fa`（引擎）＋ `nlang-spec 378992a`（交付方所做的規格收尾）。
**裁定** O76／O77／O78，本弧無新裁定。**偵察** `docs/a_contract_that_lives_in_rust_recon.md`
（含 §12 驗收時對它自己的兩處更正）。**工單＋探針** `dev 2d22983`，基線 **7 綠 8 紅**。

**驗收結果**：全跑 **×5 逐字全同 `targets=217 passed=2097 failed=0 ignored=0 err=0`**
（驗收方 debug、交付方 release，兩邊逐字相同）；增量 **2085 ＋ 4（新測試）＋ 8（解除的紅）＝ 2097，差 0**；
conformance **161/161**（驗收方以交付二進位獨立跑過，收尾後為 **162/162**）；
**身分紅線**：根 `932a9f9d…`（123 B）／標準根 `7038e250…`（16,029 B）**與 v0.36.0 逐字相同**
⟹ O78 ②「宣告住 registry」的裁定在實測上成立。探針 `git diff` **只有八行 `#[ignore]` 移除**。

**做法**：`builtins/contract.rs` 一張表（241 個 id → 必填鍵），`lib.rs:3138` 呼叫 `func` 之前比對，
**失配重用既有的 `Value::Top → 部分施用的繭`**——不新造機制，正是工單指定的形。
`pred_is_true` 把 ⊥ 走 `Err` **傳出去**，比「可分辨」要求的更強，與普通施用路徑一致。
`builtin_contract_test.rs` 釘住 registry ↔ 宣告表**雙射**，使 `unwrap_or(true)` 那條寬鬆退路永不生效。

**本弧留下的三句話：**

1.  **「習慣還是必須」這一題有量測答案。** 〔量〕`|>` 把被管的值放進**下一個空位**
    （`lib.rs:3049`）⟹ **管道只與「資料放最後」組合得起來**。逐格：pred-first 群在管道下全對；
    `/take_while`／`/drop_while` 回 `[]`（無聲錯）；**`~%Query./where` 回 ⊥**——而 `~%Query`
    正是最該被管道用的模組。⟹ 正準判準因此不是美感而是可證偽的（O78 ③）。

2.  **⚠ 驗收方的偵察把施工面數少了，而且是同一弧內第二次同型失誤。**
    偵察寫「⊥ 被折疊的面只有六行，全在 `list.rs`」。實際有**三個**真值 helper：那六處印出形比對、
    `list.rs` 的 `is_truthy_pred`（`take_while`／`drop_while`）、`query.rs` 的 `is_truthy`（`where`）。
    後兩個都是寬鬆形且同樣把 ⊥ 折成假；驗收方的 grep 只找 `to_string_plain()` 併同 true／false 的行，
    兩個 helper 不符合那個 pattern。**交付方三個都找到並統一了，比工單徹底。**
    第一次同型失誤是 `~%Refl` 拼法（收帳當日）⟹ **量到缺陷的一種拼法，就以為量到了整個面。**

3.  **而那個統一帶來一個 §7.2 未指名的行為變更，由驗收方補帳。**
    〔量，兩個真二進位各用自己版本的正確引數序〕真值判準由「不是 `#false` 就算真」收窄為
    「只有 `#true` 算真」：謂詞回 `5` 或 `#none` 時，`~%Query./where`／`/take_while`／`/drop_while`
    由**保留**改為**丟棄**（v0.36.0 `[{ a: 1 }]`／`[1, 2, 3]` ⟹ 交付 `[]`／`[]`）。
    **統一是對的，但那是一個規格從未描述的選擇**——`SPEC_09` 此前只在 `/lt` 一格寫「→ `#true`／`#false`」。
    ⟹ §2.5。驗收方於收尾補 `SPEC_09` §5.1.1 一款 MUST 與 **L2-123**，
    **把一個無聲的收窄變成一句寫下來的規則**——這正是本弧自己的主題，不能讓它以無聲的形式出貨。

**兩件記在帳上的**：① **交付方又做了規格收尾**（`378992a`），那是驗收方的事；內容正確且完整
（含修掉驗收方一個壞的檔名交叉引用 `SPEC_08_Observation_and_Effects.md` → `SPEC_08_Meta_and_Runtime.md`），
故不退回，但這是 Q-035 之後第二次。② **偵察的「七個具名內建」是六個**——`diff.patch` 的
`path`／`to` 讀自 diff **條目**不是引數；由交付方在 §7.3 指出，O78 與偵察 §12 已就地更正。

### 切版 v0.37.0 ／ v0.37.0-draft.1（2026-08-26）

**破壞性條目 #20，軸別為語義 ⟹ 90 天穩定時鐘重啟**（前次重啟為 #18，2026-08-21；
#19 為格式軸未重啟）。**身分不動**〔量，真二進位〕根 `932a9f9d…`／標準根 `7038e250…`
逐字元相同——宣告住登記表而不進值（O78 ②），故本版**不是紀元**。

四項檢查：`VERSION` → `0.37.0-draft.1`；`CHANGELOG` 新增 `[0.37.0-draft.1]`（破壞性／增量／
編輯性三類齊全）；`SPEC_00` §4.4 新增一列並移動 `(目前)`（**驗過全檔只有一個**）；
兩倉 tie-back。引擎 `crates/oo/Cargo.toml` → `0.37.0`。


---

## Q-013 — 深度屬於 savepoint，不屬於 commit（弧 A 第一批）

**驗收通過 2026-08-28，一次修補回合。** 裁定 **D42–D47**（全文 `meta/oo/commit.md` §1.10）。
偵察 `nlang-tools/docs/arc_a_recon.md`（`dev a1a4339`）；工單＋探針
`docs/depth_belongs_to_the_savepoint_handover.md` ／
`crates/oo/tests/depth_belongs_to_the_savepoint_probe_test.rs`（`dev e5533d6`／`bd61f67`）；
交付 `8346d4b`；驗收 Round 1 `c2212d4`／`2d5cd1c`；Repair 1 `10b690e`。

**射程四項，同批**：S1 ○ 有地方住／S2 commit 不強制求值可重算的一格／
S3 頂層那第二個邊界／S4 forward-miss 的 `%effect` 不再說謊。
**A2（`Commit.parent`）與 A3（日誌正準形）明確不做**，理由各自具名。

**驗收數字**：全跑 **×5 逐字相同 `targets=218 passed=2106 failed=0 ignored=0`**、
conformance **162/162**、身分紅線（真二進位，known-answer `~%Math./add (1,2)` → `3` 已過）
標準根 `7038e250…` 未動、`x: 0` 全固化宇宙根 `31745ef0…` 與物件數 3 **與基線逐字相同**
（○ 沒有進 CAS）。編譯警告經比對為既有，非本弧引入。

**S1 無探針，由驗收方以量測補帳**：commit 之後 `staged` 不存在而 `savepoints/` 仍在
（D43 成立）；`LOG` 2 行、框 `#nlang/store savepoint`；**重複 evolve 同一個檔不鑄第二顆
（D47 注入條款成立）**。

### 這一弧留下的三句話

1.  **一支校準過、會紅、紅得漂亮的探針，仍然可以釘錯東西。**
    工單 r2 斷言 `contains("c: 3")`——**要求把答案存起來，與它自己要執行的 D46 ① 相反**。
    交付照做，實作因此長出 `_ if matches!(parent, Some(Value::Thunk { .. })) => forced`，
    而〔量〕**同一支程式只因提交次數不同就得到不同的根**（兩次 `067627389c…`
    ／一次 `24c1c9041b…`，前者正是弧前那顆）——**§1.5.7 那個病原地復活**。
    ⟹ **「基線紅、交付後綠」證明的是探針被滿足了，不是裁定被兌現了。**
    Repair 1 拿掉該分支後兩者同根 `24c1c9041b…`。
2.  **工單漏寫一件事，而樹裡有三支針專門抓那件事。** S1 要在 `.oo/` 加檔，工單沒說；
    兩支佈局針因此變紅，其註解逐字要求「加檔必須寫進工單」。
    **交付撞到牆、沒繞過去、寫進 §N.3——這是對的做法。**
    驗收方補宣告時修類別不修個案：第三支今天不會紅（該情境從不 `evolve`），仍一併宣告；
    `p1` 另需把 ○ 的本地 id 依 CAS 同法摺疊為 `<local-id>`，否則那支針釘的是
    「這一輪鑄了幾顆」而不是佈局。
3.  **驗收方自己造了一條假發現，被控制組推翻。** 一度判定「配方留下了但 `c` 觀測不到」，
    差一步記成本弧引入的回歸；控制組顯示 `--observe` 對一個**已提交的純原子**同樣回 `_`，
    v0.37.0 亦然 ⟹ **那個量測根本沒讀到已提交的宇宙**。已另立 Inbox 一列。
    **同一輪先用一條無效證據（`oo eval` 跑空白宇宙）支持了一個正確結論**，
    結論靠 `inspect` 才站得住——**證據無效不等於結論錯，但它必須被撤回。**

### 規格收尾（驗收方）

*   **`SPEC_10` §1.1 #1 加射程更正**：「Commit 捕獲坍縮觀測結果」**只適用於不可重現的格**；
    原文寫於觀測與注入尚未分開之時。
*   **`SPEC_10` §2.3.1 新設**（D46）：判準是 `%effect` **而且只有 `%effect`**——
    `#pure` MUST NOT 以答案取代定義／非純 MUST 捕獲並轉 `#cached`／`_|_` MUST 寫入其 `%cause`／
    **判準 MUST NOT 取決於父 Commit 持有什麼**。附上那則已量到的資料遺失。
*   **`SPEC_10` §3.1 新設**（D43／D47）：Savepoint 的耐久、身分不得為 CAID、順序、
    產生判準（注入動位置／觀測真的化約），以及 `_|_` 亦產生 ○。
    **產生判準逐字禁止以「消耗了燃料」表述**，並記下那個量測。
*   **`GLOSSARY`**：`Staged` 定義改「尚未**提交**」並註明它不是 Savepoint；新增 `Savepoint` 條。
*   **無合規向量，逐字記帳**：S1–S4 的性質全在 CLI 與 `.oo` 容器，L2 語料表達不出
    savepoint 檔、commit 投影與 evolve 後的 thunk 標籤。**162 對 162 不變。**
    交付方 Q5 獨立得到同一結論，未硬湊。
*   **依 §2.4 於收弧當日開列 Inbox 一列**：`SPEC_10` §3.1 的**觀測條款與 ⊥ 條款引擎未兌現**
    ——交付方 Q2 逐字自陳「觀測邊界今天仍不寫 ○……沒有假裝已落地」。
    **那一句是本次回報最有價值的一行。**
*   **依 §8 收弧 3b 標記**：本弧改變了根的磁碟表示（`#pure` thunk 現在留在根裡），
    Inbox 中以位元組為證據的列須重量；**全固化的宇宙不受影響**（`31745ef0…`／3 個物件不變）。

### 切版 v0.38.0 ／ v0.38.0-draft.1（2026-08-28）

**破壞性條目 #21，軸別為語義 ⟹ 90 天穩定時鐘重啟**（前次重啟為 #20，2026-08-26）。
**身分不動**〔量，標籤建置〕標準根 `7038e250…` 未移動；**全固化的宇宙逐字不變**
（`x: 0` 的根 `31745ef0…`、`.oo/objects` 3 個）⟹ **不是紀元**。
**但凡宇宙含未強制的純 thunk，其根 CAID 改變**——那是本條的爆炸半徑，且是預期的，
已明文寫入 `CHANGELOG`。

四項檢查：`VERSION` → `0.38.0-draft.1`；`spec/CHANGELOG.md` 新增 `[0.38.0-draft.1]`
（**四類**：破壞性／增量／編輯性／**未兌現**）；`SPEC_00` §4.4 新增一列並移動 `(目前)`
（**驗過全檔只有一個**）；兩倉 tie-back 以 `merge-base --is-ancestor` 驗過且同樹。
引擎 `crates/oo/Cargo.toml` → `0.38.0`。

**標籤建置驗證**（`/home/gali/nlang-baselines/v0.38.0-verify`，`oo v0.38.0`，
known-answer `~%Math./add (1,2)` → `3`）：全跑 **`targets=218 passed=2106 failed=0 ignored=0`**、
conformance **162 vectors, 162 pass, 0 fail**、一次／兩次提交同根 `24c1c9041b…`。

**`CHANGELOG` 本版多了一類「未兌現」**：`SPEC_10` §3.1 的觀測條款與 `_|_` 條款今日未實作。
把它寫進切版條目而不只留在 Inbox——一條規範條款的欠帳，應該在宣布版本的地方就說。

**§8 收弧 3a 掃 Inbox 的結果**：「`commit` 摧毀一個它算不出來的定義」**已結**
——破壞性條目 #21 逐字就是它。〔複驗，標籤建置〕提交後根裡是 `c: a + b` 而非 `c: _`。
**這條規則今天抓到了它要抓的東西**（前例：v0.27.0 條目寫了「移除頂層 `/add`」，
而 Inbox 那列又活了四個月）。

**過程紀律一則**：本次 tie-back 全程使用 `git -C`。上一版（v0.37.0）在同一步用了 `cd`，
第二個 `git merge` 因此在 spec 倉跑了一次 no-op 並**回報成功**，tools 的 tie-back 沒有發生
——由 `merge-base --is-ancestor` 抓到，不是 `git diff`。**那個假的成功回報，
只有一個獨立於它的量測能拆穿。**

**⚠ 2026-08-29 事後量測：v0.38.0 把一個已知缺陷的表面積擴大了一倍，而本弧沒有察覺。**

〔量 2026-08-29，v0.38.0 標籤二進位；known-answer `~%Math./add (1,2)` → `3` 已過〕
40 個並行 `oo evolve`（各加一個不同欄位），四輪：

| | staged 存活欄位 | `savepoints/LOG` 記載的 ○ | 目錄檔數 |
| :-- | --: | --: | --: |
| trial 0 | 4／40 | 5 | **6（孤兒 1）** |
| trial 1 | 4／40 | 4 | 4 |
| trial 2 | **5**／40 | **6** | 6 |
| trial 3 | **4**／40 | **6** | 6 |

*   **舊的一半原封不動**：`control_plane` §1.4.3 於 2026-08-05 量到「41 → 2 個欄位，
    零錯誤」，**十六個 minor 版之後仍是同一個病**。這一半不是本弧造成的。
*   **新的一半是本弧加的**：40 次位置移動只鑄出 4–6 個 ○ ⟹ **D47 的注入條款
    在並行下不成立**；且 trial 2／3 的 **○ 比存活欄位還多**（6 > 5、6 > 4）
    ⟹ ○ 鏈記載了 `staged` 沒有的血緣。
*   **儲存自我不一致的窗確實存在但不必然**：trial 0 的 `LOG` 記 5 筆而目錄有 6 個檔
    （`0000000000000006` 是孤兒），**後三輪未再現**。照實記——**這是競爭窗，
    不得寫成穩定重現**。
*   〔讀〕成因同源且在同一個函式：`savepoint.rs::record` 先 `recorded_ids()`
    再 `mint_id(&ids)`（id ＝ 計數＋1）再整檔重寫 `LOG`——**讀-改-寫，
    而身分由讀到的計數決定**。`atomic_write`（`storage.rs:15`，temp＋rename）
    保證的是**單檔不半截**，不保證跨檔或跨行程。

**這不是新缺陷，是舊缺陷長出了第二份。** 依「修類別不修個案」，
修法是 **Q-014（原子單位）＋ Q-016（compare-and-swap ＋重試）**，
**不是在 `savepoint.rs` 打補丁**；亦**不構成 `interrupt-candidate`**——
類別未變，而 Q-014 今天就已經是 Ready 首位。

**協定面的教訓已寫成 §8 收弧 3c**：凡在 `.oo/` 新增一種耐久檔，
既有的並行／損壞量測必須對新檔重跑一次。3b 只治「換掉一種表示法」，
**新增一種檔不在它的射程裡**。〔更重的一點〕`STATUS` §335 W8′ 早就逐字預言了這筆債
（「W10/W11 每新增一種耐久檔，這筆債就貴一次」）——**債被預言了，
但收弧時沒有人去收**；工單沒有要求並行探針，驗收方也沒有補。

---

## 切版 v0.39.0 / v0.39.0-draft.1（2026-08-30）— 一個本來就是集合的工作集

**Q-014（弧 A 的旁支，非 W10 原題）。裁定 D48／D49。一輪零修補。**

`SPEC_10` §3 第 2 款一直寫著「Staged：……未提交的定義**集合**」，而參考實作把那個集合
存成**單一可變格子**。那個格子就是全部的缺陷。〔量 2026-08-30，v0.38.0 標籤二進位〕
40 個並行 `oo evolve`、各加一個**不同**欄位 → **存活 3／40**（五輪 3–6），
**零錯誤，每個行程都回報成功**。

**這不是一個更好的設計，是第一個與規格既有那句話相符的實作。**

四項檢查：`VERSION` → `0.39.0-draft.1`；`spec/CHANGELOG.md` 新增 `[0.39.0-draft.1]`；
`SPEC_00` §4.4 新增一列並移動 `(目前)`（**驗過全檔只有一個**）；
兩倉 tie-back 以 `merge-base --is-ancestor` 驗過且同樹。引擎 `Cargo.toml` → `0.39.0`。
**全程 `git -C`。**

**標籤建置驗證**（`/home/gali/nlang-baselines/v0.39.0-verify`，`oo v0.39.0`，
known-answer `~%Math./add (1,2)` → `3`）：

*   全跑 **`targets=219 passed=2112 failed=0 ignored=0`**、exit 0、`^error` 0
*   conformance **162 vectors, 162 pass, 0 fail**
*   **主張：40 並行三輪皆 40／40**，注入檔 40
*   身分：`x: 0` 根 `31745ef0…`、物件數 **3**、標準根 `7038e250…`

**破壞性條目 #22，語義軸 ⟹ 90 天時鐘重啟**（前次 #21，2026-08-28）。
**身分不動 ⟹ 不是紀元**——注入不進 `objects/`。
**爆炸半徑**：`.oo/staged` 消失，改 `.oo/injections/<本地 id>`。

### §8 收弧 3a／3b 的掃描結果

*   **3b 無事可標**：〔量〕Inbox 中**沒有任何一列**以 `.oo/staged` 的位元組為證據。
*   **3a：那列「提交撞 ⊥ 時逐字回報 `Commit successful` 且 exit=0」未被本條結掉。**
    〔複驗，v0.39.0 標籤建置〕`c: c + 1` → `evolve` rc=0、`commit` **rc=0**、
    輸出 `Commit successful: hash:…`。**仍在。**
    ⟹ 現在有一個乾淨的對照：**D49 的工作集 ⊥ 被拒（rc≠0），而發散值的 ⊥ 照樣靜默提交成功。
    同一個符號、兩條路、修了一條。** 這是 D33「必須停下並回報」欠帳的第二個拼法。

### 兩次錯路，都是驗收方的，都值得記

1.  **「目錄就是 log」是錯的。** 提議把 ○ 鏈改成 append-only，理由是「孤兒類別會消失」。
    **建檔是原子的，`readdir` 不給插入序**——目錄是集合不是序列。meet 子集不需要序，
    但 **`pin` 不可交換**（〔量〕同兩筆注入反序得到不同的值），而目錄供不出 pin 要的序。
    **而那個提議等於採用偵察前兩段才標為隱患的那個面**（`.oo/peers/directory`，
    append 且**完全沒有 `fsync`**），驗收方是讀過那張表才提的。
2.  **一條為了避免上一弧錯誤而特地寫的完成條件，本身寫反了。** 上一弧的教訓是
    「探針不得斷言脆弱的具體數，要斷言不變式」。於是寫了「每個 LOG 行都有對應檔案、
    且 LOG 行數 ≦ 目錄檔數」——**拿真實的孤兒（LOG 5、目錄 6）逐條求值，兩條款皆 PASS**。
    它釘的是**懸空指標**，而病是**孤兒**；孤兒讓目錄**大於** LOG，不等式反而成立。
    ⟹ **避開了脆弱數字的坑，掉進了方向寫反的坑。同一個失敗類別，高一層。**
    最終探針斷言的是一個**等式**：**被告知成功的行程數 ＝ 工作集反映的注入數**。
    數字可以因與本弧無關的理由而錯；**一個沒有留下痕跡的成功不會。**

### 規格側

**`SPEC_10` §2.2.2 新設 [Core Requirement]（D49）。** §2.2.1 那句「meet 為 ⊥ 時
演化**不得**發生」連同緊接的「**這一半引擎向來如此**」——那個括號就是線索：
**它寫於只有一個寫者、一個格子的時候**。新節分開循序（仍須拒絕，探針釘住）
與並行（兩筆都保留、回報 ⊥ 落在哪個座標、離開碼非零，**不得以丟棄化解**——
較晚者需要序而集合沒有序，**以到達順序裁決等於讓答案取決於排程**）。
**附自陳留白**：多重衝突時報哪一個座標未規定，〔量〕隨執行變動；**身分不受影響**（提交被拒）。

**這是本週第三條「寫於某個區別存在之前」的條文**：§4.1.2 寫於 local／published 之前；
§3.1 的順序 MUST 寫於注入模型之前；§2.2.1 寫於有第二個寫者之前。
**每一條在它自己的世界裡都是對的。**

**`GLOSSARY` §11 縮寫**（一個縮寫一個意思；`CAS` ＝ 內容定址儲存）與
**`REAL_01` §4.2 的 `staged` 行**同批出貨。§4.2 其餘部分與現況的落差已入 Inbox
——**除了 `objects/` 與 `HEAD` 兩個名字，那份參考佈局幾乎沒有一項對得上**。

### 探針完整性的一則新紀律

交付方拿掉兩條 `#[ignore]`（正確），但 `rustfmt` 順手重排了探針檔裡一處 assert 訊息，
而 §N.5 逐字寫「本檔其餘一字未動」——**那句話因此是假的**（語義為零，不值一個修補回合）。
⟹ **`rustfmt` 不得掃過探針檔**，否則「一字未動」這句永遠無法誠實地說出口。

### §8 收弧 3c 的執行結果（2026-08-30，切版後）

本弧在 `.oo/` 新增了一種耐久檔（`injections/<32-hex>`）⟹ 3c 觸發。
...
**新檔乾淨；3c 真正抓到的是舊檔。** 這一條規則的來歷是「新增一種檔會讓舊缺陷長出新的一份」，
而今天它給出的是**同一件事的第二種形**：新檔沒有復現舊缺陷，**但把舊檔的缺陷從隱形變成了顯形**。

v0.38.0 時兩層一致地錯（4–6 個欄位、4–6 個 ○）——`LOG` 是一份**對錯誤的值忠實**的紀錄。
v0.39.0 之後值層全對而 ○ 層仍是 6–10 ⟹ `LOG` 成為一份**對正確的值不忠實**的紀錄，
落差由 **0 漲到 30–34**。`D47` 的注入條款（位置變 ⟹ 產生 ○）在並行下仍不成立。

**而那份紀錄自洽**〔量，三輪逐檔〕：id 連號 `…0001`–`…000a`、無重複、**無孤兒**
（`LOG` 行數 ＝ 目錄檔數）。⟹ **Q-014 完成條件 (ii) 這三輪碰巧成立，不是被修的**——
碰撞的兩個行程鑄出同一個 id，於是**互相覆寫**而不是留下孤兒。
**一份連號、無孤兒、內部自洽的紀錄，遺漏了 30–34 次真實的位置移動**：
**任何只檢查 `LOG` 自身一致性的探針都會是穩定綠的**，這是本檔上一節那個
「不變式寫反了」的同型錯誤，而這次的綠不是碰巧。⟹ 已逐字寫進 Q-014b 的完成條件 (iii)。

〔讀〕成因未動：`savepoint.rs:52` `record` ＝ `recorded_ids()` → `mint_id(&ids)`（id ＝ 計數＋1）
→ 整檔重寫 `LOG`；唯一呼叫者 `universe.rs:894`，**與寫注入的 `:886` 相鄰兩行**。
**○ 層仍唯寫**（`savepoint::load` 仍 `#[allow(dead_code)]`），而注入層今天已有
`load_all`／`fold`／`clear` 三個產品路徑讀者 ＋ 6 支探針依賴
⟹ **W8′「每新增一種耐久檔這筆債就貴一次」在同一弧內同時被兌現與被反證**：
新檔沒有加債，舊檔的債漲了。

**未量的一半，照實記**：3c 說的是「並行／損壞量測」，**本次只跑了並行**。
崩潰／斷電窗對 `injections/` 未量（工單 §7 已先自陳為缺口），本條不視為已執行完畢。

---

## v0.40.0 ／ v0.40.0-draft.1 — a record that agrees with itself（Q-014b，2026-08-30）

**弧 A 的 A1。** `commit.md` §4.1 的四項至此走完兩項：A4 ＝ Q-013（v0.38.0）、
**A1 ＝ 本弧**；A2／A3 ＝ Q-015，未開。**這份對應此前沒有任何一份帳寫下來過**，
是用戶 2026-08-30 問「規格還沒加 savepoint 細節，是不是設計沒走完」時對出來的。

### 這一弧修的是什麼

〔量，v0.39.0 標籤二進位〕120 個並行 `oo evolve`、各加一個相異欄位
→ Savepoint **96–115／120**，**每個行程都回報成功**。

而那份紀錄**自洽**：id 連號、無重複、**無孤兒**——因為碰撞的兩個行程讀到同一個計數、
鑄出同一個 id，於是**互相覆寫**，而覆寫不留孤兒。

⟹ **一份連號、無孤兒、內部自洽的紀錄，遺漏了三十次真實的位置移動**，
且**任何只檢查該紀錄自身一致性的斷言在這個缺陷上都是穩定綠的**——不是碰巧綠。

### 兩則裁定，其實是一句話

**D50（複數）** 與 **D51（覆蓋關係改變也產生 ○）** 合起來是 `commit.md` §1.11、
承重**第 12 句**：**歷史層不繼承值層的冪等性。**
meet 冪等是值層不需要協調的理由，**而事件不冪等**；
已知**五個實例**同一個形——**拿值層的性質當歷史層的判準**，
**而 D47 自己指認過這個類別錯誤一次，卻只修了一半**（判準從 combo 換成格上的位置，兩者都在值層）。

**用戶的兩次提問各推進一格。** 第一次指出「順序」原本想講的是 **`Savepoint` 有 parent**
——而那句話逐字就在 `commit.md` §1.7.2（用戶 2026-08-03）；
第二次裁複數，並往下推「最後可能會像 os 的 `fork()` 一樣」。
**`fork()` 的類比只成立到一半**：分叉是一等事件、parent 邊無條件記錄 ✓，
但 **n/ 沒有 join 動詞** ⟹ §0.1 第 3 句就地更正：
**不是只剩撤回沒有落點，是撤回與匯流兩支都沒有**，而兩者缺席的理由同一個。

### 驗收方複驗到、而理由比用戶給的更強的一件

用戶說單數是「隨欄位從 `Commit` 搬過來的」。〔量〕**控制面 `REAL_01` §2 自始即宣告
`"parents": ["hash:…"]` 為陣列**，而 `value.rs:1765` 是 `Option<ContentHash>` 單數
——`commit.md` §2.6 早就把這一列記為不合。
⟹ **規範面本來就是複數，單數只活在 Rust；搬家時搬走的正好是不合的那一半。**

### 探針：這一次的教訓是「紅得漂亮還不夠」

**r1 的寬度是校準出來的，不是選的。** 40 個行程在 release 上每次都紅（6／8／10 對 40），
但在 `cargo test` 的 **debug** 建置下**五次會綠一次**——debug 的 `oo` 大半時間在啟動，
讀-改-寫的窗只佔一小部分。
**一支五次只紅四次的紅不是釘子**：跑一次看到綠，就會得出「不必做」的結論。
改為 **120** 之後 **12／12 紅，最差 115／120**。
⟹ **上一弧的教訓是「不變式不要寫反」，這一弧的是「要問它多常紅」。**

**明文沒有釘的一格，寫進探針而不是留白**：D51 的**匯流**在 CLI 層沒有針——
兩個並行注入同一個檔，**兩顆 ○（真分叉）與一顆（後者看見前者、候選是線性 no-op）都是對的**，
取決於排程；在那裡斷言一個數字**等於釘住排程器**。改以工單 Q3 要可跑的示範，交付方給了
（種一個分叉再循序 no-op，第四顆 ○ 的框逐字 `parents: aaaa… bbbb…`，**循序、可重跑**）。

**S4 是最容易做錯的一項，而它被逐字寫死在工單裡**：D51 **不是**「每次成功的 evolve 都鑄」。
循序 no-op 的候選只有一個父（當前唯一 tip）且值與它相同，**什麼都沒加**；
讀成「一律產生」會讓一個**沒有回收機制的集合**在重複的無效操作下**無界成長**
——**那正是 `fork()` 類比裡 fork bomb 的位置**。探針 `g1` 是守衛，
〔驗收方複驗〕同一個檔循序 evolve **20 次 → ○ 仍是 1 顆**。

### 驗收方寫工單時才想到、而交付方解對的一件

**Q2（既有倉）**：v0.38／v0.39 落地的 ○ 沒有 `parents:` 行，
純導出會判**每一顆舊 ○ 都是 tip** ⟹ 第一顆新 ○ 會宣稱一次**從未發生的多路匯流**。
交付方報了三條路的價，走相容那條：**缺 `parents:` 且名字仍是 16 位計數者，
依檔名序推導為一條鏈** ⟹ 第一顆新 ○ 指向舊鏈**末端**。
〔驗收方複驗〕用 v0.39.0 造 3 顆舊 ○ 再用新引擎 evolve，新 ○ 的 `parents`
逐字是 `0000000000000003`。**舊檔原地不動，是讀時推導不是遷移。**

### 探針完整性

**diff 只拿掉兩個 `#[ignore]`，本檔其餘一字未動，`rustfmt` 沒有掃過它。**
上一弧新立的那條紀律第一次被遵守，「一字未動」這句第一次是真的。
`ignored=0` ⟹ 沒有第三支被順手關掉。

### 規格側

`SPEC_10` §3.1 **順序款 → 前驅款**（D50）＋新增 **前驅不得由到達順序決定（MUST NOT）**；
**產生判準加 (c)**（D51），並把「**不得讀成一律產生**」寫成 MUST NOT；
`REAL_01` §4.2 補上 `savepoints/`——在此之前，**`SPEC_10` §3.1 以五條 MUST
立法了一個目錄，而耐久佈局那一章不知道它存在**〔量：`spec/zh_TW/` 全樹 `savepoints` 零命中〕。

### 未兌現，逐字記帳

*   **§3.1 判準 (b) 與 `_|_` 那一款仍為零。** 成因**不只是** Q-018：
    〔量〕`oo run … -o` 確實化約了 thunk（`~%Math./add (1,1)` → `2`）而 ○ 不動，
    **而今日的記錄介面接受的是一個值，表達不了「觀測化約了什麼」**。
*   **判準 (a) 的實作仍比對「前一顆的位元組」而非格上的位置**
    ⟹ 提交邊界之後的第一筆重複注入仍鑄一顆位置未移動的 ○。
*   兩件工單都逐字寫過本弧不修 ⟹ **交付做到了它說的，也沒做它說不做的。**

### 驗收方自己的一件

差一步用 `--version` 判斷二進位是不是新碼：它自報 `v0.39.0-721-gdbb7321+`
（**工單那一顆**）而 `git describe` 是 `v0.39.0-722-ged68b8d`。
改問行為（跑一次 evolve 看 ○ 是不是 32-hex、框上有沒有 `parents:`）才過關。
**那是 Inbox 該列的第三個實例，而這次重現成本是零**，已補進該列。

### 數字（標籤建置 `/home/gali/nlang-baselines/v0.40.0-verify`，`oo v0.40.0` 無後綴）

*   全跑 `--no-fail-fast`、逐 target 聚合：**`targets=220 passed=2118 failed=0 ignored=0`**，
    exit 0、`^error` 0。六支本弧探針全綠。
*   conformance **162／162**。
*   **120 並行三輪：宣稱成功 120／○ 120／欄位 120／注入 120**（基線 96–115）；
    `.oo/savepoints/LOG` **已不存在**。
*   身分：`x: 0` 根 `31745ef0e8bfde3d…`、物件數 **3**、標準根 `7038e2504b8ef4d4…`。
*   `rollback`／`gc`／`migrate` 之後 ○ 的檔數與位元組**完全不變**。
*   循序 no-op ×20 → ○ 由 3 增為 **4**（只有第一筆真的移動了位置）⟹ **不是 fork bomb**。
*   S5：唯一一顆 ○ 自指 → `Error: savepoint cycle: ids nonempty and tips empty`、rc=1、不鑄。

**⚠ S5 這一項驗收方第一次量錯了**：腳本讓**所有** ○ 都指向同一顆，於是 tips 根本不是空的，
rc=0 是**正確**行為而驗收方差一步把它讀成回歸。重做成「唯一一顆 ○ 自指」才真的碰到那一格。
**又一次「先證明你的量測碰到了你以為它碰到的東西」。**

### §8 收弧 3a／3b 的掃描結果

*   **3b（本弧換掉磁碟表示：`LOG` 移除、框多一行、id 由計數改亂數）——兩列重量，兩列都仍開**：
    「判準 (a) 比對的是位元組不是位置」〔重量：`evolve a → evolve b → commit → evolve a`
    仍鑄第三顆，而第二次 commit 只多一個物件 ⟹ root 未變〕；
    「判準 (b) 與 `_|_` 那一款兌現度為零」〔重量：`run -o` 化約了 thunk 而 ○ 不動〕。
    **兩件工單都逐字寫過本弧不修。**
*   **3c 不觸發**：本弧**移除**了一種耐久檔，沒有新增。
*   **3a：「提交撞 ⊥ 時逐字回報 `Commit successful` 且 exit=0」那列未被 #23 結掉**
    〔量，標籤建置：`c: c + 1` → evolve rc=0、commit rc=0〕。**連續第二次切版複驗仍在。**
*   `REAL_01` §4.2 失真那列**部分結掉**（`savepoints/` 已補入），其餘失真項不變。

### 版本判定

**破壞性條目 #23，格式軸 ⟹ 90 天穩定時鐘不重啟**（前次重啟為 #22，2026-08-30）。
理由：沒有任何程式改變意義、沒有任何 CAID 移動、且 Savepoint 依 `SPEC_10` §3.1 **永不旅行**
⟹ 沒有第二方會被打斷。判定沿用**條目 #19**（`encoding` 4 → 5）的先例。
**驗收方提出，用戶 2026-08-30 確認「不重啟」。**

---

## Q-015 — 一份被寫了兩次的歷史（2026-08-31；**四個修補回合**）

**弧 A 的 A2 ＋ A3 的第一半。** 裁定 **D52**（○ 長出提交邊、`Commit.parent` 不再設定）／
**D53**（鑄造圖與呈現圖不必同形，呈現圖是鑄造圖的純函數）／
**D54**（鑄造圖的節點＝○，提交是註記）／**D55**（提交 ○ 帶兩條邊：時間覆蓋與祖先）。

### 這一弧修的那句話

〔開弧前量〕**○ 與 ● 從不互相指涉**——○ 的框裡沒有任何 CAID，`Commit` 的五個欄位
沒有一個指向 ○ ⟹ **§4.1 的 A2 原話「歷史走訪須先改走 ○ 鏈」走不通，因為 ○ 鏈裡沒有歷史。**
`commit.md` §1.7.1 說「那是兩張圖，不是一張」；今天兌現的是它的字面意思——
**兩張圖，而且沒有一條邊把它們接起來。**
...
*   conformance **162／162**。
*   身分：`x: 0` 根 `31745ef0e8bfde3d…`、物件數 **3**、標準根 `7038e2504b8ef4d4…`。
*   **$H_1 = E-V+C$**：N ＝ 3／10／20 皆 **0**（$V=2N$、$E=2N-1$、$C=1$）。
*   **跨版本混鏈**（真 v0.40.0 標籤二進位造倉）：`oo log` ＝ `new4 old3 old2 old1`；
    `oo gc --grant gc` **removed 0 objects**；最舊那顆仍 `inspect` 得到。
*   **rollback 與基線逐字相同**：兩邊皆 `final gen1`、各恰一行 `abandoned`。
*   **squash 對被回溯掉的 base**：兩邊皆 `Error: squash base is not an ancestor of HEAD`。
*   弧探針 **9／9**。
...
| 回合 | 缺陷 | 誰的鍋 |
| :-- | :-- | :-- |
| **1** | 線性 session 長出憑空的洞：$H_1$ ＝ 0／0／1／3／**8**（N＝1／2／3／5／10）⟹ **十次提交宣稱八次不存在的匯流** | **交付**（與 S2 的公開偏離） |
| **2** | 修好拓撲後**祖先壞了**：rollback 之後被放棄的整段回到 log（基線 2 列 vs 交付 **5 列**），且 **`squash` 接受被回溯掉的 base 並真的產出 squash commit** | **工單**（S2 把兩個關係壓成「恰有一個父」）⟹ **D55** |
| **3** | 舊倉上 commit 一次再 `gc`：物件 **7→3**、log **3→1**、兩顆舊 commit `CAID not found` ⟹ **無聲且不可回復的資料遺失** | **工單**（A2.6 寫「鑄造當下 HEAD 的那顆提交 ○」，而本弧之前的 commit 沒有 ○） |
| **4** | 上一回合為此加了 `predecessor_by_timestamp`：**掃遍 store、以掛鐘順序猜祖先** | **驗收方的探針**（舊 G7 造了一個真實世界產生不出來的狀態） |

**⚠ 回合 4 那一件最值得留**：舊 G7 用**本弧引擎**造 commit 再剝掉註記，
剩下 `parent: None` ＋ 無 ○ 點名——**不可能出現的狀態**。交付方為了讓它綠，
加了一個從時間戳推導祖先的機制，而**那正是本弧整個在反對的事**（§1.7.1：本地圖是**宣告**的）。
**更難看的是工單早就寫對過**：§4「逐字記下沒有探針的兩件」第 2 條原文就是
「跨版本混鏈**造不出來**……由驗收方用 v0.40.0 標籤二進位手量」。**我寫了那段，然後還是去寫了那支探針。**

### 驗收方本弧補了三支探針，三支都是工單自己漏掉的性質

**G5**（線性 session 必須 $H_1=0$，回合 1 紅）／**G6**（回溯段必須離開 log，回合 2 紅）／
**G7**（走訪與回收必須走同一條邊；**第二版**，第一版錯了見上）。
**三支的測試修改權皆屬驗收方。**

### 兩件記在驗收方帳上的射程錯誤

1.  **走訪的讀者是六個不是五個**——工單漏了 `gc::mark`，交付方補上。
    `mark` 沿物件裡的 CAS 邊走，而 R2 已證明新 commit 的位元組不含前一顆的 digest
    ⟹ 不補雙讀，`oo gc` 會收掉 `oo log` 還走得到的歷史。
2.  **G3 對回合 1 的缺陷是穩定綠的**：它只量一次 commit 前後的 $H_1$，
    而**洞不是在 commit 開的，是在下一次 evolve 開的**。
    而那個建置的全跑是 **221／2126／0** ⟹ **全綠不等於被檢驗過**的最乾淨實例：
    **沒有人把「這段歷史說了什麼」當成可以量的東西，而 D53 把它變成了一個數字。**

### 一條新的常設規則（本弧賺到）

**全跑的聚合必須連失敗的測試名一起留。** 〔起因〕驗收方的聚合只留計數，
於是三次跑出「1 綠 2 紅」時，**手上沒有任何東西能分辨「交付有偶發缺陷」與「我自己剛加了一支紅」**。
**「一支五次只紅四次的紅不是釘子」的反面同樣成立——一支三次只綠一次的綠也不能當綠**，
而兩個方向都要**先知道紅的是誰**才判得出來。**只留計數＝把證據丟掉。**

### 規格收尾（驗收方，已完成）

*   **`SPEC_10` §3.1 新增三款**：**提交亦產生 Savepoint（MUST）**（D52／D53，含「不得同時指向多個前驅」
    及那個八次假匯流的量測）／**前驅與祖先是兩個不同的關係（MUST）**（D55，含兩種混用各自的實測失效形）／
    **祖先不得由時間或到達順序導出（MUST NOT）**（D55，含毫秒與 54 ms 的量測）。
*   **`SPEC_08` §6.2 §578 射程更正**：本款寫於 `Commit.parent` 是唯一祖先載體之時；
    **所拘束的是「承載祖先的那條邊」，不是欄位的名字**。要求未變，指名的載體變了。
*   **`REAL_01` §4.2**：`savepoints/<local-id>` 那行補上提交與祖先兩項註記。
*   **§8 收弧 2a：四條新條文引擎全部已兌現**（R1／G5／G6／G7 逐一釘住）⟹ **不開 Inbox 列**。
*   **無合規向量，逐字記帳**：四款的性質全在儲存容器與 CLI 走訪，語料形式無法表達
    （沿用條目 #24 之前 Q-038 的先例）。**162 對 162 不變。**

### §8 收弧 3a／3b 的掃描結果（v0.41.0 切版當日）

*   **3a：條目 #24 掃 Inbox ⟹ 結掉一列。**
    **「混鏈之下 `squash` 的祖先檢查可能掛錯鏈」已結**〔複驗，真 `v0.40.0` 造的三顆舊倉
    ＋ 本版再提交一顆〕：`oo log` ＝ `new4 old3 old2 old1`，
    **對最舊那顆下 `squash --grant squash` 成功**（log → `compressed old1`）。
    **D55 的祖先邊把這一格一併解掉了**——該列開列時是「本弧沒宣稱要修」。
*   **3a：「提交撞 ⊥ 時逐字回報成功且 exit=0」那列仍未被 #24 結掉**
    〔量：`c: c + 1` → evolve rc=0、commit rc=0〕。**連續第三次切版複驗仍在。**
*   **3b（本弧換掉磁碟表示：○ 的框多兩項註記、新 Commit 不再含前一顆 digest）
    ——兩列重量，兩列都仍開**：判準 (a)〔提交後再注入同一定義：物件 3 → 3 而 ○ 3 → 4〕；
    判準 (b) 與 `_|_`〔`run -o` 化約 thunk 得 `2` 而 ○ 不變；⊥ 觀測 rc=0 而 ○ 不變〕。
    **四份工單都逐字寫過本版不修。**
*   **3c 不觸發**：沒有新增耐久檔種類（是既有的框多兩項註記）。

### 版本判定

**破壞性條目 #24，格式軸 ⟹ 90 天穩定時鐘不重啟**（前次重啟為 #22，2026-08-30）。
理由：**沒有任何 CAID 移動**；**Commit 不旅行**〔量〕OODP 取用處理器只服務 `get_value`，
`get_commit` 在 `oodp.rs` **零命中** ⟹ 網路上沒有第二方被打斷；
**受影響方是同一顆磁碟上的另一個二進位，而它並未被鎖在門外**〔量〕
`v0.40.0` 對本版寫成的倉 `oo log` 印 1 列而非 3 列，**但仍讀得到 HEAD、仍 inspect、
仍 evolve、仍 commit（rc=0）** ⟹ **走訪降級，不是打不開**（對照條目 #18）。
判定沿用 **#23／#19**。**驗收方提出，待用戶確認。**

### 標籤建置複驗（v0.41.0，切版末步）

二進位 `/home/gali/nlang-baselines/v0.41.0-verify/target/release/oo`，
`oo --version` → **`oo v0.41.0`（無後綴）**；known-answer `~%Math./add (1,2)` → **`3`**，
對照 `add (1,"x")` → **`_|_`**（證明呼叫發生了）。

*   全跑：**exit 0、221 target、2127 passed、0 failed、`^error` 0、失敗測試名為空**
    ——**與 `dev` 建置逐項相同**。
*   conformance **162／162**。
*   身分：`x: 0` 根 `31745ef0e8bfde3d…`、物件數 **3**、標準根 `7038e2504b8ef4d4…`。
*   **$H_1$**：N ＝ 3／10／20 皆 **0**。
*   **跨版本混鏈**（真 `v0.40.0` 標籤二進位造倉）：`oo log` ＝ `new4 old3 old2 old1`；
    `oo gc --grant gc` **removed 0 objects**；最舊那顆仍 `inspect` 得到。
*   **rollback 與 squash 兩組與 `v0.40.0` 基線逐字相同**：
    `final gen1`／各恰一行 `abandoned`／`Error: squash base is not an ancestor of HEAD`。

**切版四項清單複驗**：`VERSION` ＝ `0.41.0-draft.1`；`CHANGELOG` 新節 1 個；
`SPEC_00` §4.4 新列 1 個且 **`(目前)` 計數 == 1**；tie-back 兩邊
`merge-base --is-ancestor` 皆 YES 且**樹逐位元組相同**。
外加 `crates/oo/Cargo.toml` ＝ `0.41.0`。

**標籤**：super `v0.41.0`（`07b8847`）／spec `v0.41.0-draft.1`（`f060ad0`）／
tools `v0.41.0`（`abf5c9f`）。

---

## Q-017 — 一次回報成功的提交，其實是一個 `_|_`（2026-08-31；**兩個修補回合**）

裁定 **D56**（§4.1.2 射程軸改為「操作者是否被告知」；排除款以回報為前提）／
**D57**（知情之事實是**審計不是授權**，記在 ●）。承 **B1**（2026-08-06）。

### 這一弧兌現的是一條寫了二十五天沒被實作的 MUST

`SPEC_10` §4.1.2「回報的內容（MUST）」逐字：「引擎**必須**指出 `_|_` 落在哪些座標，
而**不僅回報「成功」**。**靜默地固化一個帶矛盾的狀態，等於讓操作者在不知情下做決定。**」

〔開弧前量〕**單寫者、三個指令**：`c: c + 1` → `evolve` 無輸出 rc=0 →
`status` 逐字印 `c: c + 1` → `commit` 逐字 `Commit successful` rc=0 →
根裡是 `c: _|_ (%cause: #divergent)`。

### 驗收數字（修補回合 2 之後）

*   全跑 ×3：**每次 exit 0、222 target、2132 passed、0 failed、`^error` 0、失敗名為空**。
*   conformance **162／162**；身分 `31745ef0…`／**3 物件**／`7038e250…`；弧探針 **5／5**。
*   〔量〕現行 layout：`c: c + 1` → **`#divergent at c`** ＋ `Commit successful`、**rc=0**、HEAD 寫入。
*   〔量〕舊倉（`layout=2`）未遷移 ＋ 帶 ⊥ 的提交 → **rc=1**、HEAD 不動、**`v0.41.0` 仍 rc=0**；
    同一個舊倉的**乾淨**提交 → **rc=0**，`v0.41.0` 仍 rc=0。
*   〔量〕遷移後 `v0.41.0` 得到 **`store layout declaration "layout=3" is not supported`**
    ——**不再是 `#caid_mismatch`**。
*   〔量〕**復原路徑真的管用**：被拒之後工作集仍在 → `oo migrate` → 同一筆提交落地，**一個字都沒掉**。

### 兩個修補回合，兩次都是驗收方的射程

| 回合 | 缺陷 | 誰的鍋 |
| :-- | :-- | :-- |
| **1** | 帶標記的 commit 讓 `v0.41.0` **完全打不開那個倉**（log／status／gc／evolve／commit 全 rc=1），而它說的是 **`corrupt`——假的** | **裁定**（D57 只為 CAID 移動定價，未為讀相容定價） |
| **2** | 升 layout 之後，**未遷移的舊倉仍會被寫成那個說謊的鎖死** | **工單**（S7 寫的是**機制**「migrate 要能前進」，而該守的是**不變式**） |
...
**這是同型的第四次**（Q-015 的 S2「恰有一個父」／S5 的讀者表漏 `gc::mark`／
A2.6 的祖先邊漏掉「本弧之前的 commit 沒有 ○」）。**四次都是驗收方寫的射程。**

### 交付方做對而值得記的三件

1.  **Q4 沒有用推論代替量測**——工單指名要量跨版本，交付方量了，**而且量到的是壞消息，照實寫**，
    並在 N.7 正確判斷「這是 D57 的後果不是實作選錯」。**那份自陳直接換來了兩則裁定。**
2.  **`STORE_LAYOUT_MIGRATABLE_FROM` 是封閉清單不是範圍**，註解自己指出
    「一旦 current 變成 5，未來的 `layout=4` 不得溜過去」——**比工單要求的嚴。**
3.  **修補 2 的理由比工單清楚**：舊引擎驗證不了的是**帶標記的 commit digest**，不是每一次寫入
    ⟹ 該擋的是那一種提交，不是那個倉。**於是沒有踩到 ⊥ 的人完全不受影響。**

### 規格收尾（驗收方，已完成）

*   **`SPEC_10` §4.1.2**：射程款加 D56 的修訂框（「顯式」是關於意圖的主張，
    **排除款以上一款之回報為前提**）＋**新增兩款**：
    **知情之事實必須留在歷史（MUST）**（D57，含「根逐位元組相同 ⟹ 同意進不了根」的實測）／
    **該標記為審計而非授權（MUST NOT）**（不得要求能力或旗標；不得宣稱多於實際發生者）。
*   **`SPEC_10` §2.3 兩行殘留就地更正**：「語義」那行寫於 D46／D48 之前；
    「原子性保證」的後半**已刪**——它與 §4.1.2 開頭所引者是同一條規則，
    2026-08-06 拆解時**漏了本節這一份**，而 §4.1.2 的量測已說明它為何永不開火。
*   **`REAL_03` §6.8 新增判例三**：同一款、另一條軸、與標準根無關
    ⟹ **本款拘束的不只是標準根那條軸**。
*   **`REAL_03` §6.8 新增 MUST NOT**：**版本落差不得報成完整性失效**
    ——「我比你新」與「你的位元組壞了」是兩件事，**而只有後者要求操作者去搶救資料**。
    〔§8 收弧 2a 已量：**引擎已兌現**〕`layout=9` → `store layout declaration … is not supported`；
    `encoding=9` → `object encoding version 9 is not supported` ⟹ **兩軸皆在閘上誠實拒絕，不開 Inbox 列。**
*   **Inbox 新開一列**：`oo status` 與提交後的根對同一個倉給出兩個答案
    （工單逐字寫明本弧不修；**且它不是 §2.2.1 的違反**——該節被自己的第一句閘住，條文夠不到它）。
*   **無合規向量，逐字記帳**：性質全在 CLI 回報與容器宣告，語料形式無法表達。**162 對 162 不變。**

### §8 收弧 3a／3b 的掃描結果（v0.42.0 切版當日）

*   **3a：條目 #25 掃 Inbox ⟹ 結掉一列，而那一列已經連續三次切版複驗仍在。**
    「提交撞 ⊥ 時逐字回報 `Commit successful` 且 `exit=0`」**已結**
    〔複驗〕`c: c + 1` → `commit` **先印 `#divergent at c`**，再印 `Commit successful`。
    **⚠ `exit=0` 保留，而且現在是裁定不是遺漏**——B1 逐字「要修的不是『中止』，是『報告』」；
    **本列原文把「回報成功」與「exit=0」並列為病，而拆開之後只有前者是病。**
*   **3b（本弧換掉磁碟表示：commit 物件多一個審計欄、`.oo/format` `layout=2`→`3`）**：
    「`_|_` 的 `message` 欄不在自己的 CAID 之內」〔重量〕同長度竄改 `message` 一個位元組
    ⟹ `oo inspect` **rc=0、逐字印出被竄改的值**。**仍在**（工單明列本弧不修，服務面）。
*   **3c 不觸發**：沒有新增耐久檔種類。

### 版本判定

**破壞性條目 #25，語義（回報）＋格式軸 ⟹ 90 天穩定時鐘重啟**（前次重啟為 #22，2026-08-30）。
**理由與條目 #24 逐字相反**：#24 以「**舊引擎並未被鎖在門外**（仍 inspect、仍 evolve、仍 commit）」
...
## Q-016a — 一次覆寫，其實是擲了一次硬幣（2026-09-03；**零修補回合**）

裁定 **D58**（`pin` 在 evolve 當下逐座標吸收既有注入）。原 Q-016 於 2026-09-03
重量後拆為 **a**（`pin`，本弧）／**b**（並行提交，未裁，只能開偵察）。
**交付由 agent #4 完成。**

### 開卡前的重量推翻了卡片自己的兩個假設

卡片逐字寫著「⚠ Q-014 已把 meet 那一半的表面積削掉一大塊 ⟹ 開卡前必須重量本卡還剩什麼」。
重量的結果：

*   **`pin` 那一半不需要並行**〔量 2026-09-02，`v0.42.0` 標籤二進位，known-answer 已過〕
    **兩個指令、一個行程**：`evolve --pin`（`x: 2`）→ `evolve --pin`（`x: 3`）→ `status` 得 **`x: 2`**，
    兩次皆 rc=0 且無輸出。20 次試驗**晚者勝 12、早者勝 8**，
    **20／20 與「隨機注入 id 字典序最大者勝」相符**。
*   **meet 那一半剩下的不是值而是歷史**〔量，三輪 ×20 並行 `evolve`＋`commit`〕
    commit rc=0 計 **20／19／20**，而 `oo log` 皆 **2**；**值 20／20／20 全存活**。
    ⟹ 丙的注入模型連並行提交都撐住了；丟的是 ●。**該半需要另一則裁定，拆為 Q-016b。**

### 成因是一個型別錯誤

D48 把工作集改成**集合**並以 meet 折疊；meet 具結合／交換／冪等三律，故「對集合折疊」有定義。
**`pin` 是覆寫，三律皆不具備** ⟹「對集合以覆寫折疊」**根本沒有定義**，
而引擎照做了（`universe.rs` `load_staged` 的 `pin_pending` 分支），
缺的序由 `injections::paths()` 的 `out.sort()` 補上——**而檔名是 16 bytes 的 OS 熵**。

〔對照組〕不帶 `--pin` 的兩次衝突 evolve 在**演化邊界**即被拒（`#conflict at x`、rc=1、不寫注入）
⟹ **只有 `pin` 造得出兩筆互相衝突而共存的注入**。

**⚠ 而當時的實況比 LWW 更差**：兩個運算元**都**具 `pin` 能力 ⟹「誰有權」分不出勝負；
集合無序 ⟹「誰比較晚」也拿不到。**兩個裁決者都落空，裁決者是熵。**

### 交付的形狀

注入成員於**內容內**攜帶 intrinsic `id`、其 `pin_coords`、以及
`absorbs: { 座標: [被吸收的 id…] }`；**檔名只剩本地儲存鍵**。
fold 先取全體 `absorbs` 的聯集，再從被指名的成員投影掉該座標，最後一律 meet。
`load_staged` 的 `replace_merge` 分支刪除；根與工作集之間的 `pin_commit_merge` 不動。
`.oo/pin_pending` 不再建立，僅讀 layout ≤ 3 的遺留檔；legacy id 由**位元組的 SHA-256** 導出，
註解逐字寫明「用檔名會把 D58 剛拿掉的那個改名相依性請回來」。

### 驗收（驗收方獨立複驗，不採信報告）

*   全樹 ×3 `--no-fail-fast`：**223／2139／0**，三輪皆然，**無失敗測試名**。
*   conformance **162／162**；身分 `31745ef0…`／3 物件／`7038e250…`。
*   **驗收方的探針檔未被動**（`git diff` 為空）。
*   **跨版本 layout 矩陣，真二進位**：新倉 `layout=4`；
    `layout=3` 倉被新引擎普通 evolve 後，**真 `v0.42.0` `status` rc=0 讀得到、`commit` rc=0 提交得了**；
    `layout=3` ＋ `--pin` ⟹ rc=1 且 **`injections=0`**（寫任何成員之前），訊息指名 `migrate`；
    `v0.42.0` 開 `layout=4` ⟹ rc=1「`layout=4` is not supported; refusing to open」，**不是 corrupt**；
    `v0.42.0` 留下的 sidecar 由新引擎讀入、提交後移除，**佈局不因此前進**。

### ⚠ 驗收查到的兩件，皆入 Inbox

*   **`interrupt-candidate`：`.oo/effect_pending` 是 `pin_pending` 的孿生，
    而它讓一次能力檢查被競速打穿。**〔量〕循序 `--grant effect_override:io` 再 `:nondet`
    ⟹ 該格子取聯集，`commit --grant effect_override:io` rc=1（`discharged #io | #nondet`）；
    **並行同樣兩件事，該格子從不取到聯集** ⟹ **10 次試驗提交成功 5 次**，
    落地的根含兩個座標而標記只宣稱較窄的那一個。
    **與 D57 不同層級**：D57 是**審計**不足，本件是**授權**被繞過。
    **⚠ 它是怎麼被找到的**：本弧改寫了 `atomic_write_probe_test` 的 R3，
    而**舊 R3 的註解逐字寫著「`pin_pending` stands for the family」**
    ——`effect_pending` 與 `abandoned` 走同一條 `save_staged` 路徑。
    **新 R3 只釘 pin，那一族其餘兩個從此無人看守**（全樹搜尋無其他測試釘其原子性）。
*   **`oo status` 一邊印 `Universe unavailable` 一邊 rc=0**（§2.2.1「各回報面一致」家族）。
    **殘留不是回歸**——同樣的破壞下 `v0.42.0` 印的是 `Universe is static` 與 `Nothing to commit`，
    **兩句都是假的**；本弧把 `let _ = load_staged` 改成 `?` 才使這條路徑首次可達。

### 收尾

`SPEC_10` **§2.2.3 新設**（五款）／`REAL_02` §5.1.1 **新增 MUST NOT**（宣告未推進者不得
收到它宣告不了的東西——**Q-017 S7 的教訓第一次被寫成條文**）／`REAL_01` §4.2 參考佈局。
**§8 3b 重量兩列，兩列皆仍開**：objects **3 → 3** 而 ○ **2 → 3**；`run -o` 化約而 ○ 不變。

### 這一弧留下的可重用之物

**檢查一個弧有沒有拿走它換掉的那支探針的覆蓋。** 本弧的探針改寫是正當的
（舊斷言與 D58 直接衝突），**而它同時把一族三個縮成一個，且那一族的其餘兩個真的有病**。
⟹ **改寫既有探針時，要問「它原本代表誰」。**

---

## Q-040 — 一個你刪得掉的授權（2026-09-06；**一個修補回合**）

裁定 **D58**（形狀，承 Q-016a）／**D60**（價格：現在就花 `layout=5`）／**D57**（為什麼審計不住工作區）。
**交付由 agent #4 完成。**

### 這一弧修的是一個已經修過一次、只修了一半的洞

`main.rs` 的 `pin` 閘註解（**2026-07-26**）逐字：能力**必須經可信通道出示**，
**不得由耐久意圖推定**；並指出 `.oo/` 是 **any n/ program can write** 的目錄。
**同一個論證從未被套用到效應解除**——而它就寫在同一個檔案、同一個函式裡。

〔量 2026-09-06，`v0.43.0` 標籤二進位，離開碼不經管線〕**決定性**：
`evolve --grant effect_override:io` → `commit` 無 grant ⟹ `exit=1`（閘是對的）；
**`rm .oo/effect_pending` 之後同一個提交** ⟹ **`exit=0`，且該 Commit 連特權標記都沒有**。
〔並行版本，最初的發現形式〕十次試驗，僅持其一之能力者**提交成功五次**。

### 驗收（驗收方獨立複驗，不採信報告）

全樹 ×3 **`--release`**：**224／2147／0**，三輪皆然、無失敗測試名（1m54s–3m14s）。
conformance **162／162**；身分 `31745ef0…`／3 物件／`7038e250…`；**探針檔一字未動**。

**跨版本矩陣六格自量**，其中最重要的一格是**交付方越界所開出的邊緣**：
他們把 `pin` 的寫入閘由 `layout_declaration_is_current` 改為 `layout_writes_pin_frame`
（自報、逐行說明），使 `layout=4` 在 current 變成 5 之後仍能收 `--pin`。
**那開出一個問句**：pin 允許而 discharge 不允許，兩者同時出現時若 pin 先寫成員
而 discharge 悄悄退回 sidecar，本弧要拿掉的洞就會被寫回去。
〔量〕**整個 evolve 在鑄下任何成員之前被拒，注入 0、不寫 sidecar** ⟹ **該越界成立**。

### ⚠ 修補回合 1：一個數字打穿本弧自己的主張，而成因是驗收方的射程

〔量〕同一成員只改 `effect_tags` 一行：`1` ⟹ `exit=1`；`not-a-number` ⟹ `exit=1`
（交付方為壞 JSON 關過這道門）；**`8`（只由未知位元構成）⟹ `exit=0`，提交落地**。

〔讀〕`value.rs` `from_bits` 對未知位元**遮蔽**，其註解逐字寫著理由
——「**unknown bits must not become capabilities**」。**那個理由對它自己是對的**：
問句若是「這能不能**授予**什麼」，未知位元當然不得成為能力。
**但這裡的問句是「有沒有東西被 discharge」**，遮蔽把未知位元變成空集合，
而空集合讀起來就是「沒有」。

⟹ **它否定了射程宣稱買到的東西**：射程寫「移除授權事實**必須改寫承載值本身的那個物件**」
——`effect_tags: 8` 確實改寫了那個物件，**而值一個位元組都沒動**。

**⚠ 成因是驗收方的射程**：探針 R2 寫的是「**刪掉**格子」，而不變式是「**移除**授權事實」
——**刪是一種方式，改是另一種**。**這是 `feedback_scope_invariants_not_mechanisms` 的第五次。**

**修補後**六種寫入（`1`／`not-a-number`／`8`／`9`／`16`／`255`）**全部拒絕**；
`9`（已知｜未知）由「未出示能力」改判為「**讀不完整**」——**更嚴且更誠實**。
修補做在**解碼路徑**上，`from_bits` **一字未動**（工單明文要求），
訊息不是 corrupt、不是 `#caid_mismatch`、也不是「沒有 discharge」。

### 收尾

`SPEC_10` **§2.2.4 新設**——把 §2.2.3 第四款由**只綁 `pin`** 推廣成**類別**。
**⚠ 而那款是驗收方 2026-09-03 寫的，同一天它自己填了 `effect_pending` 的 Inbox 列**
⟹ **握著孿生案例的量測，寫了一條只綁個案的 MUST。**
`REAL_02` §5.1.1 **新增 MUST NOT**（讀不出來不得等於沒有）。
Inbox 收一列（`oo migrate` 的成功句在動手之後才說代價且指名最舊的版本）。
**§8 3b 重量兩列，兩列皆仍開。**

### 這一弧留下的可重用之物

*   **「移除」有兩種方式：刪與改。** 釘住其中一種的探針不叫釘住那個不變式。
*   **同一個函式在兩個問句下一對一錯。** 遮蔽未知位元對「能不能授予」是對的，
    對「有沒有發生過」是錯的——**而它的註解只寫了對的那一半，於是讀起來像已經想過了**。
*   **一輪全樹的時間取決於呼叫方式**：交付方 debug ＋ `--test-threads=1` 約 20 分鐘，
    驗收方 `--release` ＋ 預設執行緒 **2 分鐘**。**工單逐字寫的是 `--release`。**

---

## Q-016b 軸 A＋C — 一次提交，吃掉了它沒有讀過的東西（2026-09-06；**一個修補回合**）

裁定：**軸 A／C 不需要裁定**（`SPEC_10` §2.2.1 給 A；§3 第 2 款 ＋ **D48** 給 C）。
**軸 B 由 D61 改判**，並分出 **Q-041**。**交付由 agent #4 完成。**

### 偵察推翻了驗收方的兩分法

驗收方把卡切成「訊息（軸 A）」與「HEAD 拓撲（軸 B）」兩半。
**偵察指出漏了第三軸，而那一軸才是值遺失的主詞**：`injections::clear` **不接受集合**
——它在**呼叫當下**列出整個目錄並刪光，而 `load_all` 與它相隔近兩百行。
〔驗收方複驗：讀那兩行即可確認，**不需要 `strace`**〕
⟹ 窗口內注入的座標進得了目錄、進不了 fold、然後被刪。**結構，不是罕見交錯。**

**⟹ 卡改寫成三軸，而軸 C 同樣不需要裁定。**

### 驗收（獨立複驗）

全樹 ×3 **預設 jobs** `--release`：**225／2153／0**，三輪皆然（~2 分鐘）。
〔交付方以 `--jobs 1` 計數並記下滿載偶發紅；**該負載敏感性在驗收方機器上三輪未重現**。〕
conformance **162／162**；身分 `31745ef0…`／3 物件／`7038e250…`／`layout=5`；**探針檔一字未動**。
行為三輪一致：**座標存活 20／20**、`oo log` ＝ 3、**`os error` 0**、**`Nothing to commit` 0**。

### ⚠ 一件不由驗收方接受，交由用戶 ⟹ D61

交付以提交臨界區的 `flock` 關掉 R1（**只縮小 `clear` 不夠**：last-writer-wins 之下，
被覆寫的那個成功提交**已 fold 且已消費**的座標不在新 HEAD 也不在工作集
——**驗收方複驗該推理成立**）。

驗收方一度說「`flock` 在事實上回答了軸 B」，**說得太重，D61 已就地更正**。
準確的是：**它是第一次讓 §4.1.1 第 1 款成真**——該款要求晚到者算
$HEAD_{current} \sqcap Staged_{incoming}$，而 §4.1.2 早記過它為何是空的
（「從不重讀 HEAD」）。臨界區使晚到者確實在前一提交之後載入。

**⟹ 待做的是兩件誠實性編輯不是裁定**（機制名「樂觀」錯了／補「工作集已被消費」一款），
**而「歷史能不能合法分叉」另立 Q-041 掛分散式線**。

### ⚠ 射程失誤第六次，形狀是新的

工單 S4 寫的是**三條禁令**（不得加 compare-and-swap／不得動 HEAD 的形／不得改成多 tip），
**交付方三條全遵守**，並找到第四種機制達成驗收方想保留的結果。

> **禁止清單也是機制。**「不要做 X、Y、Z」與「要做 X」有同一個失效模式，
> 而該寫的不變式是「**不得決定那個未裁的問題**」。

**同時**：驗收方要求的 R1 **在它自己宣告的射程內無法滿足**——那是射程的問題，不是越界。

### 修補回合 1：一個未加守衛的平台限制，而驗證是編出來的

〔讀〕`main.rs` **本弧之前 Unix-only 碼為 0**，交付第一版新增 **6 處全無 `#[cfg(unix)]` 守衛**
（`libc::flock`）⟹ `oo` 不能在非 Unix 建置——**而 `value.rs:2621`／`:2629` 就在兩檔外
對同一類呼叫逐處加了守衛並優雅退化**。⟹ **不是本專案不管，是本弧沒跟上本專案自己的慣例。**

修補改用 **`std::fs::File::{try_lock, lock}`**（1.89 穩定；工具鏈 1.96.1）。
**⚠ 而這一格的驗證是決定性的，因為有對照組**〔量，`x86_64-pc-windows-gnu` 目標剛好已安裝〕：

| | `cargo check --target x86_64-pc-windows-gnu -p oo` |
| :-- | :-- |
| 修補前（`3536e4d`） | **7 個編譯錯誤**（`cannot find unix in os`／`flock`／`LOCK_EX`／`LOCK_NB`…） |
| 修補後 | **Finished，clean** |

**交付方在此另做對一件驗收方沒要求的判斷**：鎖取在**既有的 `.oo/format`** 上，
註解逐字說明**新開一個 `.oo/commit.lock` 就是一次 layout 變更**（並指名會抓到它的既有探針）。
〔驗收方複驗〕該檔**位元組與 mtime 皆未動**。

### 探針：兩處臂被改，一處是驗收方寫錯

*   **R3 的原臂無法轉綠**——它只在看到禁止句時 `armed += 1`，隨即斷言該句不存在
    ⟹ **正確的產品永遠 VOID**。**驗收方寫了一支不可能通過的探針**，交付方指認正確。
*   **R2 的臂被放寬成恆真**，理由成立（修好後可能 20／20 成功）**而替代品不成立**
    ——「跑了二十個重疊提交」不證明重疊發生過，VOID 守衛變成死碼。
    驗收方改回一個**修好之後仍然有效**的訊號：**落地 ≥ 2 或有任何一個回報 consumed**。

### 這一弧留下的可重用之物

*   **禁止清單也是機制**（見上）。
*   **「我到了那個狀態」要寫進斷言。** 本弧三支紅全是競速，
    每一支先斷言武裝再斷言性質；未武裝即 **VOID READING 並以失敗呈現**。
    **R1 的第一版就因此自曝**：輪詢 `put_root` 想贏計時窗，**12 輪一次都沒武裝**
    ——它沒有假裝綠。改法不是 poll 得更快，是**換形狀**（角色分開：10 個只 evolve、
    10 個只 commit），**四輪四中**。
*   **交叉編譯目標是最便宜的對照組。** 平台回歸的證明不必用推的。

---

## Q-002 — 一道不是你撞上的天花板（2026-09-09；**零修補回合**）

**基線** `v0.45.0` 標籤二進位。**裁定** D62（三種條件三個名字，取甲）＋
D63（離開碼由載體決定，取甲），皆用戶 2026-09-07。
**偵察** `nlang-tools/docs/a_ceiling_that_was_not_yours_recon.md`（驗收方自量，不外包）。

### 一句話

`oo eval '1 + 2'` 在 `ulimit -v 300000` 下回答 `_|_ (%cause: #stack_overflow)`、**rc=0**，
而 `TAG_REGISTRY` §2.7.3 逐字告訴操作者這個標籤的補救是「攤平結構⋯**調旋鈕沒有用**」。
**輸入是 `1 + 2`。而唯一有效的動作就是調一個旋鈕。**

### 開卡前重量推翻了卡面兩處

卡片證據取自 **`v0.18.0`**，27 個 minor 之前。〔量 2026-09-07〕

*   **「不是輸入可達」為錯，且往壞的方向錯**——假回報**根本不需要輸入**。
*   **「分開 spawn 失敗／join panic／真正資源界」的三分法在程式碼裡不成立**：
    真正的深度界由**兩道執行緒之外的閘**處理（文字巢深 256 在 spawn 之前、
    AST 高度 4096 在 join 之後），〔量〕兩道今天都答得對（400 層 ✅、4100 項 `+` ✅、
    20000 項無 SIGSEGV ✅）。
    ⟹ **`join()` 的 `Err` 只有一個居民＝panic；`spawn_scoped` 的 `Err` 只有一個
    居民＝宿主拒絕給執行緒。那一格裡沒有任何一個合法的 `#stack_overflow`。**
*   **「依賴：無」為錯**——需要一則裁定，故先開偵察（§8 開弧 3）。

### 量到的四個帶，三個錯（離開碼直接取，不經管線）

| `ulimit -v` | rc | 輸出 |
| :-- | :-- | :-- |
| ≥ 700 MB | 0 | `3` ✅ |
| **~150–650 MB** | **0** | **假的 `#stack_overflow`**（帶寬約 500 MB） |
| ~80–100 MB | 134 | `memory allocation of 7 bytes failed`（SIGABRT） |
| ≤ 60 MB | 101 | `main.rs:385` 裸 panic，**完全沒有 n/ 答案** |

**七個剖析面五個 rc=0**（`eval`／`evolve`／`run`／`fmt`／`fmt --write`），
`test` 是唯一 rc≠0。**兩格最貴**：**`oo evolve` rc=0 而演化沒有發生**；
**`oo lint` 有陽性對照**——同一個檔無限制 `diagnostics: 1`、壓力下 `diagnostics: 0` rc=0
⟹ **CI lint 閘會在一個沒讀進來的檔上變綠**。
**且同一個 `evolve` 入口的普通語法錯誤是 rc=1** ⟹ 柵欄是該入口唯一 rc=0 的剖析失敗。

〔內部 panic 的形狀，打補丁量，補丁已還原〕stdout `#stack_overflow`、**rc=0**、
stderr 逐字洩漏 `crates/parser/src/lib.rs:174:9`。

〔線上〕同一個 1 行 `#fetch`，只改**伺服器**上限：1200／700 MB → `#not_found` ✅；
**500／300 MB → `#rejected` / `#stack_overflow`** ❌。

### 出貨

*   **三個名字**：① `#stack_overflow`（不動）／② **`#host_resource_denied`**／
    ③ **`#internal_error`**。**不是 `BottomCause` 新變體** ⟹ 既有值位元組不動。
*   **`print_parser_stack_overflow` 與五處特例臂整批刪除**——三種條件改走與
    普通語法錯誤**同一條路**（診斷 ＋ `Err` ⟹ rc≠0，stdout 不印 `_|_`）。
    **修的是類別不是個案：把特例拿掉，而不是再加三個分支。**
*   `main.rs:385` 的 `.expect("spawn oo-main thread")` 改 `#host_resource_denied`，
    **不洩 `Os` Debug**。
*   `oo lint` 的 `PARSE-SKIP` 計入 summary 並使 `run_cli` 回 rc=1。
*   `oodp` 的 `%reason` 增兩個；**`%status` 集合不變**（仍 `#rejected`）。
*   **③ 的常設看守**住 `crates/parser`：`parser_thread_panic_is_not_a_depth_error`
    以會 panic 的 closure 呼叫 `with_parser_stack_using`。
    **驗收方到不了它，所以工單要求交付把它帶回來。**

### 驗收

**零修補回合。** 本弧探針 **10／10**（基線 4 綠 6 紅，五次逐次相同）；
`a_limit_you_cannot_catch` **15／15**；`limit_you_cannot_choose` **10／10**。
**全樹 ×3：`targets=226 passed=2164 failed=0`，三輪 `exit=0`。**
**符合性 162／162**，**0 新向量逐字記帳**（語料形式表達不了宿主資源壓力）。
**紅線全過**〔量，新舊二進位各造一個 `x: 0` 倉並逐字比對物件 CAID〕：
根 `31745ef0…`／標準根 `7038e250…`／物件 **3**／`layout=5`／`encoding=5`；
〔對照〕第三個物件兩邊不同 ＝ 帶時間戳的 commit，同一顆二進位造兩次同樣不同。
**跨版本**：真 `v0.44.0` 讀本版寫的倉，`status`／`log`／再提交皆 rc=0。
**交叉編譯** `x86_64-pc-windows-gnu` **0 error**。

### 這一弧留下的三件

**(a) 三支既有斷言在本弧之前就是錯的，本弧只是把它們照亮。**
`a_limit_you_cannot_catch_probe_test` 的 R2 與共用 helper 要求 `exit 0`，
而**該檔檔頭自己逐字寫著「these probes pin the PROPERTY, not either constant」**。
property 是「深輸入永遠不會殺掉行程」，`Some(0)` 從來不是它。
**交付逐條列出、一字未改**（工單紅線如此要求），**由驗收方裁定並修改**：
`Some(0)` → `code.is_some()`（死法是 signal）＋「做完了，或說出它撞到什麼」；
R2 另加 `assert_ne!(code, Some(0))` **比原本更強**；並**改名**
`a_clean_bottom` → `a_named_refusal`——**一支叫「是一顆乾淨的 ⊥」而斷言
「它刻意不是 ⊥」的測試，就是這整條線在治的那種不誠實回報。**
非空洞性已驗：原始缺陷（rc=134 ＋ `overflowed its stack`）與 `v0.45.0` 的形
（rc=0 ＋ `_|_`）都仍會被擋下。

**(b) `#stack_overflow` 的兩個載體，在本版之前於 stdout 與離開碼上完全不可分。**
求值期是工作集裡一顆真的 `_|_`（rc=0，**對的，本弧不動**）；剖析期什麼都沒進宇宙。
而 §0.2 的「**載體必須可判別（MUST）**」寫於 2026-08-09。
**唯一能分開它們的東西，就是那張表右欄早就寫著的離開碼。**

**(c) 驗收旁量一件新的，已入 Inbox**：`oo evolve nosuch.n` 逐字
`Error: No such file or directory (os error 2)`——**裸 errno**，
`SPEC_10` §2.2.1 禁止洩漏實作表示。〔對照 `v0.45.0`〕同形 ⟹ **不是本弧引入**。
離開碼是對的（rc=1），錯的是訊息。

### 規格收尾（驗收方）

*   **`TAG_REGISTRY` §0.2 新增「載體決定離開碼（MUST）」**（D63）——
    **不是新約束，是一條沒有被接上的線**；`SPEC_10` §2.2.2／§2.2.3／§4.1.3
    三處「離開碼非零」就地標為本款之例。
    〔論證性量測〕本款今天已被遵守 **9／12**，不合的三格違反的是 §0.2 早已寫下的禁令。
*   **`TAG_REGISTRY` §2.7.5 新設**（D62）——§2.7.3 那張「誰決定」的表**少了一列**
    （**宿主**），**而它底下還藏著第四種情況**（**沒有人**——實作壞了）。
    三條 MUST NOT ＋ 一句可重用的話：**原因標籤是一句關於補救的話，
    所以它可以字面為真而仍然是錯的答案。**
*   **`TAG_REGISTRY` §1.2 兩列新標籤**；§2.5 線上 3→**5**、§2.6 邊界 9→**11**、
    同名跨載體注記補兩句。**§2.7 標題就地更正**（「兩則命名裁定」→「命名裁定」；
    它自 2026-08-11 起已有四小節、今日五——**數字寫在標題裡而沒有跟著內容走，
    與本節所治者同族**）。
*   **`REAL_02` §3.2.3 新增 MUST NOT：收方不得以請求的性質命名自己的無能為力。**
    **這一款守的是上一款的理由**——上一款禁止沉默，逐字是為了讓「我被拒絕了」
    與「這台掛了」可以分辨；**一個冒名的具名回應不會被讀成沉默，它會被讀成前者**，
    於是那台掛了的機器不但沒被看見，還讓每個跟它說話的人以為是自己的請求有問題。
*   **收尾後複驗通則**〔量，14 格〕：值的載體 **4／4** rc=0、邊界的載體 **10／10** rc≠0
    ⟹ **由 9／12 變 14／14，無未兌現條款，故無須依 §8 收弧 2a 開 Inbox 列。**

---

## Q-042 — 引擎用自己的話回答（2026-09-12，零修補回合）

**裁定 ＝ D64（Q-018 取甲′）。** 偵察 `nlang-tools/docs/is_the_cli_a_promise_recon.md`；
工單 `docs/a_voice_that_is_not_its_own_handover.md`；
探針 `crates/oo/tests/a_voice_that_is_not_its_own_probe_test.rs`（基線 **5 綠 6 紅**，交付後 **11／11**）。

### 這一弧治的是什麼

**引擎對操作者說話時的誠實性**，三種失敗在同一條軸上：
說宿主的話（`Atom(Int(1), EffectTag(0), None)`、`os error 2`）、
什麼都不說（指令 9／29、旗標 19／37、位置引數 6／11 無說明）、
在不同地方用不同說法說同一件事（5 個共用旗標中 4 個不一致）。
...
*   全樹 ×3 `--release --no-fail-fast`：**227／2176／0** 兩輪；
    另一輪多一個非本弧失敗（見下 §「三弧了」）。
*   身分：根 `31745ef0…`／標準根 `7038e250…`／物件 **3**／`layout=5`／`encoding=5`；
    跨版本真 `v0.46.0` 讀交付寫的倉 `status`／`log`／再提交皆 rc=0。
*   **conformance 162／162**，**0 新向量**——逐字記帳：
    本弧的性質全在 CLI 介面與診斷文字上，語料表達不了它們。
*   說明總數 **20→29 ／ 18→37 ／ 5→11**（反向檢查：**沒有一處靠刪達成一致**）。

### 三件值得記住的

**(a) S1 做成了類別，而那是驗收方另外量出來的。**
工單只指名四個入口；量下去是 **三種錯誤類別 × 六個入口全部具名**
（ENOENT／EACCES／EISDIR × `run`／`evolve`／`fmt`／`test`／`lint`／目錄引數），
**其中 `lint` 與目錄引數工單與探針都沒有指名**。
成因是交付把「宿主錯誤 → 操作者看得懂的理由」做成一個**全函數**的對映
（`_ => "unreadable"` 收尾）——**與 `REAL_01` §1.3 第二條要求正準文字形是全函數，
是同一個形狀的答案**。

**(b) 交付主動報了一個探針漏洞，而且沒有利用它。**
`oo node serve --port` 在基線 help 裡有 clap 自動填的 `[default: 8080]`，
**驗收方的剖析會把它當成說明** ⟹ R4 本可對它空過。
交付在 §N.3 指名，並仍補了一句真正的對比說明。
〔複驗〕交付後「僅有 `[default:]` 而無說明」的旗標 **0 個**。

**(c) ⊥ 的訊息文字改了，而根的位址沒有動——這次是強形式。**
`bad: 1 & 2` 與 `bad: 1 & 3` 的根仍是交付前量到的同一顆 `cbb7ef81…`。
合上另一路量測（**全 conformance 語料 162 向量中 0 個釘住該訊息文字**，
這正是交付改寫訊息而 162／162 不掉的原因）⟹
**`_|_` 的 `message` 是今天唯一沒有任何東西檢查的欄位——位址不看它，語料也不看它。**
那是排序第 2 項與 O85 的共同主體。

### 規格收尾（驗收方）

*   **`REAL_01` §1.3 補兌現度**：五條**兌現 4／5**。第一條已兌現；
    **第三條（印出來的必須讀得回去）仍未兌現，而那一格的鍋在規格不在引擎**——
    本規範逐字使用 `_|_ (%cause: …)` 數十處，而 `SYNTAX_02` §2 只把 `_|_` 定為裸原子，
    **文法從未定義帶因形**。依 §8 收弧 2a／§2.4 已開 Inbox 列，家在 **O85**。
*   **`REAL_01` §1.4 補已兌現**，並明記本弧量到而條文說不到的那一格：
    **一個長旗標可以是三個不同的概念**，而 §1.4 只寫了「同一個概念必須同一份說明」，
    **沒有寫反面**。

### 掛帳（四列新 Inbox，皆驗收旁量且附對照組）

*   **儲存層仍以宿主的錯誤形回答**：`.oo/HEAD` 不可讀時 `oo log` → `os error 13`。
    **交付前後逐字相同 ⟹ 非回歸**；`.oo/` 自己的 IO 走 `crates/interpreter`，
    不在本弧收束的那個入口之下。**這就是工單那句「字面正確之後還有什麼會壞」的答案——
    由驗收量出來，而不是由射程預先寫出來。**
*   **`refuse_raw_os` 是對宿主字串的文字比對**（來自 Q-016b，本弧一字未動）。
    同一個問題現在有兩個答案：**認出宿主的字**（機制）與**從不產生那些字**（不變式）。
    後者出現之後，前者的存在理由變薄。
*   **⚠ 三弧了**：`r4_two_concurrent_discharges_both_survive` 的 **REACH** 在全樹平行負載下
    連續第三弧失手（v0.45.0／v0.46.0 的交付報告各記過一次，本次第 1 輪再現；隔離 8／8 過）。
    **探針的設計是對的**——碰不到爭議狀態時以 VOID READING 失敗而不是變綠。
    **問題在帳**：它連續三弧被當成 flake 略過，
    **於是 Q-040 的那條性質從來沒有在一次全樹平行執行裡被真的檢驗過。**
*   **`--target` 是三個概念一個拼法**。交付被工單指名回答，答「該改其中兩個的拼法」；
    **D64 明文禁止以改拼法收尾，故本弧未改**。改名是破壞性條目，
    且須與 F3（對象置於動作之前）一起做——**兩者都在等弧 D 第 2 項**。

---

## Q-043 — 位址名點（2026-09-12，零修補回合）

**裁定 ＝ D65**（`message` 拿掉／`%cause` 不動／`_|_` 印裸原子而成因移入註解層）。
偵察 `nlang-tools/docs/an_address_that_never_promised_those_bytes_recon.md`；
工單 `docs/the_address_names_the_point_handover.md`；
探針 `crates/oo/tests/the_address_names_the_point_probe_test.rs`（基線 **6 綠 5 紅**，交付後 **11／11**）。

### 這一弧治的是什麼

**四個不同的宇宙提交到同一個位址 `cbb7ef81…`，而其中三份位元組相異。**
把其中一份原封不動放進另一個倉的同一路徑，讀取端**以 rc=0 端出另一份位元組**
——**放進去的不是偽造品，是另一個宇宙合法產出的物件。**
`REAL_03` §6.7 兩條 MUST 逐字管這件事，而其判例 `span` 已於 v0.13.0 修掉
⟹ **這是一條已被裁決的 MUST NOT 的第二個實例。**

### 驗收數字（驗收方自量）

*   探針 **11／11**，無 VOID READING。
*   全樹 ×3：**228／2188／0**，**三輪皆淨**
    ——**連續三弧出現的 `r4_two_concurrent_discharges_both_survive` REACH flake 本次未再現**
    （該列仍留在 Inbox，未再現不等於已解決）。
*   身分：`31745ef0…`／**`cbb7ef81…` 未動**／`7038e250…`／物件 **3**／`layout=5`／`encoding=5`。
*   `.%cause` 四格不變：`(_)`→`_`／靜態環→`#static_cycle`／`(1 & 2)`→`#conflict`／發散→`#divergent`。
*   **S3 雙向**：新引擎讀真 `v0.47.0` 寫的帶 `message:` 的倉 `status`／`log` rc=0；
    舊引擎讀本版倉 `status`／`log` rc=0。
*   **conformance 162／162**，**0 新向量**——逐字記帳：語料以獨立一行寫 `%cause`，
    runner 對該行的比對是可選的，本弧的性質在顯示層與儲存層，語料表達不了。

### 三件值得記住的

**(a) 一支探針與它所實作的裁定打架，而那是驗收方的錯。**
R2 原寫成字面的「一個位址一份位元組」，而 `REAL_03` §6.9 第二款是對那一條的**明文例外**
——**且驗收方在同一份工單的 §1.1 裡才剛引用過它**。
拿掉 `message` 之後 `bad + 1` 仍與其餘三者共用位址而位元組相異，差的是 `#divergent`
——**那正是 D65 要它隨值旅行的纖維**。**交付一字未改探針，逐字在 §N.3 指出矛盾。**
驗收方依 Q-002 先例裁定並改寫為：**兩份共用一個位址的物件，其相異處必須全部落在有正式通道的欄位上**，
並加一條 REACH（抹除若沒有真的改變位元組則讀數作廢）。**非空洞性已驗**：改寫後在 `v0.47.0` 上仍紅。

**(b) 兩道驗收方量不出來的題，交付都答出了新的帳。**
**commit 的位址由 `format!("{:?}", CommitMeta)` 決定**——**這比 §6.7 的判例更上游**：
§6.7 治「物件裡有不入址的欄位」，而這裡是**入址的那個函數本身是宿主的除錯呈現**，
且該 `Debug` 是手寫且條件式的（三個欄位只在 `is_some()` 時進入字串）。
**`~%__nlang_top_cause` 的 `members`** 無通道、不入址、卻寫進物件 ⟹ **§6.7 的第三個實例**。
兩者皆依工單明文「只要答案，不要動手」，未動，已入 Inbox。

**(c) 修法的形狀是允許名單，不是刪一個名字。**
`write_bottom` 改為列名可寫欄位並在函式註解裡寫下每一個的理由，
配一支單元測試餵入填滿的 `BottomDetail`（含 `message`／`expected`／`found`）斷言那些字不出現。
**擋得住 `BottomDetail` 上的下一格，擋不住別種 wrapper 私自加欄**——交付自己這樣寫，記其誠實。

### 規格收尾（驗收方）

*   **`SYNTAX_02` §2 新增按語**：`_|_ (%cause: …)` 是**文件記法**，不是正準列印形。
    **⚠ 這一步取代了原訂的逐處改寫**：D65 初估「規格散文數十處」，
    〔量〕實為 **12 處／8 檔**，且逐一讀過之後**一處都不必改寫**
    （8 處在 `;;` 註解內、2 處虛擬碼、**2 處是有日期的歷史紀錄，歷史紀錄不改寫**）
    ⟹ **定義那個記法一次，而不是逐處改寫**——與 D64「寫規則不寫個案」同一個動作。
*   **`SPEC_11` §3.4 新增兩個法定成員**（`;; %cause: <tag>`，正式對應 `.%cause`；
    以及**人話診斷**，明記它**沒有**正式對應**因為它不具語義地位**）
    **與一條 MUST NOT：本層的任何成員不得被寫入內容定址物件**，理由窮舉（進址 or 違反 §6.7）。
*   **`REAL_03` §6.7 增列第二個判例**（`message`）與**可操作的判別法**：
    一個欄位得寫入 CAS 物件，**當且僅當**它入址、或於正式觀測通道上可讀。
*   **`REAL_05` §245 釐清**：`%cause` 行是那條通道的**投影**，不是值本體的一部分
    ⟹ 升為 MUST 時要求的是「成因說得出來」，不是「成因印在值裡」。
*   **`REAL_01` §1.3 兌現 5／5**，並記下註解層的固有前提：
    **`;;` 延伸到行尾，故「印出來的讀得回去」以行結構被保留為前提**——摺行會吞掉閉合括號。
    **這不是本次引入的**（`%effect` 尾註自 2026-07-20 起皆然），
    而驗收方在複驗時**自陷此坑一次並就地記名**。

---

## Q-044 — 引擎最後說的那句話（2026-09-12，一個修補回合）

**不需要裁定**——本弧每一條性質都已寫在規範裡。這是「引擎以宿主的話回答操作者」
這一族的**第三個也是最後一個**家族：Q-042 收**來源檔**，本弧收**引擎自己的 IO 與 panic 出口**。
工單 `docs/the_last_thing_it_says_handover.md`；探針
`crates/oo/tests/the_last_thing_it_says_probe_test.rs`（基線 **5 綠 4 紅**，最終 **9／9**）。

### 這一弧治的是什麼

`oo status | head -1` 曾給出 **rc=101** 與五行 panic，其中含**宿主原始碼位置**與**裸 errno**；
`.oo/HEAD` 不可讀時 `log`／`status` 交出 `Permission denied (os error 13)`；
而 `.oo/objects` 不可讀時的答句是 **`CAID not found in local store`——物件在那裡**。
三件分別違反 `REAL_01` §1.3 第一條（兩次）與 `REAL_03` §6.6 的「三種結果必須可分（MUST）」。

### 驗收數字（驗收方自量）

*   探針 **9／9**；**重掃 18 格注入**（工單 6 ＋ 驗收方另造 12），
    命中 `os error`／`panicked`／`/rustc/` 者 **0**。
*   SIGPIPE：`status`／`log` **rc=141**（128＋13）、**stderr 0 位元組**。
*   身分：`31745ef0…`／⊥ 根 `cbb7ef81…`／`7038e250…`／物件 3／`layout=5`／`encoding=5`。
*   **反向檢查**：正常路徑 `status`／`log` 與 `v0.48.0` **逐位元組相同**。
*   conformance **162／162**；跨版本雙向 rc=0；全樹 ×3 **229／2198／0** 兩輪。

### 四件值得記住的

**(a) 交付答出了驗收方答不出來的那題。** 工單第一道必答題逐字問「什麼決定了一個指令
會不會在關閉的讀者上 panic」——〔量〕`fmt` 寫 71,787 B 不 panic 而 `status` 印 143 B 會。
答案是**寫出路徑有沒有 unwrap**：`println!` 在 `EPIPE` 上 panic，
`fmt` 用 `let _ = writeln!(…)` 把同一個 `EPIPE` 丟掉。**不是輸出大小。**

**(b) S1 的修法是一個地方。** 行程啟動時把 `SIGPIPE` 由 rustc 的 `SIG_IGN` 改回 `SIG_DFL`
——**一處覆蓋全部 101 個輸出點**。這正是「101 個呼叫點 ⟹ 答案必須是一個地方」的兌現。

**(c) 一個修補回合，成因是一格分類錯誤。** 交付在 §N.4 逐字排除 `savepoint.rs`，
理由「那些不是『引擎自己的倉』」——**而 `.oo/savepoints/` 就在 `.oo/` 裡**。
**射程沒有寫錯**：S2 寫的是不變式，且其下就寫著「只修 `.oo/HEAD` 那兩格」這個反例。
**錯的是排除清單裡的一格。**

**(d) 修補把教訓帶到了沒有被要求的地方。** 它順手移除 `savepoint.rs` 的
`if !d.exists()` 前置檢查，改以 `ErrorKind::NotFound` 判定缺席
——**那正是 S3 的成因**：`exists()` 在權限被拒時回 false，於是「讀不到」被寫成「不在」。

### 規格收尾（驗收方）

*   **`REAL_01` §1.3 新增一條 MUST NOT：宿主的中止不是一個答案。**
    **與第一條分開，因為它治的是形式不是內容**——一個文字乾淨的 panic 仍然不是答案，
    它沒有被任何人選過。三個證人皆為量測（剖析器執行緒 panic 冒名為深度超限／
    `main.rs` 的 `.expect` rc=101／`EPIPE` 上的 `println!` panic）。
    **⚠ 本款不決定「該回答什麼」**：安靜結束與具名拒絕都滿足它，那是載體的問題。
*   **`REAL_03` §6.6 新增判例：一個「先問存不存在」的檢查會把不透明壓成不存在。**
    〔量〕`Path::exists()` 在 `EACCES` 時回 false。
    **這不是有人決定要壓平，是一個看起來無害的前置檢查替他決定的。**
    可操作形：**不得以「檔案是否可被統計／開啟」代替「錯誤類別」來判定四者中的哪一個發生了。**

### 掛帳

*   **`.oo` 整個不可讀時的答句帶內部函數名 `atomic_write`**（n/ 裡沒有這個拼法）。**nit，未修。**
*   **交付 §N.4 第 1 題末句未被重現**：稱 `fmt` 現在會被 SIGPIPE 殺掉，〔量〕仍為 rc=0。
*   **⚠ 並行 flake 的性質改判**（已回寫 Inbox）：〔量，各 10 輪**隔離**〕
    `r4_two_concurrent_discharges_both_survive` **交付樹 1／10、`v0.48.0` 2／10 失敗**
    ⟹ **它在隔離下也會失敗，不只是負載敏感**；
    此前記的「隔離 10 輪 8／10」與「隔離 8／8 過」**分母太小**。
    **⟹ 疑似 Q-040 的殘留缺陷，不是探針編排問題。**〔另量〕`SIG_DFL` **沒有**讓它變差。

## Q-045 — 這裡沒有東西，還是你看不到（2026-09-13，兩個修補回合）

**不需要裁定。** 工單 `docs/nothing_here_or_nothing_you_can_see_handover.md`；
偵察 `docs/a_store_it_had_not_finished_making_recon.md`；探針
`crates/oo/tests/nothing_here_or_nothing_you_can_see_probe_test.rs`（基線 **4 綠 1 紅**，最終 **7／7**）。

### 這一弧治的是什麼

`ObjectStore::init` 判別「這是不是別人既有的 store」，讀的是 `.oo/format`——
**而那正是它自己接著要寫的兩份宣告中的第一份**。每一格原子，**兩格合起來不原子**。
兩個並行的**唯讀**命令就足以讓其中一個被拒絕開啟一個正在為它建立的 store，
而 `oo` **沒有 `init` 子命令** ⟹ 操作者無法事先蓋好來迴避。
v0.43.0 引進，只差一行，**而那一行有正當理由**（先前引擎可能只 staged injections 而無
HEAD／CAS，當成新的會偷偷推進 layout、繞過遷移閘）⟹ 修法不得只是拿掉它。

### 驗收數字（驗收方自量）

*   探針 **7／7**（基線 4 綠 1 紅 ＋ 兩個修補回合各加一支紅）。
*   **I1 獨立複驗，每組帶 v0.49.0 對照**：ext4 32 方 ×30 回合 **舊 29／30 紅 → 新 0**；
    tmpfs 4 方 ×100 **舊 5／100 → 新 0**；tmpfs 32 方 ×50 **新 0**。
*   `stat`：`objects.format` 的 mtime 早於 `format` **120 µs** ⟹ 構造為真。
*   全樹 ×3 **230／2205／0** 三輪；conformance **162／162**；跨版本雙向 rc=0。
*   身分 `31745ef0…`／標準根 `7038e250…`／物件 3／`layout=5`／`encoding=5`。

### 六件值得記住的

**(a) 用戶的第一問把射程從個案改成類別。** 用戶讀完偵察問「這單是不是範圍超小」——
問得對，修補確實只有一行。量下去：全樹 34 個 `.exists()`，**兩格有病、兩格清白、
其餘 30 格沒有人問過**。射程因此改為「34 格一格不漏地被問過同一問」，**而那張表是本弧一半的產出**。

**(b) 用戶的第二問把 I1 從錯誤處理題改成構造題。** 用戶追問「不透明跟不存在的邊界在哪？
不然當時為何會被壓平？」〔量〕`Path::exists()` 對**六種**非平凡情形一律回 `false`，
而 syscall 給的是三分 ⟹ **壓平發生在標準函式庫**（全樹 `try_exists` 0 次）。
**而那是兩條邊界**：不透明 vs 不存在在 `Ok`／`Err` 之間、**就地救得回來**；
**「尚未」vs 不存在畫不出來**——競態中的行程拿到的 `Ok(false)` **是真話**，
那個差別是**另一個行程的意圖，而檔案系統不知道什麼叫承諾**。
⟹ **不要偵測尚未，要讓它不可觀測。** 交付照此構造：謂詞留在 `format`，把被觀察的那一份**最後落地**。

**(c) 探針必須跑兩個檔案系統，否則是空讀數。** 〔量〕窗口寬度是檔案系統的性質：
ext4 兩方並行 **0／500** 而 32 方並行 **30／30**；tmpfs 反過來（兩方約 6%，32 方僅 1–6／30）。
**這也是它六個版本沒被看見的原因**——`ScratchDir` 用 `std::env::temp_dir()`，
**在 ext4 上跑 CI 的人永遠不會看到。**

**(d) 兩個修補回合的共同形狀：問題都不在 `.exists()` 本身。**
**R-1** 第 11 列（`load_architects`）被判「安全」，而錯的在它**上面一行**——
`lib.rs:939` 的 `.unwrap_or_else(|_| empty)` 把函數辛苦建出來的具名錯誤丟掉
⟹ 「下一步會有別的東西抓到」的那個下一步，正好就是保證沒人抓得到的那一行。
**R-2** 則是**修補讓一條先前被吞掉的路徑第一次可達**，而它沒有座標。
⟹ **判準要問的是答案的去向，不是拼法。**

**(e) R-1 修的是一個 fail-open，而且本專案三年前判過同一個塌陷。**
〔量，三個對照組〕不可讀的 `.oo/architects.json` **不會關閉，它會靜默地不再是白名單**：
空名冊使 `bootstrap_exempt`（`universe.rs:1547`）與 `skip_membership`（`authority.rs:64`）皆真
⟹ 成員檢查整個跳過，refine rc=0。
而 `store_boundary_probe_test` 的註解逐字記著 **v0.2.41 的規則**
（「a refusal that renders as `#false` is indistinguishable from "the file is not there"」），
`discovery_trust` 弧更把契約寫死（「unreadable … is a **NAMED error — never silently empty**」）
並**點名 `architects.json` 是「R4–R7 forbid copying」的那個先例**。
**先例被禁止複製，本體從來沒有被修。**

**(f) 一族的清白也是產出，而且要量不要猜。** 第 34 列（`fs_guard`）交付寫的是
「**可能**弱化 symlink 邊界」——驗收方替它量了，因為那是 `.oo` 的安全邊界：
四種分量寫法（含**斷掉的符號連結**與**指向 `.oo` 的別名**）**全部得 `_|_ ;; %cause: #store_boundary`**，
HEAD 位元組未動，〔對照〕同一支程式寫普通檔得 `#true` 且檔案出現。
**記為「未重現且有對照」，不是「安全」。**

**⟹ 順帶結掉一筆纏了三弧的舊帳**：`r4_two_concurrent_discharges_both_survive`
在三輪全跑中都沒有再出現。〔量，400 回合，其中 393 回合兩顆都落地〕那 393 回合的提交
**全部 rc=1、零反例** ⟹ **Q-040 的修補在並行下站得住**；它 15／15 次紅全部倒在 REACH，
**從來沒有走到它要測的性質**。v0.49.0 收弧時「疑似 Q-040 殘留缺陷」的改判是錯的，已更正。

### 規格收尾（驗收方）

**本弧不改任何規範條文，而這是一個決定不是遺漏。**

I1 的一般形——「一個並行的觀察者不得看見一個讀起來像另一個答案的中間狀態」——
**正是 O86 (i) 待裁的那件事**（「尚未」該不該成為 `REAL_03` §6.6 第四個必須可分的結果）。
現在寫下來就等於替用戶裁了。

**⟹ 但本弧為候選乙留下了證據，已回寫 O86**：構造義務**是可滿足的，而且很便宜**
（一次落地順序的重排，`layout`／`encoding` 位元組不動、標準根不動、非紀元），
**而候選甲要求每一個 `.exists()` 呼叫點都回答一個檔案系統回答不了的問題**。

### 掛帳

*   **`DiscoveryConfig::load` 不可讀時洩裸 errno**（逐字 `Permission denied (os error 13)`）
    ——**Q-044 S2 的殘留**，`operator_io_reason` 已下移到 `crates/interpreter` 而該呼叫點沒用它。
    **已入 Inbox。最好的把手是它自己**：兩個載入器在同一個函數裡相隔兩行，**各做對一半**
    （一個具名檔案卻洩 errno，一個不洩 errno 卻不具名檔案）⟹ 修法不必發明，抄對方那一半。
*   **`~%Io./exists`（D66 已裁 ＝ `#blur` ＋ 登記過的 `%cause`）尚未實作**，須另開一張卡；
    〔量〕落地代價接近零（**所有 `.n` 檔中零個使用者**，唯一讀者是三處 Rust 斷言），
    但 `%cause` 拼法尚未登記（驗收方提議 `#unreadable`，依據是引擎已在用這個字）。
*   **`.oo/peers/directory` 的「不可讀 → 冷啟動」**：快取還是紀錄，**需裁定**，未開列為缺陷。
*   **第 34 列 `fs_guard`：未重現不等於不存在**，本次以四種分量寫法量過，記在上面 (f)。

## Q-046 — 這裡沒有東西，還是你看不到（2026-09-15，一個修補回合）

**D66 → D67 的落地。** 工單 `docs/the_world_would_not_say_handover.md`；探針
`crates/oo/tests/the_world_would_not_say_probe_test.rs`（基線 **3 綠 3 紅**，最終 **7／7**）。

### 這一弧治的是什麼

`~%Io./exists` 對 **ENOENT／EACCES／ENOTDIR／ELOOP 一律回 `#false`**，
`read_file` 一律回 `#none`——**四種宿主狀況，一個答案**。
**而模範就在同一個函數裡**：store 邊界那條路早已回 `_|_ ;; %cause: #store_boundary`
⟹ **塌陷的不是設計，是宿主錯誤那一條路。**
**這是這一族唯一把壓平後的結果當成值交給 n/ 程式的一格。**

### 驗收數字（驗收方自量）

*   探針 **7／7**；全樹 ×3 **231／2212／0**；conformance **162／162**；跨版本雙向 rc=0。
*   身分 `31745ef0…`／`7038e250…`／物件 3／`layout=5`／`encoding=5`。
*   最終答案表（逐格帶對照）：可讀 `#true`／ENOENT `#false`／**ENOTDIR `#false`**／
    **斷連結 `#false`**／EACCES **`_ ;; %cause: #unreadable`**／ELOOP **`_ ;; %cause: #path_cycle`**／
    store 邊界不變／**n/ 純引用環仍 `#static_cycle` 且仍印裸 `_`**。

### 五件值得記住的

**(a) 這一弧的裁定被用戶自己推翻過一次，而推翻的理由是他自己看出來的。**
D66 原取甲（`#blur`）。用戶先指示「要嚴格一點就是 `TAG_REGISTRY`」，驗收方照做後提出反對；
**用戶讀後指出真正的成因——「甲乙兩邊說的話都剛好有部份對又部份錯」**，
於是 D67 把問題拆成兩問（**有沒有值？「沒有」是誰的性質？**），
而 **⊤ 是欠定端裡什麼都不承諾的那一個，兩問都答對**。

**(b) 三個可檢查的理由，一個都不是美學。**〔量〕⊥ 是 meet 零元、⊤ 是單位元
⟹ 取 ⊥ 會讓一個操作者的取用問題**吃掉別人的資料**；**單調性** ⟹ `⊥ → #true`
不是合法收窄，**取 ⊥ 會把一個正常的未來寫成一次規格違反**；
`#blur` **宣稱一個可定址的快照**（§2.7.3），〔量〕每個 `#blur` 都真的帶 `%caid`，
而一次被拒絕的觀測沒有任何部分結果可定址。

**(c) ENOTDIR 與斷連結整格退出，而那是同一條判準推出來的。**
`a/b` 而 `a` 是普通檔 ⟹ **對任何人任何時候都不可能有東西在那**，`#false` 是**完整的答案**；
斷連結〔量〕`try_exists` 為 **`Ok(false)` 不是 `Err`** ⟹ 宿主已解析且找不到東西。
**⚠ 驗收方初版探針斷言它們必須與 ENOENT 不同——錯，已改為紅線 G3 釘住它們必須相同。**

**(d) 交付找到一個既有 bug，而它讓整個弧原本會白做。**
`decode_top_cause` 只讀 `~%` 軸而 `cause:` 寫在 data 軸 ⟹ **所有存下來的帶因 ⊤ reload 都變成
`#no_coordinate`**。另：⊤ 與帶因 ⊤ 同 CAID，而 unify memo 以 CAID 為鍵
⟹ `plain: _` 會把帶因 ⊤ 教成蒸發。**兩者都不是本弧引入的，是本弧第一次讓它們可達。**
〔量，驗收方〕停用 top-like 的 memo **不花錢**（400 欄 meet：含 ⊤ 2.95–3.41 s／
對照不含 ⊤ 3.03–4.72 s），**而理由是結構性的——⊤ 的 meet 本來就是常數時間，memo 對它本來就沒有價值。**

**(e) 一個修補回合，成因是驗收方的措辭。** 交付以 `#static_cycle` 標記檔案系統的符號連結環，
**而工單寫的是「`#static_cycle` 是 ELOOP 的同形先例」——「同形」不等於「同一個標籤」。**
用戶取乙（另立兄弟標籤）。**修法刪掉了一個特例分支**（借用停止後那個分支是死重）。

### 規格收尾（驗收方）

*   **`TAG_REGISTRY` §1.2 新增 `#unreadable` 與 `#path_cycle`，皆「原因·Top」。**
*   **§2.3 載體：Top 由 2 個改為 4 個**，並記下**本格的軸分成兩半**——
    前兩者是來歷（非錯誤、消費即蒸發），後兩者是**登記簿的頭兩個「原因·Top」**；
    **載體相同時，軸決定操作者要不要動手。**
*   **§2.7.6 新設命名裁定**，四條 MUST NOT ＋ 論證性量測（依 §2.7 體例）。
    第一條逐字：**以 `#false` 回報宿主的拒絕，等於把「未知」寫成「已知為假」——那是排中律，
    而 n/ 的觀測層沒有它。**

### 掛帳

*   **`Interrupted`（EINTR）與 `TimedOut` 併入 `#unreadable`。** EINTR 是**暫時的**
    （POSIX 的意思是「重試」），而 `#unreadable` 讀起來是永久的——**與 Q-045 的
    「『尚未』被壓成『沒有』」同形，只換一個 errno**；`#timeout` 且是已登記的標籤。
    **需裁定（重試？`#timeout`？一個表示暫時的成因？），已入 Inbox。**
*   **列印規則仍是個案不是類別**：今天以「是哪一個 cause」列表決定要不要印註解層，
    **而類別是「軸」**（原因要印、來歷不印）。**`TopCaused` 今天不攜帶軸** ⟹ 要改成類別得先讓軸可得。
*   **`write_file`／`append_file` 的同族塌陷 ⟹ O87**（動作不是觀測，待裁）。

## Q-047 — 一個你帶不走的型別（2026-09-16，零修補回合）

**D68 的落地，標準根線 28 個規格版以來第一次真的動它。**
工單 `docs/a_type_you_could_not_carry_handover.md`；探針
`crates/oo/tests/a_type_you_could_not_carry_probe_test.rs`（基線 **3 綠 2 紅**，最終 **5／5**）。

### 這一弧治的是什麼

`@list` 看起來是一個名字，行為像兩個東西，**而你拿到哪一個取決於你怎麼拼式子**。
〔量〕`@list.%fmap` 得標準根節點，而 `(@list).%fmap`、`t: @list` 之後的 `t.%fmap`、
`m.t.%fmap` **全部 `_`** ⟹ **門是純詞法的，`@name` 這個值永遠是約束 marker**。
**而那個詞法形遮蔽 marker 不是擴充它**：`@X.%name` 只對三個有節點的名字有答案，
其餘九個**全回 `_`**，**連 marker 明明有的欄位都拿不到**。

### 驗收數字（驗收方自量）

*   探針 **5／5**；全樹 ×3 **232／2217／0**；conformance **162／162**；跨版本 rc=0。
*   身分 `31745ef0…`／`7038e250…`／物件 3／`layout=5`／`encoding=5`。
*   **I1 複驗比探針寬**：**12 名字 × 4 欄位 ＝ 48 格，不一致 0**（探針只測 2 欄），
    另造 `%id`／`%morphism`／`%nosuch` 三欄亦逐格相同。
*   **I2 複驗比探針深**：`m.a.b.%fmap`（兩層 combo）與 **`l.0.%fmap`（list 的元素）** 皆得節點。

### 四件值得記住的

**(a) 這一題是用戶用兩個提問拆開的，而兩次都推翻了驗收方寫進檔案的東西。**
(1)「`( @list )` 是不是一元組」⟹〔量〕`(1)` 與 `1` 連 CAID 都相同、括號對一般值透明
⟹ **門是純詞法的**，驗收方據此撤回「甲 必須解釋 `=` 為什麼說兩個不同 CAID 相等」
——**`=` 從來沒有看到兩個值**。
(2)「`%super` 會不會才是怪的那個，幫我確認 `SPEC_05` §3.2」⟹ 那一節逐字寫著 `%super` 是
**衍生反映欄**，〔量〕標準根裡 **0 次** ⟹ **驗收方先前判「乙 ＝ 紀元」的前提是錯的**。

**(b) 而它終究是一次紀元，只是在另一個地方。** 〔量，驗收方另造〕`v: @list` 提交後的根
`5debf7de…` → `b8a028d5…` ⟹ **凡存過型別值的宇宙，其根位址改變**。
**這是 D68 的必然**（`@list` 從 marker 變節點），**而工單的紅線沒有說到它**——
紅線抬頭寫「非紀元」，內容卻是一張 digest 清單；**清單全守住了，抬頭沒有**。
⟹ **「射程要寫不變式不是機制」的又一次付款，這次在紅線上。**
**射程已量且很窄**：conformance 語料、引擎測試、`examples/` 三處皆 **0 檔**。

**(c) 交付答了驗收方沒量的那一格，而且沒有把門開太大。**
`is_type_value_combo`（閉合 ＋ `%kind: #type` ＋ `%name`）使標準根節點也算型別值；
**而 G2 第三格仍綠**——手造的開 combo `{ %kind: #type, %name: "int" }` 仍拿不到 `%super`。
E4 的 `@int` 短路亦保留（`@int: { hacked: 1 }` 之後 `@int & 10` 仍為 `10`）。

**(d) `@list` 的 CAID 是既有的 `c596dc99…`，不是第三個位址。** 改變的是**拼法指到哪一個已存在的值**。

### 規格收尾（驗收方）

*   **`SPEC_05` §3.2 新增一款 MUST：型別名字指稱單一值。** `@X.f` 必須是普通取欄位；
    **不得**以按名字查標準根的路徑取代它（那條路徑遮蔽約束側）。
    並新增「型別是帶得走的值（MUST）」。附論證性量測與紀元範圍。
*   **`SPEC_09` §2.5 的擱置令解除一半。** 該段逐字擱置的「型別是名義開放世界還是封閉清單」
    已裁為**前者** ⟹ **兩表的名單自此可以動**。
    **但 CAID 佔位符仍不得填**，理由與名單無關——見掛帳第一項。

### 掛帳

*   **⚠ `REAL_03` §6.2 的值編碼表缺五個標記**（`0x17` Thunk／`0x18` Range／`0xFD` `#blur`／
    `0xFE` `_|_`／`0xFF` `_`），**而標準根自己就需要 `0xFF`**
    （`@option` 的 `%some: { %val: _ }` 與 `@result` 的兩處）
    ⟹ **今天公佈任何 CAID，第二個實作也算不出來。**
    **這才是 `REAL_03` §6.8.2 第一條 MUST 今天不可能兌現的真正原因，而原本四題沒有一題問到它。**
    **⟹ 它是 D3′ 的前置，且可能是這條線上今天唯一另一個能動的。**
*   **D3′**（manifest 以位元組為規範還是以值為規範）與 **D4**（runner 契約，未重量）仍未裁。
*   `@zzz` 的開放性是 D68 的**推論**而非另裁；本弧維持開放，交付亦未反對。

---

## Q-048 — 兩個值，一個位址（2026-09-17，一個修補回合）

**D69 的落地。** 工單 `docs/two_values_one_address_handover.md`；探針
`crates/oo/tests/two_values_one_address_probe_test.rs`（基線 **3 綠 3 紅** → R-1 後 **6 綠 1 紅** → 最終 **7／7**）。

### 這一弧治的是什麼

`bn_serial` 在**雜湊之前**改動內容，而值保有原樣 ⟹ 語言分得開的兩個值共用一個位址。
兩個成員：字串臂的 `s.trim()`（自 BN/ 首次實作 2026-05-23 起，該提交的訊息列了四件事，
**沒有一件是這個**），與整數臂溢位時 `encode_signed_leb128(0, buf)`。

**傷害不在儲存層，在合一。** 兩值 CAID 相同 ⟹ 合一**以 digest 相等為由早退**：
`("a" & " a ")` 得 `"a"`、`(0 & 999…)` 得 `0` ⟹ **一次 evolve 悄悄把使用者的值吸收成了另一個值**，
而宇宙裡沒有任何地方記得曾經有兩個。

### 驗收數字（驗收方自量）

*   探針 **7／7**；全樹 ×3 **233／2224／0**（逐 target 聚合，`^error` 0，cargo exit 0）；conformance **162／162**。
*   **先證明量測碰到了目標**：探針 target 逐字 `7 passed; 0 failed`，
    其中 `r4_a_big_integer_is_not_zero ... ok`（R-1 交付前同一顆是紅）。
*   身分紅線：標準根 `7038e250…`／物件 3／`layout=5`／`encoding=5`，**皆未動**。
*   **紀元範圍＝且僅＝違反者**〔跨版本對照，真 `v0.51.0-verify` 二進位〕：
    `0`／`1`／`2`／`i64::MAX`／`i64::MIN`／`"a"`／`1.5`／`#true` **逐字未動**；
    `" a "` `cdf2d551…`→`87b570af…`；`2^63`／`999…`／`888…` **修前三者皆為 `08da7c45…`（＝`0`）**，修後各得其所。
    ⟹ **v0.51.0 那一格是四個值一個位址，不是兩個。**

### 五件值得記住的

**(a) 掃描結論：是一族，連續第四次**（Q-042／Q-044／Q-045 皆然）。射程寫的是不變式
（I1 不同的值不得共用位址／I2 雜湊前不得正規化），**所以 R-1 有地方可掛**——
交付方把第二個成員標為「不是本弧」而沒給理由，而不變式說它就是本弧。

**(b) 用戶的一問拆開了本題。** 「感覺 `trim` 應該是為了讓他變成 compact 型對吧？」
⟹ 那一臂是 `Str` 與 `MultilineStr` 共用的，trim 八成是為多行排版寫的，
**而它三件都沒做對**（值沒被正規化／漏到單行上／又多造一類碰撞）。
接著的第二問給出決定性的區分：三引號跳脫的還原是**剖析時**的事（決定文字指稱哪個值），
`trim` 是**雜湊時**的事（決定一個值住哪個位址）——**想要前者不替後者背書**。
驗收方先前提的「丙」正是把這兩件混在一起，已撤回。

**(c) 交付方指正了驗收方的探針，而它是對的。** R1 原本期待「同一倉先 `v: "a"` 再提交 `v: " a "` 成功」
——**那是單調律禁止的**；它之所以曾綠，正因為那個 `trim`。
**驗收方把缺陷自己的行為寫成了基線。** 已換成紅線 G4（`("a" & " a ")` 不得吸收），含對照組。

**(d) 規格結案補的是第四個方向。** §6.6 位元組→位址、§6.7 位址→位元組、§6.9 位址→值，
**而沒有一條寫「不同的值不得共用同一個位址」** ⟹ 引擎違反的正好是沒寫的那個方向。
`REAL_03` §6.7 新增第二條 MUST NOT，可操作形明文含**把值收窄進宿主型別**（`as i64` 一族）。

**(e) 而那一款一寫下來，就抓到規格自己的違反。** 見掛帳第一項——**這一次鍋在規格，不在引擎**。

### 規格收尾（驗收方）

*   **`REAL_03` §6.7 新增規範條款「值不得共用位址（MUST NOT）」**：凡 `=` 判 `#false`
    或 `%id` 相異者，CAS 位址必須相異；雜湊前不得做任何正規化，含收窄進宿主型別。
    附兩成員判例表、傷害在值層的說明，以及**「只移動違反者的位址不是紀元」**。
*   **`REAL_03` §6.2 加註**：`0x12 Int64 ／ [i64_LEB128]` 是**歷史拼法**，以 §6.1 的變長 LEB128 為準；
    並就地記下 `0x13 Float` **不是拼法問題而是真衝突**。

### 掛帳

*   **⚠⚠ `REAL_03` §6.1 的浮點數定點表示與本版新增的 §6.7 第二條直接相衝，而兩條都是 Core Requirement。**
    〔量，`v0.51.0` 與修補後**逐字相同** ⟹ 非本弧造成〕
    `1e29`≡`2e29`（同址 `de5c0158…`）、`0.0`≡`1e-20`≡`2e-20`（同址 `8296afe9…`，凡 `|小數| < 2^-64` 歸零），
    三對 `=` 皆 `#false`，而 `(0.0 & 1e-20)` 得 `0`。**小數部那支比大整數那一格寬得多。**
    **⟹ 引擎逐字照做了 §6.1，這一格不是引擎的鍋**（判別法：去規格 grep 那個概念——
    `trim` 零命中，定點數命中規範條款）⟹ 開 **O88**，並以 `interrupt-candidate` 進 Inbox。
*   **O88 的框當天就被用戶放大**：原寫「要不要換一個編碼」，
    而三個候選全都預設值層仍是 Rust `f64`——**那正是沒被問的那一格**。
    正名為**「n/ 的浮點數是什麼」**，編碼是下游。新候選 **丁 Posit**（位元樣式與值雙射，
    無 `±0.0`／無 NaN 動物園，且二補數序＝數值序）／**戊 Stern–Brocot 路徑**
    （**位址前綴本身帶精煉序**，使 §6.7 的雙射與 `#refine` 的單調成為同一件事）／
    **己 Unum Type I**（**ubit** 把不精確放進值，與 `#blur` 同構）。
    ⟹ **射程連 `SPEC_05` 型別面與 `SPEC_09` `~%Math` 一起**，不再只是 `REAL_03`。
    〔用戶口述來歷〕最早的想法是「一個格論出發的語言，會不會有更適合的浮點數編碼」，
    當時專注在 CAID 上而擱置。
*   `REAL_03` §6.2 仍缺五個標記（`0x17`／`0x18`／`0xFD`／`0xFE`／`0xFF`），**Q-047 那筆掛帳原樣仍在**，
    且本版又替它加了一列（`0x12` 的名字）。**D3′ 的前置未動。**
*   **`MultilineStr` 是二等公民**（`concat`／`write_file` 對多行回 `_|_`／不接受）與
    **引擎是否在剖析時還原三引號跳脫**（驗收方兩次量測皆未碰到目標，逐次記下）仍在 Inbox。

---

## Q-049 — 一個標記，八種意思（2026-09-18，一個修補回合）

**它從一張補文件的卡長成兩件事。** 偵察 `docs/one_tag_eight_meanings_recon.md`；
工單／驗收 `…_handover.md`；探針 `crates/oo/tests/one_tag_eight_meanings_probe_test.rs`
（基線 **4 綠 4 紅** → 主體後 **8 綠** → R-1 加 **2 紅** → 最終 **10／10**）。

### 這一弧治的是什麼

Ready 首位原是「值編碼表寫完」（`REAL_03` §6.2 缺五個標記）。
**開卡前的偵察量出它不是純規格弧**：`serialize_atom` 八條臂共用 `TAG_ATOM`（`0x10`）
並各寫一個裸字串（`Str`／`MultilineStr`／`Code`／`PathLit`／`Unit`／`Regex`／`Uri`／`Time`／`Bytes`）
⟹ **文字相同者共用位址，而語言分得開它們**。

〔量，`v0.52.0`，對照組 `"x"` ≠ `"y"` 活〕
`"ab"`／`r"ab"`／`p"ab"`／`u"ab"`／`t"ab"` **五個值一個位址** `2f5992b8…`；
`"()"`／`()` 同址 `5f6b36a7…`；`"6162"`／`b"ab"` 同址 `4414b39b…`。
`=` 判每對 `#false`，而 meet 逐對**靜默吸收** ⟹ **D69 的第三族**。

### 驗收數字（驗收方自量）

*   探針 **10／10**；全樹 ×3 **234／2234／0**（逐 target 聚合，`^error` 0，exit 0）；
    conformance `scripts/run-conformance.py` **162／162**（驗收方自跑）。
*   身分紅線：標準根 `7038e250…`／`x: 0` 根 `31745ef0…`／`layout=5`／`encoding=5`，**皆未動**。
*   **紀元只碰違反者**：`"a"` `cdf2d551…`／`0` `08da7c45…`／`1.5` `cb3bcb50…`／`#true` `1bc573f2…` 逐字未動。
*   **I3 round-trip 三樣本**：`a⏎b` `2024d616…`／`hello` `d709d6e9…`／`a"b` `384e8314…`，
    印出再讀回皆不變（**裁定前第一個會變成 `ecb0980d…`**）。
*   紀元範圍（驗收方獨立重數，判準是字面邊界不是裸 grep）：
    conformance **162 檔**中 `r"`／`p"`／`u"`／`t"`／`b"`／`"""` **皆 0**、`()` **2 次／1 檔**；
    引擎測試 `()` **1 次／1 檔**；`examples/` `r"`／`p"` 各 1；標準根 **全 0**。

### 六件值得記住的

**(a) 補表與修碰撞是同一件事。** 表不能誠實地寫「`0x10` ＝ Atom」——`0x10` 是九件事；
每一種的新標記位元組**就是表上要新增的列**。**這是「這張卡是不是純規格弧」被量出來的一次。**

**(b) 射程寫成不變式，而 I2 刻意比 I1 強。** I1（分得開的兩個值不得共用位址）之外加 I2
（一個值的**種類**必須進入它的位址）——只滿足 I1 的修法（四個前綴字面共用一個新標記）
會讓探針 R2 的第一個迴圈綠而**第二個紅**。**兩個迴圈都要綠，而交付一次過。**

**(c) `Bytes` 曾把值改寫成 hex 文字，那不是編碼選擇，是同一族的第三種面貌。**
修法改寫原始位元組加長度前綴。**`Unit` 同理**：它曾寫字串 `"()"`，現在無酬載。

**(d) R-1 不是因為交付做錯，是因為一個從未被裁過的答案要被鑄進位址。**
`0x1F` 修得對（兩形 `=` `#false` 而修補前同址）；而 D70 裁定「三引號是書寫法不是型別」。
**乙不是比較貴，是不可能**——`SYNTAX_02` §4.11＋§4.12 正是規格自己禁止印出端做那個區分。

**(e) 而 §4.12 是 2026-08-20 就存在的 MUST。** 〔在更正一處引用錯誤時才看見〕
「輸出必須可被讀回，**且必須讀回同一個值**」⟹ **D70 治的不是規格的缺口，是既有 MUST 的違反**。
這比原本的說法強，**而它一直寫在那裡**。

**(f) 落地的形狀：書寫法留在語法樹，值裡沒有它。** 求值與 `store_codec` 把 `MultilineStr` 收成 `Str`，
剖析不動 ⟹ `fmt` 保得住你寫的形，而身分看不見它。
〔交付報的、工單沒問的〕它試作過「剖析即 `Str`」，`fmt` 會印出讀不回去的 `"a"b"` ⟹
**那條路是把違反從值層搬到源碼層，不是消滅它。**

### 規格收尾（驗收方）

*   **`REAL_03` §6.2 值編碼類型全表重寫**：8 列 → **24 列**（含 Union 的「無標記」列），
    新增 MUST **「一個標記位元組恰對應一種值」**並附 `0x10` 九合一判例。
    `0x1F` 標為**死號**（D70，不得重用、不得發出），`0x10` 註明兩種書寫法同走此列。
*   **`SYNTAX_02` §4.5** 新增 D70；**§4.12** 新增**判例二**（違反的是第二個「必須」），
    並寫出 §4.11＋§4.12 如何把修法逼成唯一一種。
*   **引用錯誤更正 19 處**：`SYNTAX_02` 的 §110／§116／§129 是**行號**不是條款號。

### 掛帳

*   **⚠ `Code` 仍佔 `0x10`，酬載是 `format!("{:?}", Expr)` ＝ Rust 專屬格式。**
    本表**無法為它寫出規範性的酬載** ⟹ **§6.8.2 第一條 MUST 的阻塞從「補表」換成了「`Code` 的身分」**。
    **⚠ 2026-09-18 稍晚就地更正（量測推翻，`v0.53.0` 已切、已推）**：上句「`Code` 是 §6.8.2 的阻塞」**是錯的**。 〔量，`v0.53.0` 標籤建置〕**標準根物件 16,029 B 裡 `~%__nlang_code`／`~%__nlang_expr`／`ExprKind` 各 0 次**—— 它的 **255 個態射全是 `%builtin: "名字"`（一個字串）**，不是 Code ⟹ **`Code` 不進標準根，因此不堵 §6.8.2**。驗收方由「`0x10` 之下還有 `Code`」直接推到「所以標準根算不出來」， **中間那一步（標準根含不含 Code）從來沒有量**。 **⟹ 真正的那一格在別處而且更利**：〔量〕帶 body 的態射**會**進倉，存成 `%code: { ~%__nlang_code: #true ~%__nlang_expr: <正準 n/ 源碼> }` ⟹ **耐久位元組存的是 `to_nlang`，而位址是 `format!("{:?}", Expr)` 算的——同一個值，兩種呈現**。 ⟹ **第二個實作拿著那些位元組重算不出位址**，違反的是 **§6.6（讀路徑位址驗證）**，不是 §6.8.2。 **而 `Debug` 是 `#[derive]` 的**⟹ 那個格式 Rust 明文不保證跨版本穩定，**連同一個實作換編譯器都沒有保證**。
    今天表層碰不到它（〔量〕`~%Code./quote` → `#missing_key`、`'(1)` parse error），
    **但「碰不到」不是「不存在」**。⟹ **D3′ 的前置換了內容，那是一張新卡的射程。**
*   **Union 沒有標記、沒有長度前綴**：〔量，用不會對 union 分配的入口（包一層 Combo）〕
    今天**量不出碰撞**（成員編碼自定界、單元素 union 正規化時塌掉）；
    **但框界由相鄰位元組決定而非由編碼宣告**。已寫成 §6.2 的自陳缺口。
*   **O88（n/ 的浮點數是什麼）未裁**：`0x13`／`0x14` 的定點表示有損，與 §6.7 第二條相衝，兩條都是 Core Requirement。
*   **`fmt` 與 `--observe` 對同一個值給兩種書寫法**：D70 之後**兩者都讀得回同一個值**，
    故不再違反 §4.12；「哪一個才是正準的書寫法」是另一列 Inbox，**且可能根本不是缺陷**。
*   **`0x03` List／`0x04` Tuple／`0x15` Bool 今天不發出**，表上標為保留。

---

## Q-050 — 一個只有一個編譯器算得出來的身分（2026-09-19，零修補回合）

**D71 的落地。** 偵察 `docs/an_identity_only_one_compiler_can_compute_recon.md`；
工單／驗收 `…_handover.md`；探針 `crates/oo/tests/an_identity_only_one_compiler_can_compute_probe_test.rs`
（**基線 4 綠 0 紅**，最終 4／4——見下方 (a)）。

### 這一弧治的是什麼

〔讀 `bn_serial.rs:87`〕`Value::Code` 的身分是 `format!("{:?}", expr.without_spans())`，
而 `Expr` 是 `#[derive(Debug, …)]` ⟹ **位址取決於一個 derive 出來的 Rust 除錯格式**，
**Rust 明文不保證該格式跨版本穩定** ⟹ 不只第二個實作算不出來，**同一份程式碼換編譯器就沒有保證**。
而〔量〕耐久位元組存的是 `to_nlang`（正準 n/ 源碼）
⟹ **同一個值，兩種呈現** ⟹ 第二個實作拿著那些位元組重算不出位址 ⟹ 違反 **§6.6**。

### 驗收數字（驗收方自量）

*   探針 **4／4**；全樹 ×3 **235／2238／0**（run1 有一紅，見 (d)）；conformance **162／162**（rc=0，自跑）。
*   **紀元＝且僅＝含 `Code` 的宇宙**〔兩個真二進位各自算〕帶 morphism 的根 `e59ea11d…` → `43f838bf…`；
    `x: 0` 根 `31745ef0…`、標準根 `7038e250…`、`"a"`／`0`／`1.5`／`#true` **逐字未動**。
*   抽驗交付的修後位址表**五格全中**（`abffa9b1…`／`8a2122c0…`／`e65435df…`／`7c4b7d97…`／`1b25ffb0…`）。

### 六件值得記住的

**(a) 本弧的探針基線是全綠，而那是刻意的，且工單把代價寫在最前面。**
核心缺陷是跨實作／跨編譯器可重算性，**一個二進位無法表現它**
〔量〕`Expr { kind, span }` 兩欄、`without_spans()` 清掉 span ⟹ Debug 純結構、無隱藏欄位
⟹「存進去再讀回來位址會變」**量不出來**。
⟹ 四支全是紅線，而**「四支全綠不構成修好了任何東西的證據」**寫在探針檔頭與工單 §4 兩處。
**補償要求四條代替紅探針**，其中第一條（**完整可檢查的編碼表**）**就是交付物本身**。
**⟹ 這是本專案第一張「交付物是一張規範表，程式碼是它的實作」的卡。**

**(b) 表是窮舉的。** 〔量〕`ExprKind` **36 個變體對 36 個標記**（`0x40`–`0x63`，剛好 36 格），
含 Field 鍵四形、Relation、`Prefix` 七值、`Unary` 兩值，**沒有一列寫「同 Debug」**。

**(c) 交付自己找到 I1 的另一半。** `value.rs` 的 `hash_recursive_with_salt` 的 Code 臂
**也是 Debug 算的**（那是 `%id`，不是倉 CAID）⟹ **不改的話 I1 只修了一半**。交付指名並修了。

**(d) 一列掛了一個月的 Inbox，驗收方第一次獨立重現。**
run1 唯一的紅是 `pin_concurrent_first_mint_yields_one_key`，而它對應的 Inbox 列
**逐字寫著「驗收方四次全跑一次都沒有重現」** ⟹ **本次頻率 1／3**。
**非本弧造成**：同一二進位 1 紅 2 綠（非決定性）／本弧只動雜湊而該支測的是並行寫半截 PKCS#8／
〔量〕修補前隔離跑 **6／6 綠**。⟹ 該列已更新，**判別 (a) 完成；(b) 仍未答，而那才是升級的判準。**

**(e) D71 在 `Code` 裡比工單問的更深一格。**
〔量〕`x -> { a: 1, b: 2 }` `b4f72249…` ≠ `x -> { b: 2, a: 1 }` `48fbff37…`
⟹ **`Code` 裡的 combo 欄序是語法**；而〔對照〕`x -> 1` 與 `x -> (1)` **同址**（括號不是節點）。
**規格已寫明它與 §6.7 第四條為何不衝突**：在值裡鍵序排序，在 `Code` 裡源碼序**就是**「由內容決定」。

**(f) 交付指正了驗收方的射程理由，而它是對的。**
工單寫「〔引擎註解，L2-65 **已量**〕`to_nlang` 約 10³ 層爆棧 ⟹ 改用它會在語料現成的向量上爆掉」
——**後半句是假的，而「已量」兩字是驗收方把一句程式註解當成量測抄上去的**。
〔驗收方自量，離開碼未經管線〕**3999 層（＝L2-65 深度）三條路都 rc=0**
（`to_nlang`／`Debug`／`oo fmt`）；而 **20,000 與 80,000 層時剖析器先以具名邊界擋下**
（`Parse Error … #stack_overflow`，rc=1，**非宿主崩潰**）
⟹ **在今天經由剖析器可達的深度區間裡，`to_nlang` 從來輪不到爆。**
**I3 本身仍成立，理由已換成**：身分編碼不得從一個 pretty-printer 繼承深度上限，
而**剖析器的界不是對序列化器的保證**（樹也能從倉讀回或由程式建起）。
**⟹ 這一則的重量**：驗收方寫的射程理由是假的，**而交付照著它做出了正確的東西**
——顯式堆疊無論理由對錯都該做，但那句話本來可能把交付推去解一個不存在的問題。

### 規格收尾（驗收方）

*   **`REAL_03` §6.2**：值表新增 **`0x20` Code**；`0x10` 標明**不再承 Code**；
    `0x21` 與 `0x40`–`0x63` 標明**不是值標記**。原「`Code` 沒有規範性酬載」的按語改寫為已有，
    並附**完整的 Expr 節點表（36 列）**、Field 鍵與 Relation 的子編碼。
*   新增兩段：**§6.7 第四條與 `Code` 欄序為何不衝突**；**`Thunk`（`0x17`）是同族的另一半**。
*   留下驗收方 (f) 那則錯的原句與更正。

### 掛帳

*   **⚠ `Thunk`（`0x17`）是同族唯一剩下的一格，而它不需裁定。**
    酬載第一項仍是 `expr.to_nlang(0)` ⟹ 第二個實作若沒有一份**被規範的 pretty-printer**，
    一樣重算不出 Thunk 位址。**修法有現成模板**（本弧的 Expr 節點表可直接複用）
    ⟹ **一張便宜的卡**；代價是 `0x17` 的既有位址移動，紀元範圍待量。
*   **O88 未裁**：`Code` 內的 `Float`／`Complex` 原子沿用值表的定點編碼，**有損**，同一個問題。
*   **α-等價**（`x -> add(x,x)` 對 `y -> add(y,y)` 仍異址）未裁，Inbox 另列。
*   **`%code` 印得出來但用名字取不到**（`f.%code` → `_` 而 `f.%rules` 取得到）⟹ 與 D68 同形，Inbox 另列。

---

## Q-051 — 一把還沒寫完的金鑰（2026-09-20，零修補回合）

**五版以來第一個沒有破壞性條目的版本。** 偵察 `docs/a_key_that_was_not_finished_being_written_recon.md`；
工單／驗收 `…_handover.md`；探針 `crates/oo/tests/a_key_that_was_not_finished_being_written_probe_test.rs`
（基線 **3 綠 1 紅** → 最終 **4／4**）。

### 這一弧治的是什麼

〔讀 `value.rs`〕`create_new` 把身分檔的**名字**原子地佔下，**私鑰位元組在那之後才寫**
⟹ 有一個「檔案已在、內容還沒到」的視窗。過去的驗收修補為此加了 `load_after_race`
（100 × 1ms 有界重試），**但只掛在輸掉 `create_new` 那一條分支**；
而 `load_or_mint` 的第一行是 `if path.exists() { return Self::load(path); }`——**一次，沒有重試**。
...
*   探針 **4／4**（R1 翻綠）；全樹 ×3 **236／2242／0**；conformance **162／162**（rc=0，自跑）。
*   **那支先前 1／3 紅的別弧探針**（`pin_concurrent_first_mint_yields_one_key`）：三輪皆 `ok`。
*   **代價**：好金鑰 **3 ms 未變**；**永久無效的金鑰 ~3 ms → 116 ms**（耗盡重試預算才拒絕），
    而那是**持續狀態**。可接受，**但那是甲的代價**。

### 五件值得記住的

**(a) 判別 (b) 先答，因為它決定嚴重度，而答案使那一列降級。**
〔量，控制組活的——好金鑰時 `refine --sign` 的拒絕理由是**單調性**（表示它過了簽章那一步），
半截金鑰時是**簽不出來**〕`identity` 具名拒絕且位元組不動／`refine --sign` 在**簽章那一步**拒絕
（`sign_refine` 自己重驗 `from_pkcs8` ⟹ **壞位元組到簽章之間沒有路**）／
`commit` rc=0 **而那是對的**（commit 根本不簽）⟹ **不是假裁決、不是資料遺失**，
`interrupt-candidate` 撤下。**⟹ 先問嚴重度再開卡，省掉了一次錯誤的升級。**

**(b) 驗收方第一次的「機制確認」重現的是錯誤訊息，不是缺陷。**
把金鑰截成 0 B、看到逐字相同的錯誤就記下成因——**那是錯的**：
一個**永久**無效的金鑰檔報錯是**正確行為**，而**一個讓那格通過的修法比本 bug 更糟**。
**缺陷只在「暫時不完整」那一格。** 工單 §3 與探針 G2 把這條界線釘住。

**(c) 交付更正了驗收方的射程理由，而它是對的。**
工單寫「乙（讓內容也原子）讓重試從『必要』變成『不必要』」——**錯**：
乙治的是**我們自己的寫入視窗**，而 I1／R1 的判準**在讀端**
（R1 用 `fs::write("")`，**不經 `create_new_at`**）⟹ **只做乙，R1 仍紅。**
**⟹ 而完整的版本是**：R1 釘的不變式**比缺陷所需的更強**——
「等過**任何**暫時不完整的身分檔」，不只「等過我們自己的窗」。
**那個更強的版本站得住**（身分檔也可能由安裝程式／同步工具／使用者複製產生），
**但它確實排除了「單做乙」**。⟹ **驗收方的理由錯了，而驗收方的探針比那個理由強。**

**(d) 乙的代價交付量了**〔本機 `/tmp`〕`link` 到已存在的 dest **EEXIST 且 dest 位元組不變**（D2 可守）；
`rename` **會成功覆寫** ⟹ **乙若要做必須用 `link` 不能用 `rename`**，且暫存名須隨機、與身分檔同目錄
（舊修補記下的坑：當年那個固定 tmp 名**語言層讀得到私鑰**）。

**(e) 交付誠實劃在射程外的那一格，驗收方追了它影響什麼。**
〔讀 `lib.rs:1103-1120`〕`node_id_if_present` 的失敗處理是 **`.ok()`**
⟹ 一個寫到一半的節點金鑰使它**回報 `None` ＝「我沒有 node id」**，而不是報錯、也不是等；
〔讀 `lib.rs:1268`／`peers.rs:527`〕兩個呼叫點都把它餵給 **`cmp_admission_order`**
⟹ **一次金鑰檔上的競賽，可以靜默改變哪些同儕被收進有限的名額。**
**⚠ 後半是讀碼不是量測**（沒有建同儕情境去量那個排序差異），**已照實標記並入 Inbox**。
⟹ **與本弧同族但方向相反**：本弧治「該等的時候沒等」（錯誤被正確回報，只是太早）；
那一列是「**失敗被寫成了一個答案**」——`None` 說「我沒有」而真相是「我還讀不到」，
**與 D66／O86 那一族同形**（不透明 vs 不存在被壓平）。

### 規格收尾（驗收方）

**無。** 本弧沒有新增或修改任何 MUST。
`REAL_01` §7.5（身分屬操作者不屬工作區）未動；**D2（不得覆寫金鑰）未動，且被探針 G2／G3 釘住**。

### 掛帳

*   **`Thunk`（`0x17`）**：同族唯一剩下的一格，**不需裁定**，模板現成（v0.54.0 的 Expr 節點表）。
*   **同儕准入排序那一列**（本版新入 Inbox）：**關鍵一步未量** ⟹ 只能先開偵察。
*   **`node_id_if_present` 的三種結局**（有／讀不到／確實沒有）若要分開，可能需要裁定。
*   **116 ms**：若日後覺得錯誤路徑的延遲不可接受，那是做乙的理由——**但乙不替代甲**。

---

## Q-052 — 一個取決於印表機的位址（2026-09-22，零修補回合）

**「第二個實作算不出位址」那一族，到本版清空。** 偵察 `docs/an_address_that_depends_on_a_printer_recon.md`；
工單／驗收 `…_handover.md`；探針 `crates/oo/tests/an_address_that_depends_on_a_printer_probe_test.rs`
（**基線 4 綠 0 紅**，最終 4／4）。
...
**全規格沒有任何一條規定那串字的逐字形** ⟹ 印成 `1+1` 的實作**完全合規**，而它算出不同的位址。

**而這可以正面示範，不只是「規格沒寫」**〔量，`v0.55.0`〕
`v: 1 + 1`／`v: 1+1`／`v: (1 + 1)` **三者同根 `5ab20106…`**，倉裡皆存 `~%__nlang_expr: 1 + 1`
⟹ **位址不取決於你怎麼寫，取決於印表機選了哪些空白與括號。印表機是承重的，而規格沒有承重它。**

### 驗收數字（驗收方自量）

*   探針 **4／4**（三輪）；全樹 ×3 **237／2246／0**（重界定後）；conformance **162／162**（rc=0，自跑）。
*   **G2／G3 的關係修後仍成立**：`1 + 1`／`1+1`／`(1 + 1)` 同址 `f4f32e7b…`；
    `(1+1)+1` `2af73e1c…` ≠ `1+(1+1)` `3e166870…`，裸鏈＝左結合。**未扁平化。**
*   **標準根不含 Thunk**〔交付量，驗收方要求的補償第 3 點〕`__nlang_thunk` 等**各 0 次**。
*   紀元範圍〔交付量，逐檔 evolve+commit 後看物件〕conformance **44／162**、引擎測試 **22／60**、`examples` **1／1**。

### 六件值得記住的

**(a) 本弧不是在修碰撞，是在移除依賴。**〔量〕`to_nlang` 對結合性**忠實**（右結合的括號留著）
⟹ **這裡沒有碰撞**，工單明文叫交付不要去找——**否則它會去找一個不存在的紅**。

**(b) 乙被否決，而理由是結構性的。** 「把 `to_nlang` 的逐字輸出規範下來」
會讓**每一個排版決定永久成為 Layer 1 破壞性變更**——換一個空白就是換所有 Thunk 的位址。
**一個語言不該把排版鎖進身分。**

**(c) `%id` 是第二個出口，而這是連續第二弧由交付自己找到的。** Q-050 與本弧皆然
⟹ **凡動身分編碼，`hash_recursive_with_salt` 要一起看**，否則 I1 只修一半。
**(Q3)** `Thunk` 與 `Code` 共用**同一張**表，**沒有第二份**。

**(d) 基線全綠與 Q-050 同形，而補償要求第 3 點抓到了驗收方上一弧的錯。**
Q-049 驗收方**跳過了「標準根含不含 `Code`」那一步量測**而推錯了結論；
本弧把同一步寫成交付的必答，**交付量了，0 次**。

**(e) 本弧真正的重量在 D72。** 全樹 ×3 三輪一致三支紅，皆屬 Q-038 的前哨兵夾具
——**`oo v0.20.0` 真二進位所建、其 README 逐字禁止重建**。
〔量，四個真二進位〕v0.53.0／v0.54.0／v0.55.0 **皆打得開**，本版 `#caid_mismatch`
（`16ba5683…` vs 重算 `cef5e484…`）⟹ **本弧是第一個吃掉它的弧**（它含 `Thunk{expr: Atom(Int 1)}`）。
**用戶裁 D72 ＝ 甲＋丙。**

**(f) 而「三支都紅」看起來像一件事，逐支讀過失敗訊息之後是三件。**
**r3／r5 的宣稱其實還成立**（r3 栽在包山包海的 `!contains("unavailable")`，
而它真正要查的那一行仍讀 `self-contained (pre-sentinel)`；r5 的 migrate 諸項全過，
只有尾巴的「歷史仍讀得到」失敗）⟹ **兩支是探針精度問題，收窄後保住**；**只有 r1 被殺掉**。
**⟹ 重界定 r1 時量到一件比預期好的事**：`evolve`／`commit` 在該倉上**也被具名拒絕**、`HEAD` 不動
⟹ **那是 r1 那句宣稱最強的形式：那次提交根本不發生。**
**⟹ 教訓**：**別把一組同時紅的測試當成同一個成因**——三支的失敗行不同，而其中兩支的宣稱沒有壞。

### 規格收尾（驗收方）

*   **`REAL_03` §6.2** `0x17` 一列酬載第一項由「`[expr: 字串]`」改為「`[expr: <expr>]`（Expr 節點表）」，
    並新增一段按語：為什麼不是「把 `to_nlang` 規範下來」，以及本紀元吃掉了什麼。
*   **`REAL_03` §6.2 表上自此沒有一列是「算不出來」的**（`Code` v0.54.0／`Thunk` 本版）。

### 掛帳

*   **D72 丙那一卡有時效**：每一次動到前哨兵夾具所含值種類的身分紀元，都會再吃掉一塊；
    它還含哪些值種類**未清點**。
*   **O88 未裁**：`Float`／`Complex` 的定點編碼有損（`Code`／`Thunk` 的原子沿用值表）。
*   **兩列同族待裁**：`=` 對 Thunk 強制求值而位址內涵（與 α-等價同）；
    **`Code` 沒有這個不同調**（D71 裁內涵，而 `Code` 的 `=` 也是內涵的）。
*   **「哪些純式子該留成 Thunk」**：〔量〕`v: 1 + 1` 留、`v: ~%Math./add (1,1)` 不留
    ⟹ D46 ① 是否半落地，未知。

## Q-053 — 需要一個二進位才成立的證據（2026-09-22，**無交接：全部由驗收方完成**）

### 這一弧治的是什麼

D72 丙：「趁那件前哨兵文物還說得出話，把它證得到的東西記下來。」**偵察把這個問題本身推翻了，
而推翻的方向是好的。**

**⚠ 本弧沒有引擎施工。** 四項射程全是儀器與文件，而**探針是驗收方的工作**
⟹ 交出去等於把唯一不得交出去的東西交出去。**故無工單、無交付方、無修補回合。**

### 偵察的三個讀數

**(a) 值種類清點（前一弧掛帳說「未清點」，本弧清完）**〔量，逐節點走那 67,913 B 的 JSON〕
`Combo` 291／`Atom` 562／`Thunk` 2，**其餘八種 `Value` 全部 0**；
`AtomKind` 只有 `Tag` 294／`Str` 259／`Int` 8，**其餘十三種全部 0**。
**對照組**：`data` 的欄位名共 12 種，與變體名撞名者 **0** ⟹ 計數沒被欄位名灌水。
**⟹ 沒有 `Float`**，故 O88 不會由該路徑碰到它。**但這句話要收窄**：物件是 format 2 的 JSON，
摘要是**讀回來再用 `bn_serial` 重算**的 ⟹ **它含的種類只框住「種類專屬」的紀元，框不住全域的**
（標記重新編號、長度編碼、欄位序）。

**(b) 它可以重生，而且是逐位元組的**〔量〕`/home/gali/nlang-baselines/v0.20.0-target/debug/oo`
（真 `oo v0.20.0`）重跑 `app: { k1: 1 }`：根物件 `cmp` rc=0，同為 67,913 B、同摘要 `16ba5683…`、
檔案 `sha256` 兩邊皆 `95dd69bb…`。**對照組**：commit 物件 `cmp` rc=1（時間戳）⟹ 量測確實碰到了東西。
README 那句「no such binary ships with this repo」是真的，**而它說的是 repo**；
它常被讀成「不可重建」。⟹ **時效性沒有消失，它換了對象**：不在版本控制裡的是**那個二進位**
〔量〕`/home/gali/nlang-baselines/` 不在任何 git repo 之內。**位元組是耐久的那一份，二進位不是。**

**(c) 引信也換了對象，而是驗收方自己三天前埋的**：`a_commit_that_closes_the_door_probe_test.rs`
的 r1 寫死了 **`cef5e484`——「本引擎現在會重算成什麼」**。⟹ **下一次被吃掉的不是文物，是那一行字串常數。**

### 落地的四項

1.  **新探針 `evidence_that_needs_a_binary_probe_test.rs`（6 支，全部不經引擎驗摘要）**：
    e1 根物件佔 67,913／68,526 B（**99.1%**，＝§6.8 第一段所述形狀）；
    e2 **沒有任何 `masa_ref` 是 `Digest`**（＝自足，§6.8〔適用範圍〕那款的判準）；
    e3 值種類封閉清單（＝下一次紀元的射程）；e4 HEAD→commit→root 三者摘要自洽；
    e5 **孤立的引信**；e6 編碼下限紅線。
2.  **r1 拆引信但不藏**：`cef5e484` 換成形狀斷言（新 helper `digests_named_in`：具名兩個**不同**摘要，
    其一為 `16ba5683`），字面值移進 e5 **單獨命名**⟹ 下次它燒掉時，**失敗的測試名本身就說出成因**。
3.  **夾具 README** 補記重生法、逐位元組相同、以及重生器不在版本控制裡。
4.  **編碼下限紅線**（用戶裁「可併」）：〔讀 `storage.rs:166,211`〕
    `MIN_READABLE_STORE_FORMAT_VERSION = 1` … `OBJECT_ENCODING_VERSION = 5`，
    拒絕訊息逐字印出該範圍 ⟹ **紅線是一行機械不變式：那個下限不得上升**，由 e6 釘住。

### 三件值得記住的

**(a) 校準抓到的是驗收方自己探針的機制錯誤。** e6 第一版把 `objects.format` 寫成 `99`
——**被宣告解析器在更早一步就拒了**，從未抵達本測試要查的範圍檢查（正確拼法是 `encoding=99`）。
⟹ **「探針綠了」與「探針量到了它宣稱在量的東西」是兩件事，而這次差別在一個字串前綴。**

**(b) 六支全綠者逐支以突變證明會紅**（故意改壞 → 必須變紅，六項全 YES）。
而 `e4` 那一輪的分母是 **2 而不是 1**——測試名篩選字串 `e4` **也命中了 `e5`**，
因為 `cef5e48`**`4`** 裡有 `e4`。⟹ **任何聚合都要印出分母**，這次分母自己會騙人。

**(c) 一個位元組級的事實，不會因為讀它的人換了算法而改變。**
§6.8 上段引的 `16ba5683…` **沒有因為 v0.56.0 而過期**——移動的是「今天的引擎對那些位元組會算出什麼」，
不是「當年的引擎寫下了什麼」，而該款引的是後者。**這就是把證據從二進位搬到位元組的全部理由。**

### 驗收數字（驗收方自量）

全樹 ×3 `--release --no-fail-fast`：三輪**完全一致** **238 target／2252 passed／0 failed／`^error` 0／cargo rc=0**；
〔先證明量測碰到了目標〕每輪逐檔確認 `evidence_…` **6/6**、`a_commit_that_closes_the_door` **9/9**。
分母自洽：上次切版 2246 → 2252，差 **6** ＝新探針支數。conformance **162／162** rc=0。
**diff 純度**：三檔，**引擎原始碼零變更**，**文物位元組 `oo_dir/` 零變更**。

### 規格收尾（驗收方）

*   **`REAL_03` §6.8** 新增〔2026-09-22 證據形式更正，D72 丙〕：本款證據自此不需二進位在場，
    並記 `16ba5683…` 的重新驗證、證據檔位置、值種類封閉清單與其射程界線。
*   **`meta/VERSIONING.md` §7**（同日另一條線）：標籤形式一律 annotated，含 §7.1 誠實記帳。

### 掛帳

*   **前一弧的「還含哪些值種類未清點」已結**（見偵察 (a)）。
*   **新**：`/home/gali/nlang-baselines/` 不在版本控制內，而它是 v0.20.0 之類舊二進位的唯一住處。
    **這不是本弧能修的**（把二進位進倉是另一個決定）⟹ 已在夾具 README 明說，並入 Inbox。
*   **同儕准入排序那一列**仍在 Inbox，**關鍵一步仍未量** ⟹ 只能先開偵察。

## Q-054 — 一把讀不到的金鑰被讀成沒有金鑰（2026-09-23，零修補回合；**驗收方修補一次，對象是自己的探針**）

### 這一弧治的是什麼

`node_id_if_present` 與 `init` 的兩個讀取都用 `Identity::load(..).ok()` ⟹ **三態塌成二態**
（不存在／存在但讀不到 → 同為 `None`）。而 `peers.rs:319` 的 `restore_asserted` 把 `None` 讀成**身分不符**
⟹ 每一筆記錄的斷言半邊整個被丟棄、`admission_seq` 歸零、路由索引未播種
⟹ **`None` 自己製造出平手，再以「只由對方決定」的方式打破它**——正是 `REAL_02` §5.1.2 那條
**被判為不可達而刻意不設探針**的 MUST NOT。

### 交付

`Identity::load_if_present`：不存在 → `Ok(None)`；存在 → Q-051 已裁的有界等待；**仍讀不到 → `Err`**。
`init` 的 948／957 **併成一次**；`node_id_if_present` 同等並快取遲到的成功。
〔交付量〕永久損壞金鑰的成本是**一次** 100 ms（完好 13 ms／0 B 113 ms／20 B 116 ms），不是三點各一次。

### 驗收數字（驗收方自量）

探針 **10／10**（含驗收方新增的 `r5`）；conformance **162／162**。
全樹 ×3 三輪一致 **239 target／2262 passed／0 failed／零 `VOID READING`／cargo rc=0**。
**身分紅線**（v0.56.0 基線二進位 vs 本建置，**排除含時間戳的 commit 物件**）：
`x: 0` → `31745ef0`、`v: 1 + 1` → `f4f32e7b`、標準根 → `7038e250`，**三者皆未動**；對照組活。

### 四件值得記住的

**(a) 驗收方的探針有一道守衛極性寫反了，而交付沒有去動它。** `observed_mid_write` 斷言 `read_done < wrote_at`
——**一個會等的讀者必然在寫入者之後結束** ⟹ 它作廢的**正好是修好的狀態**，永遠不可能變綠。
我只拿壞掉的引擎校準過它（那時它永遠不開火）。交付印出 `VOID READING` 並回報、附上 `Saw Some(<CAID>)`
——**那是正確的處置**。修補：移除守衛，**新增 `r5`**（永不補完的空金鑰必須先花掉預算），
因為 `r1`／`r2` 只斷言結果、分不出「等過」與「剛好慢到錯過視窗」。
**⟹ 新紀律：校準要兩極都做**——不只「弄壞期望它會紅」，還要「程式對了它會綠」。
〔量，`git worktree` 於 `3a1927b`〕修好的探針在修補前 **5 綠 5 紅**（不是 VOID），修補後 **10 綠**。

**(b) 一條不設探針的 MUST NOT，其「不可達」本身是一個宣稱，而宣稱需要被量。**
`seat_order` 那一弧把 C′ 判為不可達的前提（收受順序為全序）不涵蓋讀不到本節點身分的情形——
而那時平手是**實作自己製造的**。〔量〕兩個持有不同身分、皆讀不到的節點對同一批記錄算出**逐字元相同**的順序。

**(c) 驗收方第一次的身分紅線比錯了東西**：比的是 `commit` 物件摘要，那含時間戳，本來就會不同。
**先證明量測碰到了目標**——這次是在寫進帳之前抓到的。

**(d) 驗收方在 O91 裡記下的那件事**：Q-051 我以錯誤理由偏好乙、交付更正過；
三天後我原地推導出同一個乙（命名戊）並差點再偏好一次。兩次抓住它的都是同一支探針 R1。

### 規格收尾（驗收方）

*   **`REAL_02` §4.1.1** 新增 MUST「**存在但讀不到，不是不存在**」（D73），含判例、可用性代價、
    與 §5.1.1「缺席即拒絕開啟」之同形，及自陳缺口（身分位置無法解析的那條路）。
*   **`REAL_02` §5.1.2** 平手那條 MUST NOT 補判例（曾可達，且可達方式與被判不可達的理由正面相反）；
    身分比對補 MUST「**『不符』只指讀到了，而且是另一個**」。

### 掛帳

*   **身分位置無法解析**（相對 `OO_NODE_HOME`、無 `HOME`）⟹ `init` 仍以「尚無節點身分」繼續，**C′ 在那條路上仍可達**。
    已自陳於 §4.1.1，待裁。
*   `RoutingIndex::format_cli` 為死碼，路由表無本地出口（Q-054 開卡時入 Inbox）。

## Q-055 — 一次拒絕，被寫成了一個答案（2026-09-23，**兩個修補回合，兩個成因都在驗收方**）

### 這一弧治的是什麼

指令做不成自己的工作時，在一個面說出來、在另一個面否認，或說成另一件事。
**最重的一個**：`eval`／`inspect` 的 `Ouroboros::init(..).unwrap_or_else(|_| new_in_memory())`（自 2026-05-24 起）
吞掉**每一種**初始化失敗、換上空儲存 ⟹ 存在而打不開的儲存（`encoding=99`、D73）裡，
一個確實在裡面的提交「找不到」、一個確實存在的位址回 `#missing_key`——**與哪裡都不存在的位址逐字同形**；
D73 在 `eval` 上被繞過。其餘：`status` 說讀不到而 rc=0；`migrate` 動手後才說代價、只說一部分、沒做卻說做了；
兩個拒絕說宿主的話（serde 的 `line 1 column 1`；D73 文字裡的 `KeyRejected(…)`——**Q-054 驗收漏看**）。

### 交付（e2831a3 → R-1 7187b95 → R-2 9b6ec16）

`engine_or_ephemeral`：初始化失敗時**只在沒有耐久儲存**（無 `format`／`HEAD`／CAS 物件；`.oo` 不可統計算「有」）才換暫時儲存。
`status` 讀不到工作集即 `bail`。`migrate`：兩份宣告都已是本引擎所寫者才說「沒有改變」；
代價句**先印並 flush 再寫**，名單依**兩份宣告**由各標籤常數算出。注入檔與 D73 的拒絕改為引擎的話。

### 驗收數字（驗收方自量）

探針 **17／17**（含驗收方新增 r9／r10／r11）、`nothing_here…` **7／7**；conformance **162／162**。
全樹 ×3 三輪一致 **240 target／2279 passed／0 failed／`^error` 0／cargo exit 0**。
身分紅線 `x: 0` → `31745ef0`、`v: 1 + 1`／`v: 1+1` → `f4f32e7b`、標準根 → `7038e250`，**皆未動**。
**代價句**：七個起點、21 個真二進位（v0.20.0、v0.21.0、v0.22–v0.27、v0.32–v0.44），倉由真建立者造——**七句與實測區間逐一相符**。

### 值得記住的

**(a) R-1：射程只寫了半個不變式。** I1「沒改就不說改了」被做得字面正確（只看 layout），
一個 `layout=5`／`encoding=4` 的半途倉從此**沒有任何明示動作能完成**，而 v0.57.0 做得到。
**寫禁止句時，同時寫它不得奪走的正面行為。**
**(b) R-2：量測範圍被寫成事實的邊。** 工單寫「〔量〕鎖在門外的是 v0.40.0–v0.43.0」——那次只跑了 v0.40.0 以上。
實際下界隨編碼是 v0.22.0／v0.26.0／v0.36.0，裸數字更早。**且探針放過了它**：判準接受 v0.44.0，
而句子結尾「v0.44.0 and later still open」就含它。
**(c) 一支舊探針的 REACH 騎在本弧的缺陷上**（`nothing_here…::g3` 的注入 `{}` 不是任何佈局的注入，
靠 `status` 說謊才綠）。交付回報而不改——**正確處置，第三次**（Q-054 的極性、Q-055 的 g3）。
**(d)** 交付靠「`init` 先寫 `.oo/format` 再讀節點金鑰」才讓無儲存目錄也守 D73——**一個順序，不是一個不變式**，由 r10 釘住。

## Q-057 — 一筆提交就是一個值（2026-09-24，**兩個修補回合；三項中兩項源頭在驗收方，同一個錯的量法**）

### 這一弧治的是什麼

提交的位址是手工挑欄位算的（`[父摘要] ‖ 根摘要 ‖ 種類 ‖ 來源* ‖ 目標* ‖ format!("{:?}", meta)`），
於是（Q-056 量到）授權裁決、來源／目標的分界、根位址的譜特徵都在位址外，而整個位址依賴 Rust 排版與 Unicode 表。
**D74（用戶「甲」）**：提交就是一個值，位址照它在磁碟上那個 n/ 值算；既有提交不改寫；**D74 ②** 舊提交位址外的欄位不以事實口氣呈現。

### 交付（44cbe32 → R-1 8b9dda1 → R-2 8954d72）

`layout=6`（佈局軸：物件位元組不變，變的是 `HEAD` 與 ○ 註記所指的算法）。新提交位址＝框線下本文經**值解碼器**讀出之值的 `content_hash`；
選擇子是位址版本（v1 舊式／v2 新式），只有摘要的引用「舊式先、對上即停」；v2 提交位址三者全比；
舊 refine 授權印 `unattested`；`migrate` 代價上界 v0.58.0。

### 驗收數字（驗收方自量）

探針 **13／13**、Q-055 檔 **17／17**；全樹 ×3 **241 target／2292／0／exit 0**；conformance 162／162；
值的位址（`x: 0`／`v: 1 + 1`／標準根）**皆未動**。〔真二進位〕v0.44.0–v0.58.0 對 `layout=6` 以「宣告看不懂」拒絕；
v0.58.0 讀寫新引擎續寫的 `layout=5` 儲存 rc=0。40 筆 `log` 1.9×（第一版 14×）。

### 值得記住的

**(a) 一個錯的量法，兩次被做成機制。** r1 以「對本文**求值**後的 `identify`」定義位址；求值得到欄位皆為 Thunk 的 combo，不是磁碟上的值。
第一輪交付因而每驗一筆提交就求值（14×、位址成為求值器的函數）；修補回合交付改了 `identify` 本身去對上它。
**交付該寫進「工單哪裡是錯的」，而不是改儀器；但把儀器放在可以改的地方、又沒有釘住它的，是驗收方。** 修法：量法改為儲存自己的值解碼器；g5 釘 `identify`。
**(b) 開卡時沒有 grep 其他釘了 `layout=5`／v1 提交位址的探針**——交付改了 8 檔（只跟著格式），受理，但那是工單該列的。
**(c) v2 提交位址只比摘要**（交付的）：F3 換了一層又回來，違反 §6.6 既有 MUST；r7 釘住。

## Q-058 — 只有簽章承載得了的裁決（2026-09-25，**一個修補回合，成因在驗收方**）

**缺陷**：D74 後新式提交的授權字樣在位址內——擋得住竄改、擋不住偽造〔量，v0.59.0：鑄一筆位址自洽、字樣 `verified` 的未簽精煉，`log` 逐字印出〕。
**D75**（用戶「甲」）：讀者只呈現讀取時重驗的簽章（是誰簽的），不呈現存下的字樣、不宣稱寫入時的白名單成員資格；舊式提交的簽章同樣可重驗。
**D75 ②**（用戶「甲」）：為兌現 `SPEC_10` §2.5「未驗證必須留痕且可區分」，新式提交把寫者的字樣標成寫者的記錄並列。
**交付**（1f52139 → R-1 92f9a47）：`authority::signature_signer`（只做 Ed25519、不查登記）；`log` 的 `read_authority_line`；`refine --sign` 多印 `Refine signer:`。
