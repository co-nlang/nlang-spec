...
*   **防禦機制（2026-07-11 兩軌修訂，APP_02 §0/§6）**：
    *   **身分幾何驗證 (GPP)**：節點必須附帶 $\omega/q$-Gram symplectic 指紋一致性證明
        （$\mathbb{F}_2$ STARK，**[APP_02](./APP_02_Formal_Verification.md)** §6；傳輸層義務
        見 **[REAL_02](./REAL_02_Ouroboros_Protocols.md)** §7）——驗證其真實持有宣告服務
        對應的子空間**身分**。驗證失敗＝身分不可信，逐出路由。
    *   **質量退位**：質量 $m$ 為執行軌啟發值、**不可證也不需證**——謊報質量只影響
        路由優先序，路由誘導由反模式機制吸收（黑名單、tiebreaker、hop 預算、§7.2 引力
        門檻）。防禦的錨不在質量而在身分：指紋證明＋外部物理錨點（ORDER_00 §1.1——
    ...
    *   **攻擊成本模型（重估 TODO）**：舊模型「偽造譜」已失效；新模型＝「偽造身分幾何
        ＋外部錨點」的成本量化，待補（meta/ROADMAP「APP_02 衍生 TODO (a)」）。
...
| 未經 GPP 身分驗證的服務廣告。 | OODP 安全 | 女巫節點干擾路由（質量聲稱本身無驗證義務，見 §7.1 質量退位）。 |
