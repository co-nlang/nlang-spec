...
| **`v0.12.0-draft.1`** (目前) | **`v2`**（凍結） | `v1`~`v2` | 演化邊界的衝突揭露（SPEC_10 §2.2.1 新設）：演化撞到 `_|_` 時**必須**說出座標，且**必須到葉**——不得只是該次演化所寫入的頂層欄位；回報必須含 ERROR_CODES 的錯誤碼名，**不得**以引擎內部資料結構之形式呈現；多個回報面必須給出同一個座標。自陳缺口：座標要求可被**指認**而未要求可被**導航**（引號鍵寫得進去、路徑語法拼不出來），歸 SYNTAX 側 |
