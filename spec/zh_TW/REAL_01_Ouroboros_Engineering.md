...
> **[!IMPORTANT] §1.1 與 §1.2 的「拼法」不是規範性的（2026-09-12）。**
> 本規範**不**約束指令名與旗標名。約束的是**行為**（§1.3）與**介面形狀**（§1.4）。
> 判準：**一條規則會不會因為改名而變假？** 會，它是機制，不寫進規範性條文；
> 不會（只會被違反），它是不變式，可以寫。
> 逐字寫下一個拼法即是寫下一個機制，而機制會腐爛——
> 〔量 2026-09-12，參考實作 v0.46.0〕**下列 10 個拼法中，存在者 3 個，
> 且其中一個的簽名是假的**（`--format` 不接受值），**故以簽名計為 2½／10**。

### 1.1 One-shot 模式 (`oo run`) **[Reference Recommendation]**
```bash
oo run --observe <path> [--commit] [--format json|n] [--load <file>]
```
> **實況（2026-09-12，參考實作 v0.46.0）**：`oo run`、`--observe` 存在；
> `--format` 存在但**不接受值**（`--format json <file>` 會把 `json` 當成檔名）；
> **`--commit` 與 `--load` 不存在**——提交是一個獨立的頂層指令。

### 1.2 Service 模式 (`oo service`) **[Reference Recommendation]**
*   **宇宙節點 (Universe Node)**：承載特定視角內的子空間狀態，並透過標準協議暴露「投影與演化」能力。
```bash
oo service [--socket <path>] [--host <h>] [--port <p>] [--privileged-token <t>]
```
> **實況（2026-09-12，參考實作 v0.46.0）**：**`oo service` 不存在**；
> 其職能由一個以節點為對象的子指令承載。
> `--socket`／`--host`／`--privileged-token` **不存在**；`--port` 存在但在別的指令上。

### 1.3 引擎不得以自身的表示回答 **[Core Requirement，2026-08-09 新增]**

**[SPEC_10 §2.2.1](./SPEC_10_Evolution_and_Commit.md)** 對演化邊界的座標立下
「**不得洩漏實作表示**」。該條的**理由與射程不相稱**：那個理由——
「那些是**關於引擎的事實**，不是關於使用者的宇宙的事實」——與座標無關，
與演化邊界也無關。它適用於引擎向操作者說的**每一句話**。本節將其升為通則。

*   **不得洩漏實作表示（MUST NOT）**：引擎任何**面向操作者的輸出**，
    **不得**以宿主語言的內部表示呈現值或診斷。尤其**不得**含原始碼位元組偏移、
    AST 節點名稱、宿主語言的除錯輸出格式、同步原語、或任何在 n/ 中無拼法之物。
*   **值的正準文字形必須是全函數（MUST）**：把一個值印成 n/ 文字的函數，
    **必須**對該實作的**每一種值**有定義。**不得**存在「其餘一律以宿主語言格式輸出」
    的退回分支——**一個只對某些值有定義的正準形不是正準形**。
*   **印出來的必須讀得回去（MUST）**：所印出的文字**必須**是合法的 n/。
    未觀測者**應當**印出它的**來源**而非它的答案；持有型引用**必須**印成其結構形，
    使重讀仍得到一個引用而非其求值結果。
*   **未觀測不是錯誤（MUST NOT）**：引擎**不得**因一個值尚未被觀測而拒絕印它，
    也**不得**為了印它而觀測它。**觀測由操作者發動**（[SPEC_04](./SPEC_04_Navigation_and_Duality.md) §6）；
    列印不是觀測。
*   **宿主的中止不是一個答案（MUST NOT，2026-09-12 新增）**：引擎**不得**以
    宿主執行期的中止（panic／abort／未處理的訊號預設處置所產生的崩潰訊息）
    作為對操作者的回答。**這一款與第一條分開，因為它治的不是那段文字的內容，
    是它的形式**——一個文字乾淨的 panic 仍然不是答案：它沒有被任何人選過。
    〔三個證人，皆為量測〕剖析器執行緒 panic 被冒名為深度超限（`TAG_REGISTRY` §2.7.5）；
    主執行緒建立失敗時的 `.expect` 給出 rc=101 而沒有任何 n/ 的答案；
    下游提早關閉管線時，`println!` 在 `EPIPE` 上 panic，交出宿主原始碼位置與裸 errno。
    **⚠ 本款不決定「該回答什麼」**：安靜結束與具名拒絕都滿足它，
    那是載體的問題（[TAG_REGISTRY](./TAG_REGISTRY.md) §0.2），不是本款的。

*   **介面必須自述（MUST，2026-09-12 新增）**：引擎所暴露的**每一個**指令、
    旗標與位置引數，**必須**附有一句面向操作者的說明，說出它做什麼。
    **一個對操作者什麼都不說的指令，與一個用宿主語言的內部表示回答的指令，
    是同一條誠實性軸上的兩個失敗**——說明文字本身就是「面向操作者的輸出」，
    本節前四條約束的是它的**內容**，本條約束的是它的**存在**。
    〔量 2026-09-12，參考實作 v0.46.0，**逐層走遍整個指令樹**：
    指令 **9／29**、旗標 **19／37**、位置引數 **6／11** 完全沒有說明。〕
    **〔已兌現 2026-09-12，Q-042〕** 同一量測於交付後為 **29／29、37／37、11／11**，
    且參考實作以一支走遍 clap 樹的測試把它變成建置期的閘
    ——**條文要的是那個閘，不是那一次補齊**。

> **論證性量測（2026-08-09，參考實作 v0.12.0）**：`app: { k1: 1, msg: "hi" }`
> ——四行來源，兩個葉子。`oo status` 回給操作者的是
> `k1: Thunk { expr: Expr { kind: Atom(Int(1)), span: Span { start: 13, end: 14 } },
> closure: [ComboVal { … cache_id: RwLock { data: None, poisoned: false, .. },
> legacy_fields: {} … }] }`。其中 `span` 是原始碼位元組偏移，`RwLock` 是宿主語言的
> 同步原語，`legacy_fields` 是一個該實作**沒有任何程式路徑會寫入**的欄位。
> 三者皆非關於使用者宇宙的事實。
>
> 同一份輸出寫回檔案再交給該實作的格式化器，**剖析失敗**。
> 而該實作的 **CAID 計算路徑正在把這個函數的輸出當成 n/ 原始碼餵回求值**
> （`~%Discovery./identify`，見 [REAL_02](./REAL_02_Ouroboros_Protocols.md) §4.2）——
> 亦即「印出來的讀得回去」在該實作中**已經是身分正確性的前提**，
> 而非輸出慣例。這是本節第三條為 MUST 的原因。

> **兌現度（2026-09-12，Q-042 驗收方自量；同日 Q-043 後更新為 5／5，見本段末）**：本節五條當時**兌現 4／5**。
> 第一條（不得洩漏實作表示）**已兌現**——`1 & 2` 的診斷改印運算元的正準文字形；
> 面向操作者的檔案讀寫收束為單一入口，其錯誤理由是一個**全函數**的對映，
> 故任何宿主錯誤類別都不可能再以 `os error N` 的形式交給操作者
> 〔量，三種錯誤類別 × 六個入口，含條文與探針都未指名的入口〕。
> **第三條（印出來的必須讀得回去）仍未兌現**，且缺口收斂到單一形式：
> `_|_` 的列印形。〔量〕該形式餵回格式化器**在 `(%cause:` 處剖析失敗**，
> **而錯不在實作**——本規範逐字使用該記法數十處，
> 而 [SYNTAX_02](./SYNTAX_02_Literals_and_Atoms.md) §2 只把 `_|_` 定為裸原子，
> **文法從未定義帶因形**。⟹ 這是規格自身的缺口，待裁（`meta/oo/STATUS.md` O85）。
>
> **〔已兌現 2026-09-12，Q-043／D65〕本節五條現為 5／5。** 缺口由**規格側**補上而非引擎側妥協：
> ⊥ 的正準列印形定為**裸原子**，成因移入診斷註解層（`;; %cause: <tag>`，
> 與 `%effect` 同拼法同層，[SPEC_11](./SPEC_11_Reflection_and_Synthesis.md) §3.4 新增之法定成員），
> 正式觀測通道為 `.%cause`。〔量，有對照組〕裸 `_|_` 回讀通過，
> **且巢在記錄中時整筆記錄亦回讀通過**——後者才是本條真正要保的，
> 因為舊形式會把**每一筆含 ⊥ 的記錄**一起弄成讀不回去。
>
> **⚠ 註解層的固有前提，明記於此**：`;;` 註解延伸到行尾，
> 故「印出來的讀得回去」**以行結構被保留為前提**——
> 把輸出的換行摺成空白，註解會吞掉其後的一切（含閉合的 `}`）。
> **這不是本次引入的**：`%effect` 尾註自 2026-07-20 起即如此。
> ⟹ 消費輸出的工具**不得**摺行；本節第三條要求的是**逐行原樣**可剖析。
> 〔驗收方 2026-09-12 自陷此坑一次並就地記名：以 `tr '\n' ' '` 取輸出後回讀失敗，
> 那是量測的錯不是引擎的錯。〕

> **自陳缺口（2026-08-09）**：本節第三條要求的是**可剖析**，**不是可重求值**。
> 一個未觀測值印出的是它的來源運算式，而該運算式在別處重讀時所處的環境
> 未必是它原本的環境；要讓「重讀必得同一個值」成立，需要**環境隨行**，
> 那是另一個決定（見 [SPEC_10](./SPEC_10_Evolution_and_Commit.md) §2.2.1 之相鄰議題）。
> 同理，被引述的程式碼（`%code`）印成其運算式後重讀會**被求值而非被引述**。
> 本節不宣稱這兩者已解決。

### 1.4 介面形狀 **[Core Requirement，2026-09-12 新增]**

本節約束介面的**文法**，**不得**出現任何指令名或旗標名。
本節每一條都必須通過 §1.1 前言的判準：**改名不會使它變假，只會使它被違反**。

*   **一個概念一個拼法、一個簽名、一份說明（MUST）**：
    若兩個指令暴露**同一個概念**，該概念在兩處**必須**是同一個拼法、
    接受同一種引數、並附**同一份說明**。
    **不得**出現「同一個旗標在某些指令上有說明、在另一些指令上沒有」。
    這是本規範一貫的「**一個概念一種正準拼法**」
    （[SYNTAX_05](./SYNTAX_05_Prefix_Ontology.md) §3 之 O72 裁定、
    [SYNTAX_03](./SYNTAX_03_Paths_and_Assignment.md) 之「跳脫只有一處」）
    **從文法搬到介面**；理由亦相同——
    **等價而不同的拼法讓操作者無法由一處學會另一處**。

> **論證性量測（2026-09-12，參考實作 v0.46.0，走遍整個指令樹）**：
> 出現於一個以上指令的長旗標共 **5 個，其中 4 個不一致**。
> 表示「選擇性能力授與」的旗標出現於 **8** 個指令，**4 個附有說明、4 個空白**；
> 表示「全量授與」的旗標出現於 **7** 個指令，**3 有 4 無**，
> 且無說明的正好是同樣那四個指令。
> **這不是漏寫四句話。**〔量〕那四個指令**不是同一批加入的**——
> 它們來自**三次不同的變更**，而**每一次都重複了同一個省略**。
> 沒有任何機制會在加入第五個時提醒任何人；**本條就是那個機制。**
>
> **〔已兌現 2026-09-12，Q-042〕** 交付後該兩個旗標在**每一個**指令上引同一份字串；
> 出現於一個以上指令的長旗標**全部一致**。
> **⚠ 同時量到本條說不到的一格**：有一個長旗標出現在三個指令上而**是三個不同的概念**。
> 本條寫的是「**同一個概念**必須同一份說明」，**沒有說不同概念不得共用拼法**
> ——**那條反面今天沒有人寫過**，見 `meta/WORK_QUEUE.md` Inbox。

> **待定（不在本節）**：「指令的對象若有顯式身分，該對象是否必須置於動作之前」
> 是一條真正的形狀規則，但它的內容取決於**當前工作區是不是一個要說出名字的對象**
> ——而那是另一個尚未裁定的問題。
> 〔量 2026-09-12〕頂層 18 個指令 ＝ 12 個動作 ＋ **5 個名詞葉子** ＋ 1 個對象空間；
> 那 5 個名詞葉子**全部**作用於同一個隱含對象（當前工作區），
> 而唯一的對象空間是唯一具顯式身分者。**在該問題裁定前寫下此條即是先押注。**
...
#### 4.1.1 耐久寫入的原子性 **[Core Requirement,2026-08-06 新設]**

`.oo/` 之下**任何**耐久寫入,**必須**是一次**原子安裝**:寫入與目標**同目錄**的
臨時檔、`fsync`、再以 `rename` 就位。同目錄是必要的——`rename` 的原子性只在
單一檔案系統內成立。

*   **並行讀者不得看見截斷的目標(MUST)**。就地覆寫(如以 `O_TRUNC` 開啟後再寫)
    在截斷與寫入之間留下一個窗口,窗內的檔案比新舊兩版都短。
*   **並行讀者亦不得看見短暫缺席的目標(MUST)**。「刪除後重建」同樣使目標換上
    新的 inode,卻只是把「截斷窗口」換成「不存在窗口」——**故驗證此條的量測必須
    同時計「不可解析」與「找不到」兩種失敗**,只計前者會讓後者讀起來像通過。
*   **失敗不得留下殘檔(MUST)**。中斷或失敗的寫入**不得**留下任何可被目錄掃描
    誤認為內容的檔案。此非潔癖:`objects/` 之普查以**目錄項**為單位,分片目錄下
    的殘檔會成為一個**有大小的幻影物件**。
*   **內容定址物件的重複安裝(SHOULD)**:同內容同位址,重複寫入應為無操作;
    以 `rename` 就位者天然冪等,故「先檢查存在再寫入」之競態在此消失。

> **論證性量測(2026-08-06)**:一個 94 KB 的 `staged`、60 次寫入、一個連續解析的
> 讀者 ⟹ **63,769 次讀取中 12 次解析失敗**(每次寫入 0.2 次)。
> 拿掉這個數字,本節的 MUST 就只剩斷言。
...
**判別方式:問「若這件事是假的,誰付出代價、付出什麼」。** #2 假 ⟹ 有人被冒名,代價是不可否認性;#3 假 ⟹ 有人做了不該做的事,代價是特權;#4 假 ⟹ 你白跑一趟取物,代價是**延遲**;#5 假 ⟹ 路由被整群佔據,代價是全網可用性;#6 假 ⟹ 有人把某操作者的信譽延伸給一個該操作者並未擔保的節點,代價是**錯置的同意**,且**只由將該操作者列為歸屬信任根的人承擔**。**#4 的代價與其他四者不同量級,這正是它屬於執行軌的判準。**
...
**「算子應用」的範圍（2026-08-11 裁定；理由見 O41）**：本表的「算子應用」專指一次**態射應用**，
而非任何在語法上看起來像運算子的節點。由既有 AST 所界定的字面算術、結構建構、
或 AST 走訪本身，不因含有運算子便另扣 MBU；若該走訪實際觸發態射應用、升寫、
合併或其他本表操作，仍依觸發的語義操作計費。
