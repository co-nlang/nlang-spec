...
**測試即觀測：通過的判準是「這次觀測決定了一個定形事實」**（SPEC_00 §1.2
靜與動；2026-07-20 裁定 B，取代舊文「任何非 `_|_` 即過」——空洞真禁止）：

1.  **通過 (Pass)**：節點收斂為**定形值**（Atom、Combo、List 等；含 `#true`），
    且過程中無未捕獲異常。收斂為定形非布林值＝合法的冒煙斷言（斷言「可收斂」
    本身）。
2.  **失敗 (Fail)**：
    *   收斂為 `_|_`——引擎自動提取 `%cause` 產生錯誤報告；
    *   收斂為 `#false` 或 `#fail`——斷言被駁斥；
    *   收斂為 `_`（Top／TopCaused）——**未定 (undetermined)**：什麼都沒決定
        的觀測不是證明（如 `(_) == 5` → `_`；歷史教訓：孤兒測試 `_ == #io`
        曾藉舊判準空洞通過）；
    *   收斂為 `#blur`——視界內未定，報告 blur 之 `%cause`（如
        `#fuel_exhausted`）。欲斷言視界行為，請斷言其元讀：
        `(x).%cause == #fuel_exhausted` → `#true`。
