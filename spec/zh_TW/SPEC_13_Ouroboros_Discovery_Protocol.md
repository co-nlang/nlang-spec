...
3.  **宇宙不得含引擎本地狀態** **[Core Requirement，2026-07-27 新增]**：引擎**不得**將任何**引擎本地**且**非決定性**之值鑄入宇宙——包括但不限於行程或安裝所生之金鑰、識別碼、亂數、時間戳、路徑與主機資訊。**理由**:義務 #1 是對**內容**的斷言;若引擎自行把逐行程變動之值放進內容,義務 #1 便無法被滿足,而失敗形式是**沉默的**——雜湊仍然正確,只是內容每次都不同,於是兩個引擎讀同一份原始碼永遠得不到同一個宇宙身分,根層級的互通在原則上不可達。
    ...
    *   **與全域治理物件之區別**:`~%Official`(**[ORDER_01](./ORDER_01_Evolution_and_Governance.md)** §117 之信任根)是**經發現取得**的全域治理物件,其內容依 ORDER_01 §88 只經 RFC 變更。引擎於本地**合成**一個同名節點並自任為其成員,不是該物件的退化版本,而是**另一個東西冒用其名**;停止合成不構成 ORDER_01 §66 所禁止的「廢棄特性之物理移除」——被移除的是偽造品,規格所定義的那一個從未被本地引擎持有。本地觀測該路徑得 `⊥ #missing_key`,即「本引擎不持有該全域物件」之誠實答覆。
    ...
    > **已兌現（2026-08-20，v0.27.0）**：本節寫下時,語言層**沒有**這個答覆
    > ——〔量,v0.26.1〕缺席的 `~%` 名字一律得 `_`,而參考實作以一個本地合成的
    > 空殼 `{{ }}` 佔住該路徑。兩者已同弧修正:空殼移除,且
    > [SPEC_09](./SPEC_09_Standard_Library.md) §0 新設「系統軸是封閉世界」
    > 使缺席即 `⊥ #missing_key`。**該條是通則不是為本節開的特例**
    > ——理由為 `~%` 唯引擎鑄造,故引擎對該軸具備完全知識。
...
*   **語言表面結果**：
    ...
    >
    > **實測(v0.2.54)**:任一來源之任何失敗皆收斂為 `#caid_mismatch` 並置起「見過不符」旗標,故上述情形實際回 `⊥ #caid_mismatch`——而本節與 TAG_REGISTRY `#caid_mismatch` 條(「純粹『無人持有』不適用此碼」)**皆已明文禁止**。此為既有條文未被滿足,非新增義務。
