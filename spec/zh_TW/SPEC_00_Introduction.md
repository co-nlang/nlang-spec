...
| **`v0.9.0-draft.1`** (目前) | **`v2`**（凍結） | `v1`~`v2` | 自動准入與其上限（REAL_02 §4.2.6.2 / §4.2.6.3 新設，§4.2.6 修訂）：歸屬信任根成為同意的聚合單位，資格六條綁確切廣告，自動來源上限 3 且與手動來源分開計數，准入不撥號；新裁定「`ttl: 0` 不禁止取物」；§4.2.6.3 自陳「跨重啟的在席未定義」為待決 |
