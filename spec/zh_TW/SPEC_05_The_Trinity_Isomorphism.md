...
*   **型別名字指稱單一值（MUST，2026-09-16 新增；裁定 D68）**：`@X` **必須**是**一個**值，
    同時承載約束側（`%kind`／`%name`／衍生的 `%super`）與標準根節點側的欄位（如 `%fmap`）。
    **`@X.f` 必須是對該值的普通取欄位**；實作**不得**以「先按名字查標準根」的路徑取代它
    ——那條路徑**遮蔽**約束側，使一個沒有標準根節點的名字**連它明明有的欄位都取不到**。
    **型別是帶得走的值（MUST）**：把 `@X` 綁到名字、放進容器、傳遞之後，**必須**仍是同一個值。

> **論證性量測（2026-09-16，參考實作 v0.50.0 → Q-047）**：改動前 `@X.%name` 只對
> `@list`／`@option`／`@result` 三個有答案，對其餘九個測過的名字**全回 `_`**，
> 而 `(@X).%name` 十二個全有——**同一個名字，兩種拼法，兩個答案**；
> 且 `t: @list` 之後 `t.%fmap` 為 `_`、`m.t.%fmap` 亦然 ⟹ **型別帶不走**。
> **本節自己的例子當時寫的就是那個會動的拼法**（`(@int).%name` → `"int"`）。
> 改動後〔量，**12 名字 × 4 欄位 ＝ 48 格**〕不一致 **0**，
> 且型別在兩層 combo 與 **list 的元素**裡皆保有節點。
> **⚠ 這是一次紀元，範圍已量且很窄**：`@X` 由約束 marker 改為標準根節點，
> ⟹ **凡存過型別值的宇宙，其根位址改變**〔量〕`v: @list` 的根 `5debf7de…` → `b8a028d5…`；
> 而〔量〕conformance 語料、引擎測試與範例**三處把型別當值存的檔案皆為 0**。
> **標準根自身、`x: 0` 根與物件數不動**——`%super` 是本節所定的**衍生**反映欄，
> 〔量〕在標準根裡出現 **0 次**，合成單一值**不需要存它**。
