...
| **`v0.15.0-draft.1`** (目前) | **`v2`**（凍結） | `v1`~`v2` | **視界參數的歸屬（W4″）**。SPEC_09 **§6.0.1「視界參數不進歷史」**：`~%Config` **不得**進入被提交的根——同一份定義，加不加旋鈕覆寫，**提交後的根 CAID 必須相同**；旋鈕**不得**使相鄰的普通寫入失敗；未被提交**必須說出來**；覆寫**必須**續存於暫存區。SPEC_09 **§6.0.2「哪些旋鈕治理哪個階段」**：旋鈕**必須**治理它所命名的那道閘——**一個設得進去、讀得回來、而不改變任何事情的旋鈕，比沒有這個旋鈕更糟**；階段歸屬**必須**成文。自陳缺口：歸屬表本身尚未裁定 |
