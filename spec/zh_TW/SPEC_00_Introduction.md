...
| **`v0.26.0-draft.1`** (目前) | **`v2`**(凍結) | `v1`~`v2` | **從未被寫下來的那一半。** 標準根**不再併入根的身分**——根**指名**它,而那個指名進入雜湊。**破壞性 #16,身分軸,90 天時鐘重啟**;讀相容雙向成立,舊倉不需遷移〔量,兩個真二進位〕。**SPEC_08 §4.1 新增兩條 MUST**:傳染而得的效果**不得因為住在別處而不進 CAID**〔量:此前一個從環境讀進來的值與一個打進去的值**位址相同**,而該程式甚至不需要偽裝〕;正準形**欄位缺席即 `#pure`**,兩條界線入文(判準是格的 ⊥ 故 `#cached` 必須落成欄位;正規化必須在靜態守護**之後**,否則正規化器會消滅一個該被抓到的謊)——**REAL_03 §6.7「位元組為值之函數」此前被違反著,本版關閉**。**§6.8 判例二(O63)**:持有舊的**值**不夠,還要持有舊的**規則**;讀法歸容器宣告,**編碼軸自 2026-07-28 引入以來第一次被真的撥動**。**§6.8.1 更正**一句被量測推翻的斷言,並自本版起為真。順帶關閉**四個孤兒**——孤兒從來不是 `/add` 一個 |
