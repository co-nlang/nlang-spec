...
| **`v0.13.0-draft.1`** (目前) | **`v2`**（凍結） | `v1`~`v2` | 引擎不得以自身的表示回答（REAL_01 §1.3 新設）：SPEC_10 §2.2.1 的「不得洩漏實作表示」升為通則，並補上——**值的正準文字形必須是全函數**（不得有「其餘一律以宿主語言格式輸出」的退回分支）、**印出來的必須讀得回去**（未觀測者印其來源、持有型引用印其結構形）、**未觀測不是錯誤**（不得為了印它而觀測它）。自陳缺口：要求**可剖析**而非**可重求值**，後者需環境隨行，屬另一個決定 |
